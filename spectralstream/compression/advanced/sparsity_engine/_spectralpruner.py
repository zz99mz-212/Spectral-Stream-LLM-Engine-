...
class SpectralPruner(BasePruner):
    def prune(self, weights: np.ndarray, target_sparsity: float,
              layer_name: str = "", **kwargs) -> PruningResult:
        w_spec = _dct(weights)
        power = w_spec ** 2
        total_power = np.sum(power)
        n_coeffs = power.size
        n_keep = max(1, int(n_coeffs * (1.0 - target_sparsity)))
        flat_idx = np.argsort(-power.ravel())
        keep_idx = flat_idx[:n_keep]
        mask_spec = np.zeros(n_coeffs, dtype=bool)
        mask_spec[keep_idx] = True
        mask_spec_2d = mask_spec.reshape(weights.shape)
        energy_kept = float(np.sum(power.ravel()[keep_idx]) / (total_power + 1e-30))
        return PruningResult(
            mask=mask_spec_2d, sparsity=target_sparsity, strategy="spectral",
            layer_name=layer_name, importance_scores=power,
            metrics={"energy_kept": energy_kept},
        )
