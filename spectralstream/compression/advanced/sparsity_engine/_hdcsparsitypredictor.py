...
class HDCSparsityPredictor:
    def __init__(self, hidden_dim: int = 512, hd_dim: int = 4096,
                 seed: int = 42):
        self.hidden_dim = hidden_dim
        self.hd_dim = hd_dim
        rng = np.random.RandomState(seed)
        self._projection = rng.randn(hidden_dim, hd_dim).astype(np.float32)
        self._projection /= np.linalg.norm(self._projection, axis=0, keepdims=True) + 1e-10
        self._thresholds: dict[str, float] = {}

    def predict(self, hidden: np.ndarray, layer_name: str = "",
                target_sparsity: float = 0.9) -> np.ndarray:
        hd_vec = hidden @ self._projection
        hd_sign = np.sign(hd_vec)
        scores = np.abs(np.mean(hd_sign, axis=0))
        n_keep = max(1, int(self.hd_dim * (1.0 - target_sparsity)))
        keep_idx = np.argsort(-scores)[:n_keep]
        mask = np.zeros(self.hd_dim, dtype=bool)
        mask[keep_idx] = True
        return mask

    def set_threshold(self, layer_name: str, threshold: float):
        self._thresholds[layer_name] = threshold
