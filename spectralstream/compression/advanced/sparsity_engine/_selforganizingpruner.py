...
class SelfOrganizingPruner(BasePruner):
    def __init__(
        self,
    ...
    def prune(
        ...
        **kwargs,
        ...
        rows, cols = w.shape
        mask = np.abs(w) >= float(np.percentile(np.abs(w), 50))
        for it in range(self.n_iterations):
            ...
            for i in range(rows):
                for j in range(cols):
                    ...
                    local_density = float(np.mean(nbhd))
                    global_density = float(np.mean(mask))
                    if local_density > global_density * 1.5:
                        new_mask[i, j] = True
                    elif local_density < global_density * 0.3:
                        if np.random.random() > local_density:
                            new_mask[i, j] = False
            mask = new_mask
        target = 1.0 - target_sparsity
        current = float(np.mean(mask))
        if current > target:
            excess = current - target
            zeroable = np.where(
                mask & (np.abs(w) < float(np.percentile(np.abs(w[mask]), 50)))
            )[0]
            n_remove = min(len(zeroable), int(excess * mask.size))
            if n_remove > 0:
                remove_idx = np.random.choice(zeroable, n_remove, replace=False)
                mask.ravel()[remove_idx] = False
        elif current < target:
            deficit = target - current
            nonzero = np.where(
                ~mask & (np.abs(w) > float(np.percentile(np.abs(w[~mask]), 50)))
            )[0]
            n_add = min(len(nonzero), int(deficit * mask.size))
            if n_add > 0:
                add_idx = np.random.choice(nonzero, n_add, replace=False)
                mask.ravel()[add_idx] = True
        actual = 1.0 - float(np.mean(mask))
        return PruningResult(
            mask=mask,
            sparsity=actual,
            strategy="self_organizing",
            layer_name=layer_name,
            importance_scores=np.abs(w),
        )
