...
class ResonantSparsity:
    def __init__(self, n_frequency_bands: int = 8):
        self.n_frequency_bands = n_frequency_bands

    def compute_resonance(self, data: np.ndarray,
                          sample_rate: float = 1.0) -> np.ndarray:
        spec = np.abs(np.fft.fft(data.astype(np.float64), axis=-1))
        freqs = np.fft.fftfreq(data.shape[-1], d=1.0 / sample_rate)
        positive = freqs >= 0
        spec = spec[..., positive]
        bands = np.array_split(spec, self.n_frequency_bands, axis=-1)
        resonance = np.array([b.mean() for b in bands])
        return resonance / (resonance.sum() + 1e-30)

    def keep_resonant_weights(self, weights: np.ndarray,
                               training_data_frequency: Optional[np.ndarray] = None,
                               keep_fraction: Optional[float] = None) -> np.ndarray:
        if keep_fraction is None:
            keep_fraction = 0.1
        w_spec = _dct(weights)
        if training_data_frequency is not None:
            freq_weights = np.abs(training_data_frequency)
            if freq_weights.shape != w_spec.shape:
                freq_weights = np.ones_like(w_spec)
            resonance = np.abs(w_spec) * freq_weights
        else:
            resonance = np.abs(w_spec)
        n_keep = max(1, int(resonance.size * keep_fraction))
        flat_idx = np.argsort(-resonance.ravel())[:n_keep]
        mask = np.zeros(resonance.size, dtype=bool)
        mask[flat_idx] = True
        out_spec = w_spec * mask.reshape(w_spec.shape)
        return _idct(out_spec).astype(weights.dtype)
