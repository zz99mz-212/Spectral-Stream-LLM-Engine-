...
class CombinedPruner(BasePruner):
    def __init__(
        ...
        **kwargs,
        ...
        self._pruners = {
            "magnitude": MagnitudePruner(pattern),
            "wanda": WandaPruner(pattern),
            "sparsegpt": SparseGPTPruner(pattern),
            "spectral": SpectralPruner(pattern),
            "movement": MovementPruner(pattern),
        }
    ...
    def prune(
        ...
        **kwargs,
        ...
        all_scores = np.zeros_like(weights, dtype=np.float64)
        for name, pruner in self._pruners.items():
            if name not in self.weights or self.weights[name] <= 0:
                continue
            result = pruner.prune(weights, target_sparsity, layer_name, **kwargs)
            all_scores += self.weights[name] * result.importance_scores

        all_scores /= sum(self.weights.values())
        mask = self._apply_pattern(all_scores, target_sparsity)
