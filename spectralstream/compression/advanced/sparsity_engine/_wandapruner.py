...
class WandaPruner(BasePruner):
    def prune(self, weights: np.ndarray, target_sparsity: float,
              layer_name: str = "", activation_norm: Optional[np.ndarray] = None,
              **kwargs) -> PruningResult:
        if activation_norm is None:
            activation_norm = np.ones(weights.shape[-1], dtype=np.float64)
        else:
            activation_norm = np.asarray(activation_norm, dtype=np.float64).ravel()
            if len(activation_norm) < weights.shape[-1]:
                activation_norm = np.pad(activation_norm,
                                         (0, weights.shape[-1] - len(activation_norm)))
        scores = np.abs(weights) * activation_norm[np.newaxis, :]
        mask = self._apply_pattern(scores, target_sparsity)
        actual = 1.0 - float(np.mean(mask))
        return PruningResult(
            mask=mask, sparsity=actual, strategy="wanda",
