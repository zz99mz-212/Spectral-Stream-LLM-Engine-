...
class VlasovPruner(BasePruner):
    def prune(self, weights: np.ndarray, target_sparsity: float,
        ...
        if token_interactions is not None:
            interaction_strength = np.abs(token_interactions).mean(axis=0)
            if interaction_strength.shape != w.shape:
                interaction_strength = np.ones_like(w)
        else:
            interaction_strength = np.ones_like(w)
        row_norms = np.linalg.norm(w, axis=1, keepdims=True) + 1e-10
        col_norms = np.linalg.norm(w, axis=0, keepdims=True) + 1e-10
        mean_field = row_norms * col_norms / np.sqrt(w.size)
        scores = np.abs(w) * interaction_strength * mean_field
        mask = self._apply_pattern(scores, target_sparsity)
        actual = 1.0 - float(np.mean(mask))
        return PruningResult(
            mask=mask, sparsity=actual, strategy="vlasov",
