...
class ResonantPruner(BasePruner):
    def prune(self, weights: np.ndarray, target_sparsity: float,
        ...
        if training_frequencies is not None:
            w_spec = _dct(w)
            freq_mask = np.abs(training_frequencies)
            if freq_mask.shape != w_spec.shape:
                freq_mask = np.ones_like(w_spec)
            resonance = np.abs(w_spec) * freq_mask
            scores = _idct(resonance)
        else:
            w_spec = _dct(w)
            power = w_spec ** 2
            total = np.sum(power)
