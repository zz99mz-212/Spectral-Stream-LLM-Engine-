...
class MultiStableSubcriticalPitchfork:
    """Subcritical pitchfork"""

    name = "multistable_subcritical_pitchfork"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        rsp = _extract_chaos_param(tensor) * 2.0 - 1.0
        x = 0.1
        traj = np.zeros(256)
        for i in range(256):
            dx = rsp * x + x**3 - x**5
            ...
            x = np.clip(x, -3, 3)
            traj[i] = x
        Smod = _modulate_by_attractor(Sk, traj, rsp)
        return _hdr(m, n, k) + struct.pack("<f", rsp) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
