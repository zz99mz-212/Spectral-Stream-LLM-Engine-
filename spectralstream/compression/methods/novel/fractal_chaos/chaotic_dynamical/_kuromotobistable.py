...
class KuromotoBistable:
    """Bistable"""

    name = "kuromoto_bistable"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        K_bi = _extract_chaos_param(tensor) * 10.0 + 2.0
        traj = _kuromoto(16, K_bi, 0.1, 256)
        Smod = _modulate_by_attractor(Sk, traj)
        return _hdr(m, n, k) + struct.pack("<f", K_bi) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
