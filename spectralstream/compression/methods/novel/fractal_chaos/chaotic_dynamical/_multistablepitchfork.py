...
class MultiStablePitchfork:
    """Pitchfork bifurcation"""

    name = "multistable_pitchfork"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        rp = _extract_chaos_param(tensor) * 2.0 - 1.0
        x = 0.1
        traj = np.zeros(256)
        for i in range(256):
            dx = rp * x - x**3
            ...
            x = np.clip(x, -5, 5)
            traj[i] = x
        Smod = _modulate_by_attractor(Sk, traj, rp)
        return _hdr(m, n, k) + struct.pack("<f", rp) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
