...
class LogisticTangentBifurcation:
    """Tangent bifurcation"""

    name = "logistic_tangent_bifurcation"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        r3 = 3.8282 + 0.05 * _extract_chaos_param(tensor)
        traj = _logistic(r3, 0.5, 512)
        Smod = _modulate_by_attractor(Sk, traj)
        return _hdr(m, n, k) + struct.pack("<f", r3) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
