...
class HybridStochasticResonance:
    """Stochastic resonance"""

    name = "hybrid_stochastic_resonance"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        sr_h = _extract_chaos_param(tensor) * 0.3
        x = 0.5
        traj = np.zeros(256)
        a_sr = 0.5
        for i in range(256):
            drive = a_sr * math.sin(0.1 * i)
            dx = 4 * x * (1 - x) - 0.5 * x + drive + sr_h * np.random.randn()
            x += 0.01 * dx
            x = np.clip(x, 0, 1)
            traj[i] = x
        Smod = _modulate_by_attractor(Sk, traj, sr_h)
        return _hdr(m, n, k) + struct.pack("<f", sr_h) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
