...
class KuromotoHierarchical:
    """Hierarchical"""

    name = "kuromoto_hierarchical"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        hier = _extract_chaos_param(tensor) * 3.0
        traj = _kuromoto(32, 5.0, 1.0, 128)
        Smod = _modulate_by_attractor(Sk, traj, hier)
        return _hdr(m, n, k) + struct.pack("<f", hier) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
