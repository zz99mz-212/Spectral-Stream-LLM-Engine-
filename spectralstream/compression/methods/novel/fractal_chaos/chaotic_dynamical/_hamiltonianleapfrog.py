...
class HamiltonianLeapfrog:
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        q, p = 0.5, 0.0
        om = 1.0 + _extract_chaos_param(tensor) * 3.0
        traj = np.zeros(256)
        for i in range(256):
            ph = p - 0.005 * (om * om * q)
            q += 0.01 * ph
            p = ph - 0.005 * (om * om * q)
            traj[i] = q
        Smod = _modulate_by_attractor(Sk, traj, om)
        return _hdr(m, n, k) + struct.pack("<f", om) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
