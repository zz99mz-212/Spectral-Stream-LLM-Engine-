...
class HeteroclinicFoldHopf:
    """Fold-Hopf"""

    name = "heteroclinic_fold_hopf"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        fh = _extract_chaos_param(tensor) * 2.0 - 1.0
        x, y, z = 0.2, 0.2, 0.2
        traj = np.zeros(256)
        for i in range(256):
            dx = fh * x - y - x * z
            ...
            z += 0.01 * dz
            traj[i] = x
        Smod = _modulate_by_attractor(Sk, traj, fh)
        return _hdr(m, n, k) + struct.pack("<f", fh) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
