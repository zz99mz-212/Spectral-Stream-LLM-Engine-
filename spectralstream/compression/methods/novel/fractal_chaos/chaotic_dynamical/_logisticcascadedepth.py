...
class LogisticCascadeDepth:
    """Cascade depth"""

    name = "logistic_cascade_depth"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        stp = max(32, min(1024, int(128 * _extract_chaos_param(tensor) + 32)))
        traj = _logistic(3.8, 0.5, stp)
        Smod = _modulate_by_attractor(Sk, traj)
        return _hdr(m, n, k) + struct.pack("<I", stp) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
