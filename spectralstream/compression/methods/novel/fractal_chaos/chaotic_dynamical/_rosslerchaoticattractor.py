...
class RosslerChaoticAttractor:
    """Rossler: chaotic"""

    name = "rossler_chaotic_attractor"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        var = float(np.var(tensor.ravel()))
        c = 5.0 + 10.0 * (var % 1.0)
        traj = _rossler(0.2, 0.2, c, 0.01, 512)
        Smod = _modulate_by_attractor(Sk, traj, var)
