...
class HamiltonianSeparable:
    """Separable"""

    name = "hamiltonian_separable"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        ad = (
            float(
                np.abs(
                    f[: min(len(f) // 2, 128)]
                    + f[min(len(f) // 2, 128) : min(len(f), 256)]
                ).mean()
            )
            if len(f) > 2
            else 0.0
        ...
        for i in range(256):
            ...
            ph = p - 0.005 * dV
            q += 0.01 * Tg
            p = ph - 0.005 * (q + 0.1 * q**3)
            traj[i] = q
        Smod = _modulate_by_attractor(Sk, traj, ad)
        return _hdr(m, n, k) + struct.pack("<f", ad) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
