...
class LorenzRotationSymmetry:
    """Lorenz: rotation"""

    name = "lorenz_rotation_symmetry"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        sr = Sk / max(Sk[0], 1e-30)
        iso = float(np.mean(sr))
        s = 10.0 * (1.0 + iso)
        traj = _lorenz(s, 28.0, 8.0 / 3.0, 0.01, 256)
        Smod = _modulate_by_attractor(Sk, traj)
        return _hdr(m, n, k) + struct.pack("<f", iso) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
