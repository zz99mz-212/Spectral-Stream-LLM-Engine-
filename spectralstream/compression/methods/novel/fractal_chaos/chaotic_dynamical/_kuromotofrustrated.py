...
class KuromotoFrustrated:
    """Frustrated"""

    name = "kuromoto_frustrated"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        frus = _extract_chaos_param(tensor) * math.pi
        traj = _kuromoto(32, 4.0, 1.0, 128)
        Smod = _modulate_by_attractor(Sk, traj, frus)
        return _hdr(m, n, k) + struct.pack("<f", frus) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
