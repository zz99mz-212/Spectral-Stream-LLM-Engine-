...
class DoublePendulumInitialAngle2:
    """Initial angle 2"""

    name = "double_pendulum_initial_angle2"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        t2_0 = 1.0 + 3.0 * _extract_chaos_param(tensor)
        traj = _double_pendulum(9.81, 1.0, 1.0, 0.01, 256)
        Smod = _modulate_by_attractor(Sk, traj, t2_0)
        return _hdr(m, n, k) + struct.pack("<f", t2_0) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
