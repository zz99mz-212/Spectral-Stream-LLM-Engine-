...
class LorenzAttractorBasin:
    """Lorenz: basin"""

    name = "lorenz_attractor_basin"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        th = np.percentile(np.abs(tensor.ravel()), 75)
        s = 10.0 + 5.0 * (th / max(np.max(np.abs(tensor.ravel())), 1e-30))
        traj = _lorenz(s, 28.0, 8.0 / 3.0, 0.01, 256)
        Smod = _modulate_by_attractor(Sk, traj)
        return _hdr(m, n, k) + struct.pack("<f", th) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
