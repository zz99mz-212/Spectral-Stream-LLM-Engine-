...
class KuromotoChimeraState:
    """Chimera state"""

    name = "kuromoto_chimera_state"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        ext = _extract_chaos_param(tensor) * 5.0
        traj = _kuromoto(64, 2.0, ext, 256)
        Smod = _modulate_by_attractor(Sk, traj)
        return _hdr(m, n, k) + struct.pack("<f", ext) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
