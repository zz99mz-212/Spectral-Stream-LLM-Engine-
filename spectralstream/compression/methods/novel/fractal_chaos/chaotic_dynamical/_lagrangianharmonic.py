...
class LagrangianHarmonic:
    """Harmonic osc"""

    name = "lagrangian_harmonic"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        ks = _extract_chaos_param(tensor) * 10.0 + 0.1
        q, v = 0.5, 0.0
        traj = np.zeros(256)
        for i in range(256):
            a = -ks * q
            ...
            q += 0.01 * v
            traj[i] = q
        Smod = _modulate_by_attractor(Sk, traj, ks)
        return _hdr(m, n, k) + struct.pack("<f", ks) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
