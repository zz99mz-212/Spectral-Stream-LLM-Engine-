...
class KuromotoAdaptiveNetworks:
    """Adaptive networks"""

    name = "kuromoto_adaptive_networks"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        adapt = _extract_chaos_param(tensor)
        traj = _kuromoto(24, 3.0, 1.0, 256)
        Smod = _modulate_by_attractor(Sk, traj, adapt)
        return _hdr(m, n, k) + struct.pack("<f", adapt) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
