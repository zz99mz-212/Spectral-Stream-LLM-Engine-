...
class HamiltonianEnergyConserving:
    """Energy dissip"""

    name = "hamiltonian_energy_conserving"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        dis = float(np.mean(np.abs(tensor.ravel())))
        q, p = 0.5, 0.5
        traj = np.zeros(256)
        for i in range(256):
            dH_dq = q
        ...
        return _hdr(m, n, k) + struct.pack("<f", dis) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
