...
class LogisticNoisePerturbed:
    """Noise perturbed"""

    name = "logistic_noise_perturbed"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        na = _extract_chaos_param(tensor) * 0.1
        traj = _logistic(3.8 + na, 0.5, 256)
        Smod = _modulate_by_attractor(Sk, traj)
        return _hdr(m, n, k) + struct.pack("<f", na) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
