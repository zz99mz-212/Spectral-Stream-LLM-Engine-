...
class DoublePendulumAsymmetric:
    """Asymmetric arms"""

    name = "double_pendulum_asymmetric"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        ratio = _extract_chaos_param(tensor) * 3.0 + 0.5
        traj = _double_pendulum(9.81, ratio, 2.0 / ratio, 0.01, 256)
        Smod = _modulate_by_attractor(Sk, traj, ratio)
        return _hdr(m, n, k) + struct.pack("<f", ratio) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
