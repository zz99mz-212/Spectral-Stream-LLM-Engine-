...
class LagrangianCoupledOsc:
    """Coupled osc"""

    name = "lagrangian_coupled_osc"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        kc = 0.1 + 0.5 * _extract_chaos_param(tensor)
        q1, q2, v1, v2 = 0.5, 0.0, 0.0, 0.0
        traj = np.zeros(256)
        for i in range(256):
            a1 = -q1 + kc * (q2 - q1)
            a2 = -q2 + kc * (q1 - q2)
            ...
            q2 += 0.01 * v2
            traj[i] = q1
        Smod = _modulate_by_attractor(Sk, traj, kc)
        return _hdr(m, n, k) + struct.pack("<f", kc) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
