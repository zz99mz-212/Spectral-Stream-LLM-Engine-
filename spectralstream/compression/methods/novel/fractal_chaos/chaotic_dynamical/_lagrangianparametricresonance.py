...
class LagrangianParametricResonance:
    """Parametric"""

    name = "lagrangian_parametric_resonance"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        eps = _extract_chaos_param(tensor) * 2.0
        q, v = 0.1, 0.0
        traj = np.zeros(256)
        for i in range(256):
            om3 = 1.0 + eps * math.sin(2.0 * i * 0.01)
            a = -om3 * om3 * q
            v += 0.01 * a
            q += 0.01 * v
            traj[i] = q
        Smod = _modulate_by_attractor(Sk, traj, eps)
