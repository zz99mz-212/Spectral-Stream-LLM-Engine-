...
class DoublePendulumMagneticPerturbation:
    """Magnetic perturbation"""

    name = "double_pendulum_magnetic_perturbation"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        B_dp = _extract_chaos_param(tensor) * 5.0
        traj = _double_pendulum(9.81 + B_dp, 1.0, 1.0, 0.01, 256)
        Smod = _modulate_by_attractor(Sk, traj)
        return _hdr(m, n, k) + struct.pack("<f", B_dp) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
