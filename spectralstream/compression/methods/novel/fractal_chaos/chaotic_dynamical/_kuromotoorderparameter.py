...
class KuromotoOrderParameter:
    """Order parameter"""

    name = "kuromoto_order_parameter"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        K2 = _extract_chaos_param(tensor) * 8.0 + 1.0
        traj = _kuromoto(32, K2, 1.0, 512)
        Smod = _modulate_by_attractor(Sk, traj)
        return _hdr(m, n, k) + struct.pack("<f", K2) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
