...
class MultiStableCuspCatastrophe:
    """Cusp catastrophe"""

    name = "multistable_cusp_catastrophe"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        a_mc = _extract_chaos_param(tensor) * 2.0 - 1.0
        b_mc = _extract_chaos_param(tensor) * 2.0 - 1.0
        x = 0.0
        traj = np.zeros(256)
        for i in range(256):
            dV = a_mc * x + b_mc * x**3 - x**5
            x += 0.01 * dV
            x = np.clip(x, -3, 3)
            traj[i] = x
        Smod = _modulate_by_attractor(Sk, traj, a_mc)
        return _hdr(m, n, k) + struct.pack("<ff", a_mc, b_mc) + _ser(Uk) + _ser(
            Smod
