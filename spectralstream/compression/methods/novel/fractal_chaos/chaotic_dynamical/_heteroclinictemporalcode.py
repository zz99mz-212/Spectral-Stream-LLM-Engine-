...
class HeteroclinicTemporalCode:
    """Temporal coding"""

    name = "heteroclinic_temporal_code"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        tc = _extract_chaos_param(tensor) * 3.0
        x, y, z = 0.5, 0.0, 0.0
        traj = np.zeros(256)
        for i in range(256):
            dx = z * (1 - z) - tc * x
            ...
            z += 0.01 * dz
            traj[i] = x
        Smod = _modulate_by_attractor(Sk, traj, tc)
        return _hdr(m, n, k) + struct.pack("<f", tc) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
