...
class LorenzPhasePortrait:
    """Lorenz: phase portrait"""

    name = "lorenz_phase_portrait"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        tn = float(np.linalg.norm(tensor.ravel()))
        rad = 10.0 + 50.0 * (tn % 1.0)
        s, r, b = rad * 0.5, 20.0 + rad, 2.0 + rad * 0.1
        traj = _lorenz(s, r, b, 0.01, 256)
        Smod = _modulate_by_attractor(Sk, traj)
        return _hdr(m, n, k) + struct.pack("<f", tn % 1.0) + _ser(Uk) + _ser(
            Smod
