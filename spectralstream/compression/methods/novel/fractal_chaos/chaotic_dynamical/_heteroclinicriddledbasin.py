...
class HeteroclinicRiddledBasin:
    """Riddled basin"""

    name = "heteroclinic_riddled_basin"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        rb_he = _extract_chaos_param(tensor) * 4.0
        x, y = 0.5, 0.0
        traj = np.zeros(256)
        for i in range(256):
            dx = x * (1 - x * x - y * y) - rb_he * y * (1 - x)
            dy = -y * (1 - x * x - y * y) - rb_he * x * (1 - y)
            x += 0.01 * dx
            y += 0.01 * dy
            traj[i] = x
        Smod = _modulate_by_attractor(Sk, traj, rb_he)
        return _hdr(m, n, k) + struct.pack("<f", rb_he) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
