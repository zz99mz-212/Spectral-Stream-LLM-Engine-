...
class RosslerPeriodDoubling:
    """Rossler: period-double"""

    name = "rossler_period_doubling"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        f = tensor.ravel()
        ac = np.correlate(f[: min(len(f), 128)], f[: min(len(f), 128)], mode="valid")
        p = int(np.argmax(ac[1 : min(33, len(ac))]) + 1) if len(ac) > 2 else 1
        c = 4.0 + 0.5 * p
        traj = _rossler(0.2, 0.2, c, 0.01, 256)
        Smod = _modulate_by_attractor(Sk, traj, p * 0.1)
        return _hdr(m, n, k) + struct.pack("<I", p) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
