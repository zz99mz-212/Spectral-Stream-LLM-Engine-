...
class LagrangianDoubleWell:
    """Double well L"""

    name = "lagrangian_double_well"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        dw = _extract_chaos_param(tensor) * 5.0
        q, v = 0.5, 0.0
        traj = np.zeros(256)
        for i in range(256):
            a = 2 * q - 4 * q**3 - dw * v
            ...
            q += 0.01 * v
            traj[i] = q
        Smod = _modulate_by_attractor(Sk, traj, dw)
        return _hdr(m, n, k) + struct.pack("<f", dw) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
