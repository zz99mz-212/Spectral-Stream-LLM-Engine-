...
class DoublePendulumParametricDrive:
    """Parametric drive"""

    name = "double_pendulum_parametric_drive"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        drive = _extract_chaos_param(tensor) * 3.0
        traj = _double_pendulum(9.81 * (1.0 + drive * 0.1), 1.0, 1.0, 0.01, 512)
        Smod = _modulate_by_attractor(Sk, traj)
        return _hdr(m, n, k) + struct.pack("<f", drive) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
