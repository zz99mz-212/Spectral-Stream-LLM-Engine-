...
class MultiStablePotentialLandscape:
    """Potential landscape"""

    name = "multistable_potential_landscape"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        curv = _extract_chaos_param(tensor) * 3.0
        x = 1.0
        traj = np.zeros(256)
        for i in range(256):
            dV = -x + curv * x**3 - x**5
            ...
            x = np.clip(x, -3, 3)
            traj[i] = x
        Smod = _modulate_by_attractor(Sk, traj, curv)
        return _hdr(m, n, k) + struct.pack("<f", curv) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
