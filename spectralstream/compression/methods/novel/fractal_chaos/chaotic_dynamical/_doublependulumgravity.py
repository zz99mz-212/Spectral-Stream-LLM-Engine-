...
class DoublePendulumGravity:
    """Gravity"""

    name = "double_pendulum_gravity"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        g = 5.0 + 15.0 * _extract_chaos_param(tensor)
        traj = _double_pendulum(g, 1.0, 1.0, 0.01, 256)
        Smod = _modulate_by_attractor(Sk, traj)
        return _hdr(m, n, k) + struct.pack("<f", g) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
