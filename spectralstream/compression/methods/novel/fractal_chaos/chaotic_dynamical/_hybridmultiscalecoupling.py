...
class HybridMultiscaleCoupling:
    """Multiscale Lorenz"""

    name = "hybrid_multiscale_coupling"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        ms_h = _extract_chaos_param(tensor) * 3.0
        x, y, z = 1.0, 1.0, 1.0
        ...
        for i in range(256):
            ...
            zs += 0.01 * dzs
            traj[i] = x
        Smod = _modulate_by_attractor(Sk, traj, ms_h)
        return _hdr(m, n, k) + struct.pack("<f", ms_h) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
