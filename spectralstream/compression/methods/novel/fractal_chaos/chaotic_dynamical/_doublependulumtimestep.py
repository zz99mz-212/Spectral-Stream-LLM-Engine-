...
class DoublePendulumTimestep:
    """Timestep"""

    name = "double_pendulum_timestep"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        dt_dp = 0.005 + 0.02 * _extract_chaos_param(tensor)
        traj = _double_pendulum(9.81, 1.0, 1.0, dt_dp, 256)
        Smod = _modulate_by_attractor(Sk, traj)
        return _hdr(m, n, k) + struct.pack("<f", dt_dp) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
