...
class LorenzEnergeticBound:
    """Lorenz: energy bound"""

    name = "lorenz_energetic_bound"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        cv = (
            float(np.cov(tensor.ravel()[: min(len(tensor.ravel()), 256)])[0, 0])
            if tensor.size > 1
            else 1.0
        )
        s = 5.0 + 10.0 * (cv % 1.0)
        ...
        return _hdr(m, n, k) + struct.pack("<f", cv) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
