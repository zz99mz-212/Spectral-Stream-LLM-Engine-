...
class HeteroclinicTorusBreakdown:
    """Torus breakdown"""

    name = "heteroclinic_torus_breakdown"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        tb = _extract_chaos_param(tensor) * 2.0
        x, y = 0.5, 0.0
        traj = np.zeros(256)
        for i in range(256):
            dx = x * (1 - x * x - y * y) - tb * y
            ...
            x += 0.01 * dx
            y += 0.01 * dy
            traj[i] = math.atan2(y, x)
        Smod = _modulate_by_attractor(Sk, traj, tb)
        return _hdr(m, n, k) + struct.pack("<f", tb) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
