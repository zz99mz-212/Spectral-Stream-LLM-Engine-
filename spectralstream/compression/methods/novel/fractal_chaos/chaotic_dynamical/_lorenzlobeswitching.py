...
class LorenzLobeSwitching:
    """Lorenz: lobe-switch"""

    name = "lorenz_lobe_switching"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        f = tensor.ravel()
        cr = np.sum(np.diff(np.sign(f[: min(len(f), 512)])) != 0) / max(
            min(len(f), 512), 1
        )
        s = 8.0 + 8.0 * cr
        traj = _lorenz(s, 28.0, 8.0 / 3.0, 0.01, 256)
        Smod = _modulate_by_attractor(Sk, traj, cr)
        return _hdr(m, n, k) + struct.pack("<f", cr) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
