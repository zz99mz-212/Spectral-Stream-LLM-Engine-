...
class LogisticSuperstable:
    """Superstable windows"""

    name = "logistic_superstable"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        rs = 3.2 + 0.6 * _extract_chaos_param(tensor)
        traj = _logistic(rs, 0.5, 512)
        Smod = _modulate_by_attractor(Sk, traj)
        return _hdr(m, n, k) + struct.pack("<f", rs) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
