...
class LorenzProjectionY:
    """Lorenz: Y-axis V"""

    name = "lorenz_projection_y"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        traj = _lorenz(10.0, 28.0, 8.0 / 3.0, 0.01, max(128, k * 2))
        p = np.roll(traj, 1)[:k] * 0.01
        Vk_mod = Vk * (1.0 + p[: min(k, len(p))][:, None] * 0.05)
        return _hdr(m, n, k) + _ser(Uk) + _ser(Sk) + _ser(Vk_mod), {
            "shape": tensor.shape
        }

    def decompress(self, data: bytes, metadata: dict) -> np.ndarray:
        m, n, k, p = _rhdr(data)
        return _recon(data, p, m, n, k, metadata["shape"])
