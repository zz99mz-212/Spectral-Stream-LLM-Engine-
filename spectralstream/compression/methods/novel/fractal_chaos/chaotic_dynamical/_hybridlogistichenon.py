...
class HybridLogisticHenon:
    """Logistic-Henon"""

    name = "hybrid_logistic_henon"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        lh_h = _extract_chaos_param(tensor) * 0.5
        x = 0.5
        u, v = 0.0, 0.0
        traj = np.zeros(256)
        for i in range(256):
            x = 3.8 * x * (1 - x) + lh_h * u
            un = 1.0 - 1.4 * u * u + v + lh_h * x
            vn = 0.3 * u
            u, v = un, vn
            traj[i] = x
        Smod = _modulate_by_attractor(Sk, traj, lh_h)
        return _hdr(m, n, k) + struct.pack("<f", lh_h) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
