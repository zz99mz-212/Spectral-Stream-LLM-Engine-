...
class HamiltonianKepler:
    """Kepler"""

    name = "hamiltonian_kepler"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        ecc = _extract_chaos_param(tensor) * 0.9 + 0.1
        q1, q2, p1, p2 = (
            1.0,
            0.0,
            0.0,
            math.sqrt((1 - ecc) / (1 + ecc)) if ecc < 1 else 0.0,
        )
        traj = np.zeros(256)
        for i in range(256):
            r3 = (q1 * q1 + q2 * q2) ** 1.5
            a1, a2 = -q1 / (r3 + 1e-30), -q2 / (r3 + 1e-30)
            p1 += 0.01 * a1
            p2 += 0.01 * a2
            q1 += 0.01 * p1
            q2 += 0.01 * p2
            traj[i] = q1
        Smod = _modulate_by_attractor(Sk, traj, ecc)
        return _hdr(m, n, k) + struct.pack("<f", ecc) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
