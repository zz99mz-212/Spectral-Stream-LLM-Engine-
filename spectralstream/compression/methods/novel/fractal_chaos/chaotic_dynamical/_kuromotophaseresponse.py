...
class KuromotoPhaseResponse:
    """Phase response"""

    name = "kuromoto_phase_response"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        prc = _extract_chaos_param(tensor) * 2.0
        traj = _kuromoto(32, 3.0, prc, 256)
        Smod = _modulate_by_attractor(Sk, traj, prc)
        return _hdr(m, n, k) + struct.pack("<f", prc) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
