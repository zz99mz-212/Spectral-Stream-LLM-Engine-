...
class HeteroclinicBursting:
    """Bursting"""

    name = "heteroclinic_bursting"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        bu_he = _extract_chaos_param(tensor) * 5.0
        v, w_ = -0.3, 0.3
        traj = np.zeros(512)
        a_f = 0.7
        b_f = 0.8
        for i in range(512):
            dv = v * (v - a_f) * (1 - v) - w_ + bu_he * math.sin(0.1 * i)
            dw_ = b_f * (v - w_)
            v += 0.01 * dv
            w_ += 0.01 * dw_
            traj[i] = v
        Smod = _modulate_by_attractor(Sk, traj, bu_he)
        return _hdr(m, n, k) + struct.pack("<f", bu_he) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
