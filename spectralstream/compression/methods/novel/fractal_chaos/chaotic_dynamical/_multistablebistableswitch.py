...
class MultiStableBistableSwitch:
    """Bistable switch"""

    name = "multistable_bistable_switch"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        bias_m = 0.5 + _extract_chaos_param(tensor)
        traj = _logistic(4.0, bias_m, 128)
        Smod = _modulate_by_attractor(Sk, traj)
        return _hdr(m, n, k) + struct.pack("<f", bias_m) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
