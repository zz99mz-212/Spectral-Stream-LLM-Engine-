...
class MultiStableAttractorRivalry:
    """Attractor rivalry"""

    name = "multistable_attractor_rivalry"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        riv = _extract_chaos_param(tensor) * 5.0
        x1, x2 = 0.3, 0.7
        traj = np.zeros(256)
        for i in range(256):
            dx1 = x1 * (1 - x1 - riv * x2)
            ...
            x2 = np.clip(x2, 0, 1)
            traj[i] = x1 - x2
        Smod = _modulate_by_attractor(Sk, traj, riv)
        return _hdr(m, n, k) + struct.pack("<f", riv) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
