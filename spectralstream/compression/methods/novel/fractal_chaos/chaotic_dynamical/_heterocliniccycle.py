...
class HeteroclinicCycle:
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        eps_he = _extract_chaos_param(tensor) * 0.5
        x, y = 0.1, 0.1
        traj = np.zeros(256)
        for i in range(256):
            dx = x * (1 - x) * (x - 0.5) - eps_he * x * y
            dy = y * (1 - y) * (y - 0.5) - eps_he * x * y
            x += 0.01 * dx
            y += 0.01 * dy
            x = np.clip(x, -1, 2)
            y = np.clip(y, -1, 2)
            traj[i] = x
        Smod = _modulate_by_attractor(Sk, traj, eps_he)
        return _hdr(m, n, k) + struct.pack("<f", eps_he) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
