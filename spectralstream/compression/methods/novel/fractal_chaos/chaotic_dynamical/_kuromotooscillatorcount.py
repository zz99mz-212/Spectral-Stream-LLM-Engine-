...
class KuromotoOscillatorCount:
    """Oscillator count"""

    name = "kuromoto_oscillator_count"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        N_osc = max(4, min(128, int(32 * _extract_chaos_param(tensor) + 8)))
        traj = _kuromoto(N_osc, 3.0, 1.0, 256)
        Smod = _modulate_by_attractor(Sk, traj)
        return _hdr(m, n, k) + struct.pack("<I", N_osc) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
