...
class LagrangianResonanceCapture:
    """Resonance capture"""

    name = "lagrangian_resonance_capture"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        eta = _extract_chaos_param(tensor) * 3.0
        q, v = 0.0, 0.0
        traj = np.zeros(256)
        for i in range(256):
            om4 = 1.0 + eta * math.sin(i * 0.01)
            ga = 0.1
            a = -om4 * om4 * q - ga * v + math.cos(1.2 * i * 0.01)
            v += 0.01 * a
            q += 0.01 * v
            traj[i] = q
        Smod = _modulate_by_attractor(Sk, traj, eta)
        return _hdr(m, n, k) + struct.pack("<f", eta) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
