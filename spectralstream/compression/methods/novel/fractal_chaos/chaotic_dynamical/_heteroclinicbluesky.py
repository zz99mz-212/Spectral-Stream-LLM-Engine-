...
class HeteroclinicBlueSky:
    """Blue sky catastrophe"""

    name = "heteroclinic_blue_sky"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        bs = _extract_chaos_param(tensor) * 0.5
        x, y = 0.5, 0.0
        traj = np.zeros(512)
        for i in range(512):
            dx = y + bs * x * (1 - x * x)
            dy = -x * (1 + x) + bs * y * (1 - x * x) + bs * math.cos(0.1 * i)
            x += 0.01 * dx
            y += 0.01 * dy
            traj[i] = x
        Smod = _modulate_by_attractor(Sk, traj, bs)
        return _hdr(m, n, k) + struct.pack("<f", bs) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
