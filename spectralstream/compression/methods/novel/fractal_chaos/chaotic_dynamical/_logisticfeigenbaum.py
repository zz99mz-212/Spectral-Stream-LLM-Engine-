...
class LogisticFeigenbaum:
    """Feigenbaum delta"""

    name = "logistic_feigenbaum"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        fd = _extract_chaos_param(tensor) * 0.5
        traj = _logistic(3.5699 + fd, 0.5, 256)
        Smod = _modulate_by_attractor(Sk, traj)
        return _hdr(m, n, k) + struct.pack("<f", fd) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
