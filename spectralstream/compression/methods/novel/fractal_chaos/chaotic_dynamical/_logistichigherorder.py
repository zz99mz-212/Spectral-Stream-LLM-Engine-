...
class LogisticHigherOrder:
    """Higher order"""

    name = "logistic_higher_order"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        ho = _extract_chaos_param(tensor) * 2.0 + 2.0
        traj = _logistic(min(ho, 4.0), 0.5, 256)
        Smod = _modulate_by_attractor(Sk, traj, ho)
        return _hdr(m, n, k) + struct.pack("<f", ho) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
