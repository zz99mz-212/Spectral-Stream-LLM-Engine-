...
class MultiStableTristable:
    """Tristable"""

    name = "multistable_tristable"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        p_ms = _extract_chaos_multi(tensor, 2)
        a_m = 0.5 + 2.0 * p_ms[0]
        b_m = 0.5 + 2.0 * p_ms[1]
        x = 0.5
        traj = np.zeros(256)
        for i in range(256):
            dx = a_m * x - b_m * x**3 - x**5
            x += 0.01 * dx
            x = np.clip(x, -3, 3)
            traj[i] = x
        Smod = _modulate_by_attractor(Sk, traj, a_m)
        return _hdr(m, n, k) + struct.pack("<ff", a_m, b_m) + _ser(Uk) + _ser(
            Smod
