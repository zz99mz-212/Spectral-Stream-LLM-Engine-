...
class HybridKuromotoLogistic:
    """Kuromoto-Logistic"""

    name = "hybrid_kuromoto_logistic"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        kl_h = _extract_chaos_param(tensor) * 3.0
        traj = _kuromoto(16, kl_h, 1.0, 256)
        Smod = _modulate_by_attractor(Sk, traj, kl_h)
        return _hdr(m, n, k) + struct.pack("<f", kl_h) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
