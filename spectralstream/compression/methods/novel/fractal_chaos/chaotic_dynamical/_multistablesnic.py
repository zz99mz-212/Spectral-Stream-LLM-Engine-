...
class MultiStableSNIC:
    """Saddle-node on IC"""

    name = "multistable_snic"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        snic = _extract_chaos_param(tensor) * 2.0 - 1.0
        x = 0.0
        traj = np.zeros(256)
        for i in range(256):
            dx = snic + 1.0 - math.cos(x) + 0.1
            x += 0.01 * dx
            x = np.clip(x, -math.pi, math.pi)
            traj[i] = x
        Smod = _modulate_by_attractor(Sk, traj, snic)
        return _hdr(m, n, k) + struct.pack("<f", snic) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
