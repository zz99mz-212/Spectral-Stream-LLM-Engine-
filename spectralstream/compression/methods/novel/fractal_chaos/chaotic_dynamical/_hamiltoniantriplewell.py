...
class HamiltonianTripleWell:
    """Triple well"""

    name = "hamiltonian_triple_well"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        mo = int(np.sum(np.diff(np.sign(np.diff(h))) < 0))
        q, p = 0.1, 0.0
        traj = np.zeros(256)
        for i in range(256):
            dH_dq = -6.0 * q + 12.0 * q**3 - 6.0 * q**5
            dH_dp = p
            p += 0.01 * (-dH_dq)
            q += 0.01 * dH_dp
            traj[i] = q
        Smod = _modulate_by_attractor(Sk, traj, mo)
        return _hdr(m, n, k) + struct.pack("<I", mo) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
