...
class LorenzFullParams:
    """Lorenz: full params"""

    name = "lorenz_full_params"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        p = _extract_chaos_multi(tensor)
        s, r, b = 5.0 + 10.0 * p[0], 20.0 + 40.0 * p[1], 1.5 + 4.0 * p[2]
        traj = _lorenz(s, r, b, 0.01, 256)
        Smod = _modulate_by_attractor(Sk, traj)
        return _hdr(m, n, k) + struct.pack("<fff", s, r, b) + _ser(Uk) + _ser(
            Smod
