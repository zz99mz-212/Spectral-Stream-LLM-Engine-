...
class HybridAdaptiveCoupling:
    """Adaptive coupling"""

    name = "hybrid_adaptive_coupling"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        ac_h = _extract_chaos_param(tensor) * 2.0
        x = 0.5
        y = 0.0
        coupl = 0.5
        traj = np.zeros(256)
        for i in range(256):
            x = 3.8 * x * (1 - x) + coupl * y
            y = 3.5 * y * (1 - y) + coupl * x
            coupl += ac_h * 0.001 * (x * y - coupl)
            traj[i] = x
        Smod = _modulate_by_attractor(Sk, traj, ac_h)
        return _hdr(m, n, k) + struct.pack("<f", ac_h) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
