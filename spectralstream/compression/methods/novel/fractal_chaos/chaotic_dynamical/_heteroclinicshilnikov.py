...
class HeteroclinicShilnikov:
    """Shilnikov attractor"""

    name = "heteroclinic_shilnikov"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        sh = _extract_chaos_param(tensor) * 2.0 - 1.0
        x, y, z = 0.5, 0.0, 0.0
        traj = np.zeros(256)
        for i in range(256):
            dx = y
            ...
            z += 0.01 * dz
            traj[i] = x
        Smod = _modulate_by_attractor(Sk, traj, sh)
        return _hdr(m, n, k) + struct.pack("<f", sh) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
