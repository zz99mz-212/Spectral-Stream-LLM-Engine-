...
class KuromotoSynchronization:
    """Global coupling"""

    name = "kuromoto_synchronization"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        K = _extract_chaos_param(tensor) * 10.0 + 0.5
        traj = _kuromoto(32, K, 1.0, 256)
        Smod = _modulate_by_attractor(Sk, traj)
        return _hdr(m, n, k) + struct.pack("<f", K) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
