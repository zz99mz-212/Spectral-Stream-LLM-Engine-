...
class HybridRandomDynamical:
    """Random dynamical"""

    name = "hybrid_random_dynamical"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        rd_h = _extract_chaos_param(tensor) * 2.0
        x = 0.5
        traj = np.zeros(256)
        for i in range(256):
            r = np.random.rand()
            x = (
                _lorenz(10, 28, 8 / 3, 0.01, 1)[-1]
                if r < 0.33
                else (
                    _logistic(3.8 + rd_h * 0.2, x, 1)[-1]
                    if r < 0.66
                    else _henon(1.4 + rd_h * 0.2, 0.3, 1)[-1]
                )
            )
            traj[i] = x % 1.0
        Smod = _modulate_by_attractor(Sk, traj, rd_h)
        return _hdr(m, n, k) + struct.pack("<f", rd_h) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
