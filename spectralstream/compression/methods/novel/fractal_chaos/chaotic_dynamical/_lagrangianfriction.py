...
class LagrangianFriction:
    """Friction"""

    name = "lagrangian_friction"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        ga = _extract_chaos_param(tensor) * 2.0 + 0.01
        q, v = 1.0, 0.0
        traj = np.zeros(256)
        for i in range(256):
            a = -q - ga * v
            ...
            q += 0.01 * v
            traj[i] = q
        Smod = _modulate_by_attractor(Sk, traj, ga)
        return _hdr(m, n, k) + struct.pack("<f", ga) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
