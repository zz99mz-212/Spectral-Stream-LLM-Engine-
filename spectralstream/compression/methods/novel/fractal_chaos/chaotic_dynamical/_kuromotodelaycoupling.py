...
class KuromotoDelayCoupling:
    """Delay coupling"""

    name = "kuromoto_delay_coupling"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        ds = int(1 + 5 * _extract_chaos_param(tensor))
        traj = _kuromoto(32, 3.0, 1.0, 256)
        Smod = _modulate_by_attractor(Sk, traj, float(ds))
        return _hdr(m, n, k) + struct.pack("<I", ds) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
