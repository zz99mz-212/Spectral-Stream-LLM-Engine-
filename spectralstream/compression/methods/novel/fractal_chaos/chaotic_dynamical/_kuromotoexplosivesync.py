...
class KuromotoExplosiveSync:
    """Explosive sync"""

    name = "kuromoto_explosive_sync"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        K_exp = _extract_chaos_param(tensor) * 6.0 + 3.0
        traj = _kuromoto(32, K_exp, 2.0, 256)
        Smod = _modulate_by_attractor(Sk, traj)
        return _hdr(m, n, k) + struct.pack("<f", K_exp) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
