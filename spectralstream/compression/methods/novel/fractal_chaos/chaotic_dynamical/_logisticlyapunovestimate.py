...
class LogisticLyapunovEstimate:
    """Lyapunov estimate"""

    name = "logistic_lyapunov_estimate"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        rl = 3.0 + 1.0 * _extract_chaos_param(tensor)
        traj = _logistic(rl, 0.5, 256)
        Smod = _modulate_by_attractor(Sk, traj)
        return _hdr(m, n, k) + struct.pack("<f", rl) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
