...
class HeteroclinicSaddleConnection:
    """Saddle connection"""

    name = "heteroclinic_saddle_connection"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        sc = _extract_chaos_param(tensor) * 2.0
        x, y = 0.1, 0.0
        traj = np.zeros(256)
        for i in range(256):
            dx = -x + sc * y * math.sin(x)
            dy = -y - sc * x * math.cos(y)
            x += 0.01 * dx
            y += 0.01 * dy
            traj[i] = x
        Smod = _modulate_by_attractor(Sk, traj, sc)
        return _hdr(m, n, k) + struct.pack("<f", sc) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
