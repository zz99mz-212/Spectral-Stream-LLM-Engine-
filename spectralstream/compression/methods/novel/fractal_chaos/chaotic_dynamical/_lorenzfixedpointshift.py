...
class LorenzFixedPointShift:
    """Lorenz: fixed pt"""

    name = "lorenz_fixed_point_shift"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        for i_ in range(256):
            ...
            z += 0.01 * dz
            tz[i_] = x
        Smod = _modulate_by_attractor(Sk, tz, bias)
        return _hdr(m, n, k) + struct.pack("<f", bias) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
