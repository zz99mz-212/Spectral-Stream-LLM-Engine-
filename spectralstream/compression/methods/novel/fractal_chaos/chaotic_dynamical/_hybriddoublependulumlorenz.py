...
class HybridDoublePendulumLorenz:
    """Double Pend+Lorenz"""

    name = "hybrid_double_pendulum_lorenz"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        dpl_h = _extract_chaos_param(tensor) * 5.0
        traj = _double_pendulum(9.81 + dpl_h, 1.0, 1.0, 0.01, 256)
        Smod = _modulate_by_attractor(Sk, traj, dpl_h)
        return _hdr(m, n, k) + struct.pack("<f", dpl_h) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
