...
class HybridDelayDifferential:
    """Delay Lorenz"""

    name = "hybrid_delay_differential"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        dd_h = _extract_chaos_param(tensor) * 5.0
        x, y, z = 1.0, 1.0, 1.0
        hist = [1.0] * 50
        traj = np.zeros(256)
        for i in range(256):
            dx = 10 * (y - x)
            dy = x * (28 - z) - y + dd_h * (hist[i % len(hist)] - x)
            dz = x * y - 8 / 3 * z
            hist[i % len(hist)] = x
            x += 0.01 * dx
            y += 0.01 * dy
            z += 0.01 * dz
            traj[i] = x
        Smod = _modulate_by_attractor(Sk, traj, dd_h)
        return _hdr(m, n, k) + struct.pack("<f", dd_h) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
