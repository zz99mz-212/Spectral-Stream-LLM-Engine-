...
class LorenzRhoEncoding:
    """Lorenz: rho peak-to-peak"""

    name = "lorenz_rho_encoding"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        r = (
            20.0
            + 40.0
            * (float(np.max(f) - np.min(f)) / max(float(np.std(f)) + 1e-30, 1e-30))
            % 1.0
        )
        traj = _lorenz(10.0, r, 8.0 / 3.0, 0.01, 128)
        Smod = _modulate_by_attractor(Sk, traj, r * 0.1)
        return _hdr(m, n, k) + struct.pack("<f", r) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
