...
class LorenzBifurcationNear:
    """Lorenz: near-bifurcation"""

    name = "lorenz_bifurcation_near"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        sp = float(np.sum(np.abs(tensor.ravel()) < 1e-8)) / max(tensor.size, 1)
        r = 24.0 + 10.0 * sp
        traj = _lorenz(10.0, r, 8.0 / 3.0, 0.01, 256)
        Smod = _modulate_by_attractor(Sk, traj)
        return _hdr(m, n, k) + struct.pack("<f", sp) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
