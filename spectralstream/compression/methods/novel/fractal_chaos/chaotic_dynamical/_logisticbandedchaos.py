...
class LogisticBandedChaos:
    """Banded chaos"""

    name = "logistic_banded_chaos"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        rb = 3.5 + 0.5 * _extract_chaos_param(tensor)
        traj = _logistic(rb, 0.5, 256)
        Smod = _modulate_by_attractor(Sk, traj)
        return _hdr(m, n, k) + struct.pack("<f", rb) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
