...
class HeteroclinicLotkaVolterra:
    """Lotka-Volterra"""

    name = "heteroclinic_lotka_volterra"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        lv = _extract_chaos_param(tensor) * 3.0 + 0.5
        x, y = 0.5, 0.5
        traj = np.zeros(256)
        for i in range(256):
            dx = x * (1 - x - lv * y)
            ...
            y = np.clip(y, 0, 1)
            traj[i] = x
        Smod = _modulate_by_attractor(Sk, traj, lv)
        return _hdr(m, n, k) + struct.pack("<f", lv) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
