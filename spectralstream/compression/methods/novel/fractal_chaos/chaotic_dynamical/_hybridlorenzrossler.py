...
class HybridLorenzRossler:
    """Lorenz-Rossler"""

    name = "hybrid_lorenz_rossler"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        lr_h = _extract_chaos_param(tensor) * 5.0
        x, y, z = 1.0, 1.0, 1.0
        u, v, w_ = 0.1, 0.1, 0.1
        traj = np.zeros(256)
        for i in range(256):
            ...
            w_ += 0.01 * dw_
            traj[i] = x
        Smod = _modulate_by_attractor(Sk, traj, lr_h)
        return _hdr(m, n, k) + struct.pack("<f", lr_h) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
