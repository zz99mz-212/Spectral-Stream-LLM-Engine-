...
class DoublePendulumTorque:
    """External torque"""

    name = "double_pendulum_torque"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        torq = _extract_chaos_param(tensor) * 5.0
        traj = _double_pendulum(9.81 + torq, 1.0, 1.0, 0.01, 256)
        Smod = _modulate_by_attractor(Sk, traj)
        return _hdr(m, n, k) + struct.pack("<f", torq) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
