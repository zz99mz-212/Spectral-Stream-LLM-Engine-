...
class DoublePendulumDamping:
    """Damping"""

    name = "double_pendulum_damping"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        ga_dp = _extract_chaos_param(tensor) * 0.5
        traj = _double_pendulum(9.81, 1.0, 1.0, 0.01, 256)
        Smod = _modulate_by_attractor(Sk, traj, ga_dp)
        return _hdr(m, n, k) + struct.pack("<f", ga_dp) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
