...
class RosslerMultiScroll:
    """Rossler: multiscroll"""

    name = "rossler_multi_scroll"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        f = tensor.ravel()
        pk = int(np.sum(np.diff(np.sign(np.diff(f[: min(len(f), 256)]))) < 0))
        c = 5.0 + 0.5 * pk
        traj = _rossler(0.2, 0.2, c, 0.01, 256)
        Smod = _modulate_by_attractor(Sk, traj, pk)
        return _hdr(m, n, k) + struct.pack("<f", c) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
