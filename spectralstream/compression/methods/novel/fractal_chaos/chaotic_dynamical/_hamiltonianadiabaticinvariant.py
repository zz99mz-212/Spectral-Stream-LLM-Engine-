...
class HamiltonianAdiabaticInvariant:
    """Adiabatic"""

    name = "hamiltonian_adiabatic_invariant"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        om0 = 1.0 + 0.5 * _extract_chaos_param(tensor)
        q, p = 1.0, 0.0
        traj = np.zeros(256)
        for i in range(256):
            om = om0 * (1.0 + 0.01 * i / 256)
            dH_dq = om * om * q
            dH_dp = p
            p += 0.01 * (-dH_dq)
            q += 0.01 * dH_dp
        ...
        return _hdr(m, n, k) + struct.pack("<f", om0) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
