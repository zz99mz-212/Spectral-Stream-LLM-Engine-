...
class HenonUnstablePeriodic:
    """Henon: UPO"""

    name = "henon_unstable_periodic"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        a = 1.4 + 0.1 * _extract_chaos_param(tensor)
        traj_a = _henon(a, 0.3, 512)
        cr = int(np.sum(np.abs(np.diff(np.sign(traj_a))) > 0))

        Smod = _modulate_by_attractor(Sk, traj_a, cr)
        return _hdr(m, n, k) + struct.pack("<If", cr, a) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
