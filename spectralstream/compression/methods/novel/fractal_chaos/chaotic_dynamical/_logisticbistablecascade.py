...
class LogisticBistableCascade:
    """Bistable cascade"""

    name = "logistic_bistable_cascade"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        lr1 = 2.8 + 0.5 * _extract_chaos_param(tensor)
        lr2 = 3.5 + 0.3 * _extract_chaos_param(tensor)
        traj = _logistic(lr1 * 0.5 + lr2 * 0.5, 0.5, 256)
        Smod = _modulate_by_attractor(Sk, traj)
        return _hdr(m, n, k) + struct.pack("<ff", lr1, lr2) + _ser(Uk) + _ser(
            Smod
