...
class HenonPeriodicWindow:
    """Henon: period"""

    name = "henon_periodic_window"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        a = 1.3 + 0.2 * _extract_chaos_param(tensor)
        traj_a = _henon(a, 0.3, 256)
        period = int(2 + abs(np.corrcoef(traj_a[:128], traj_a[128:256])[0, 1]) * 10)

        Smod = _modulate_by_attractor(Sk, traj_a, period)
        return _hdr(m, n, k) + struct.pack("<If", period, a) + _ser(Uk) + _ser(
            Smod
