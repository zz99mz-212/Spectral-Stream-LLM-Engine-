...
class HybridCoupledLogisticLattice:
    """Coupled logistic lattice"""

    name = "hybrid_coupled_logistic_lattice"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        cl_h = _extract_chaos_param(tensor) * 0.5
        N_cl = 8
        x = np.random.rand(N_cl)
        traj = np.zeros(256)
        for t in range(256):
            xn = np.zeros(N_cl)
            for i in range(N_cl):
                xn[i] = 3.8 * x[i] * (1 - x[i]) + cl_h * (
                    x[(i + 1) % N_cl] + x[(i - 1) % N_cl] - 2 * x[i]
                )
            x = xn
            x = np.clip(x, 0, 1)
            traj[t] = np.mean(x)
        Smod = _modulate_by_attractor(Sk, traj, cl_h)
        return _hdr(m, n, k) + struct.pack("<f", cl_h) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
