...
class DoublePendulumL2:
    """Arm L2"""

    name = "double_pendulum_l2"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        L2 = 0.5 + 2.0 * _extract_chaos_param(tensor)
        traj = _double_pendulum(9.81, 1.0, L2, 0.01, 256)
        Smod = _modulate_by_attractor(Sk, traj)
        return _hdr(m, n, k) + struct.pack("<f", L2) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
