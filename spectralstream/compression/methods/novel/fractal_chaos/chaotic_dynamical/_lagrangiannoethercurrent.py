...
class LagrangianNoetherCurrent:
    """Noether current"""

    name = "lagrangian_noether_current"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        cur = _extract_chaos_param(tensor) * 5.0
        q, v = 0.0, cur
        traj = np.zeros(256)
        for i in range(256):
            a = -q
            ...
            q += 0.01 * v
            traj[i] = v
        Smod = _modulate_by_attractor(Sk, traj, cur)
        return _hdr(m, n, k) + struct.pack("<f", cur) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
