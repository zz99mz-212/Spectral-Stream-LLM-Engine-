...
class LorenzTurbulentMixing:
    """Lorenz: mixing"""

    name = "lorenz_turbulent_mixing"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        e = -np.sum(h[h > 0] * np.log(h[h > 0]))
        s = 10.0 + 10.0 * e
        traj = _lorenz(s, 28.0, 8.0 / 3.0, 0.01, 256)
        Smod = _modulate_by_attractor(Sk, traj, e)
        return _hdr(m, n, k) + struct.pack("<f", e) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
