...
class KuromotoSpatialCoupling:
    """Spatial coupling"""

    name = "kuromoto_spatial_coupling"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        K_nn = _extract_chaos_param(tensor) * 5.0
        traj = _kuromoto(32, K_nn, 1.0, 128)
        Smod = _modulate_by_attractor(Sk, traj)
        return _hdr(m, n, k) + struct.pack("<f", K_nn) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
