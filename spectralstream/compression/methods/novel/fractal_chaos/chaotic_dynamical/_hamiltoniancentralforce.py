...
class HamiltonianCentralForce:
    """Central force"""

    name = "hamiltonian_central_force"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        r0 = _extract_chaos_param(tensor) * 2.0 + 0.5
        q1, q2, p1, p2 = r0, 0.0, 0.0, 0.5
        traj = np.zeros(256)
        for i in range(256):
            r = math.sqrt(q1 * q1 + q2 * q2 + 1e-30)
            F = -1.0 / (r * r)
            Fx = F * q1 / r
            Fy = F * q2 / r
            p1 += 0.01 * Fx
            ...
            q1 += 0.01 * p1
            q2 += 0.01 * p2
            traj[i] = math.atan2(q2, q1)
        Smod = _modulate_by_attractor(Sk, traj, r0)
        return _hdr(m, n, k) + struct.pack("<f", r0) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
