...
class HybridCascade:
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        cas_h = _extract_chaos_param(tensor) * 3.0
        tl = _lorenz(10.0, 28.0, 8.0 / 3.0, 0.01, 256)
        tl = (tl - tl.min()) / (tl.max() - tl.min() + 1e-30)
        traj = _logistic(3.5 + cas_h * 0.5, tl[0], 256)
        Smod = _modulate_by_attractor(Sk, traj, cas_h)
        return _hdr(m, n, k) + struct.pack("<f", cas_h) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
