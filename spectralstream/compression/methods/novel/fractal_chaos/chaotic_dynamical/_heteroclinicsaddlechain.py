...
class HeteroclinicSaddleChain:
    """Saddle chain"""

    name = "heteroclinic_saddle_chain"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        ga_he = _extract_chaos_param(tensor) * 2.0
        x, y = 0.5, 0.0
        traj = np.zeros(256)
        for i in range(256):
            dx = x * (1 - x * x - y * y) - ga_he * x
            dy = -y * (1 - x * x - y * y) - ga_he * y
            x += 0.01 * dx
            y += 0.01 * dy
            if np.abs(x) + np.abs(y) > 10:
                x, y = 0.5, 0.0
                traj[i] = x
        Smod = _modulate_by_attractor(Sk, traj, ga_he)
        return _hdr(m, n, k) + struct.pack("<f", ga_he) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
