...
class LorenzSigmaEncoding:
    """Lorenz: sigma variance"""

    name = "lorenz_sigma_encoding"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        sigma = 5.0 + 5.0 * _extract_chaos_param(tensor)
        traj = _lorenz(sigma, 28.0, 8.0 / 3.0, 0.01, 128)
        Smod = _modulate_by_attractor(Sk, traj)
        return _hdr(m, n, k) + struct.pack("<f", sigma) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
