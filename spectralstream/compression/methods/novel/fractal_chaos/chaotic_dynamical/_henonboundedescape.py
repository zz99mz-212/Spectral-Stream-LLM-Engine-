...
class HenonBoundedEscape:
    """Henon: escape"""

    name = "henon_bounded_escape"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        a = _extract_chaos_param(tensor) * 0.4 + 1.2
        traj_a = _henon(a, 0.3, 512)
        esc = int(np.sum(np.abs(traj_a) > 5.0))

        Smod = _modulate_by_attractor(Sk, traj_a, esc)
        return _hdr(m, n, k) + struct.pack("<If", a, esc) + _ser(Uk) + _ser(
            Smod
