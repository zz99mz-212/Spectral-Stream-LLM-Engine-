...
class HamiltonianNonHolonomic:
    """Nonholonomic"""

    name = "hamiltonian_non_holonomic"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        rd = 1.0 - float(k) / max(min(m, n), 1)
        q1, q2, p1, p2 = 0.5, 0.0, 0.0, 0.5
        traj = np.zeros(256)
        for i in range(256):
            C_ = q1 + rd * q2
            dC = np.array([1.0, rd])
            F = np.array([-p1 * p2, -p1 * p2])
            lm = -np.sum(F * dC) / max(np.sum(dC * dC), 1e-30)
            p1 += 0.01 * (F[0] + lm * dC[0])
            p2 += 0.01 * (F[1] + lm * dC[1])
            q1 += 0.01 * p1
            q2 += 0.01 * p2
            traj[i] = q1
        Smod = _modulate_by_attractor(Sk, traj, rd)
        return _hdr(m, n, k) + struct.pack("<f", rd) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
