...
class KuromotoNoiseInduced:
    """Noise-induced"""

    name = "kuromoto_noise_induced"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        D = _extract_chaos_param(tensor) * 0.5
        traj = _kuromoto(32, 4.0, 1.0, 128)
        Smod = _modulate_by_attractor(Sk, traj, D)
        return _hdr(m, n, k) + struct.pack("<f", D) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
