...
class HybridChaosControl:
    """Chaos controlled"""

    name = "hybrid_chaos_control"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        ctrl_h = _extract_chaos_param(tensor) * 2.0
        x = 0.5
        traj = np.zeros(256)
        r_ctrl = 3.8
        for i in range(256):
            x = r_ctrl * x * (1 - x) + ctrl_h * (0.5 - x) * math.exp(-0.1 * i)
            traj[i] = x
        Smod = _modulate_by_attractor(Sk, traj, ctrl_h)
        return _hdr(m, n, k) + struct.pack("<f", ctrl_h) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
