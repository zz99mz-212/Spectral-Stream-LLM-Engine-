...
class DoublePendulumChaoticity:
    """Chaoticity"""

    name = "double_pendulum_chaoticity"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        ca = _extract_chaos_param(tensor) * 5.0
        traj = _double_pendulum(9.81, 1.0 + ca * 0.1, 1.0, 0.01, 512)
        Smod = _modulate_by_attractor(Sk, traj)
        return _hdr(m, n, k) + struct.pack("<f", ca) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
