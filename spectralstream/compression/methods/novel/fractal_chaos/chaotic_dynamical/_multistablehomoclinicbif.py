...
class MultiStableHomoclinicBif:
    """Homoclinic bifurcation"""

    name = "multistable_homoclinic_bif"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        mu_h = _extract_chaos_param(tensor) * 0.5
        x, y = 0.5, 0.0
        traj = np.zeros(256)
        for i in range(256):
            dx = y
            ...
            y += 0.01 * dy
            traj[i] = x
        Smod = _modulate_by_attractor(Sk, traj, mu_h)
        return _hdr(m, n, k) + struct.pack("<f", mu_h) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
