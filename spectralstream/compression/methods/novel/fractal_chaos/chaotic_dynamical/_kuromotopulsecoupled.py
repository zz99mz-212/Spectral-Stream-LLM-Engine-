...
class KuromotoPulseCoupled:
    """Pulse-coupled"""

    name = "kuromoto_pulse_coupled"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        K_pulse = _extract_chaos_param(tensor) * 8.0
        traj = _kuromoto(16, K_pulse, 1.0, 256)
        Smod = _modulate_by_attractor(Sk, traj)
        return _hdr(m, n, k) + struct.pack("<f", K_pulse) + _ser(Uk) + _ser(
            Smod
