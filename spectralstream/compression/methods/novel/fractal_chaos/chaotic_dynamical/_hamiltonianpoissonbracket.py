...
class HamiltonianPoissonBracket:
    """Poisson"""

    name = "hamiltonian_poisson_bracket"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        t2 = tensor.astype(np.float64).reshape(tensor.shape[0], -1)
        cm = (
            float(np.linalg.norm(t2 @ t2.T - t2.T @ t2))
            / max(float(np.linalg.norm(t2 @ t2.T)), 1e-30)
            if min(t2.shape) > 1
            else 0.0
        )
        q, p = cm * 0.1, 0.0
        ...
        for i in range(256):
            ...
            q += 0.01 * dH_dp
            traj[i] = q
        Smod = _modulate_by_attractor(Sk, traj, cm)
        return _hdr(m, n, k) + struct.pack("<f", cm) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
