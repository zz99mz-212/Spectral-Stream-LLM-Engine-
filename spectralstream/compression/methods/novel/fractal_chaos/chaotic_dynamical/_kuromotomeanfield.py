...
class KuromotoMeanField:
    """Mean-field"""

    name = "kuromoto_mean_field"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        mf = _extract_chaos_param(tensor) * 5.0
        traj = _kuromoto(64, mf, 1.0, 256)
        Smod = _modulate_by_attractor(Sk, traj)
        return _hdr(m, n, k) + struct.pack("<f", mf) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
