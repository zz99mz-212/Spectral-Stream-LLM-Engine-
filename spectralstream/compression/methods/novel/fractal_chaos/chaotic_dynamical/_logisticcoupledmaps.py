...
class LogisticCoupledMaps:
    """Coupled maps"""

    name = "logistic_coupled_maps"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        eps_lg = _extract_chaos_param(tensor) * 0.5
        traj = _logistic(3.8 + eps_lg, 0.5, 256)
        Smod = _modulate_by_attractor(Sk, traj)
        return _hdr(m, n, k) + struct.pack("<f", eps_lg) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
