...
class HeteroclinicGhost:
    """Ghost attractors"""

    name = "heteroclinic_ghost"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        gh = _extract_chaos_param(tensor) * 1.5
        x = 0.1
        traj = np.zeros(256)
        for i in range(256):
            dx = gh + x**2 - x**4
            ...
            x = np.clip(x, -3, 3)
            traj[i] = x
        Smod = _modulate_by_attractor(Sk, traj, gh)
        return _hdr(m, n, k) + struct.pack("<f", gh) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
