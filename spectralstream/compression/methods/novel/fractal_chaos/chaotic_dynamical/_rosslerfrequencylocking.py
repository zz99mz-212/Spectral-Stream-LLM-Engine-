...
class RosslerFrequencyLocking:
    """Rossler: freq lock"""

    name = "rossler_frequency_locking"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        f = tensor.ravel()
        ff = np.abs(np.fft.rfft(f[: min(len(f), 256)]))
        pr = float(np.max(ff[1:]) / max(ff[0], 1e-30)) if len(ff) > 1 else 1.0
        b = 0.1 + 0.4 * min(pr, 1.0)
        traj = _rossler(0.2, b, 5.7, 0.01, 128)
