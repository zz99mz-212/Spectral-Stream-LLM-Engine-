...
class HamiltonianNoether:
    """Noether"""

    name = "hamiltonian_noether"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        sy = (
            float(
                np.abs(np.corrcoef(f[: min(len(f), 128)], f[-min(len(f), 128) :])[0, 1])
            )
            if len(f) > 1
            else 0.0
        )
        ...
        for i in range(256):
            ...
            q += 0.01 * dH_dp
            traj[i] = q
        Smod = _modulate_by_attractor(Sk, traj, sy)
        return _hdr(m, n, k) + struct.pack("<f", sy) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
