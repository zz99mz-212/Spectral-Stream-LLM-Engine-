...
class DoublePendulumNonlinearRestoring:
    """Nonlinear restoring"""

    name = "double_pendulum_nonlinear_restoring"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        nl_dp = _extract_chaos_param(tensor) * 3.0
        traj = _double_pendulum(9.81, 1.0 + nl_dp * 0.1, 1.0, 0.01, 256)
        Smod = _modulate_by_attractor(Sk, traj)
        return _hdr(m, n, k) + struct.pack("<f", nl_dp) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
