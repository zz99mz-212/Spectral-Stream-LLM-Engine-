...
class HamiltonianActionAngle:
    """Action-angle"""

    name = "hamiltonian_action_angle"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        J = _extract_chaos_param(tensor) * 5.0
        th, pJ = 0.0, J
        traj = np.zeros(256)
        for i in range(256):
            dH_dth = 0.0
            ...
            th += 0.01 * dH_dJ
            traj[i] = th
        Smod = _modulate_by_attractor(Sk, traj, J)
        return _hdr(m, n, k) + struct.pack("<f", J) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
