...
class LogisticGeneralized:
    """Generalized"""

    name = "logistic_generalized"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        al_lg = 1.0 + _extract_chaos_param(tensor) * 3.0
        be_lg = 1.0 + _extract_chaos_param(tensor) * 2.0
        traj = _logistic(3.8 * al_lg / min(al_lg + be_lg, 5.0), 0.5, 256)
        Smod = _modulate_by_attractor(Sk, traj, al_lg)
        return _hdr(m, n, k) + struct.pack("<ff", al_lg, be_lg) + _ser(Uk) + _ser(
            Smod
        ) + _ser(Vk), {"shape": tensor.shape}
