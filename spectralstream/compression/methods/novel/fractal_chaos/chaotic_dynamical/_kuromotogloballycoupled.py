...
class KuromotoGloballyCoupled:
    """Globally coupled"""

    name = "kuromoto_globally_coupled"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        gK = _extract_chaos_param(tensor) * 6.0
        traj = _kuromoto(40, gK, 2.0, 256)
        Smod = _modulate_by_attractor(Sk, traj)
        return _hdr(m, n, k) + struct.pack("<f", gK) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
