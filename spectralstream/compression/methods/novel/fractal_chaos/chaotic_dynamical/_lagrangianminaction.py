...
class LagrangianMinAction:
    """Least action"""

    name = "lagrangian_min_action"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        L0 = _extract_chaos_param(tensor) * 10.0
        q, v = 0.0, 1.0
        traj = np.zeros(256)
        for i in range(256):
            dL_dq = -L0 * q
            ...
            q += 0.01 * v
            traj[i] = q
        Smod = _modulate_by_attractor(Sk, traj, L0)
        return _hdr(m, n, k) + struct.pack("<f", L0) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
