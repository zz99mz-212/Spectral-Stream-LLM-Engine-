...
class HenonGeneralized:
    """Henon: generalized"""

    name = "henon_generalized"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        hl = min(len(f) // 2, 256)
        sy = (
            float(np.linalg.norm(f[:hl] - f[-hl:]))
            / max(float(np.linalg.norm(f[:hl])), 1e-30)
            if len(f) > 2
            else 1.0
        )
        a = 1.0 + 0.6 * (sy % 1.0)
        b = 0.1 + 0.3 * (1.0 - abs(sy - 0.5) * 2)
        traj = _henon(a, b, 256)
        Smod = _modulate_by_attractor(Sk, traj)
        return _hdr(m, n, k) + struct.pack("<ff", a, b) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
