...
class RosslerTransverseStability:
    """Rossler: transverse"""

    name = "rossler_transverse_stability"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        ot = (
            float(
                np.abs(np.dot(f[: min(len(f), 256)], f[256 : min(len(f), 512)]))
                / max(
                    np.linalg.norm(f[: min(len(f), 256)])
                    * np.linalg.norm(f[256 : min(len(f), 512)])
                    + 1e-30,
                    1e-30,
                )
            )
        ...
        b = 0.1 + 0.4 * ot
        traj = _rossler(0.2, b, 5.7, 0.01, 256)
        Smod = _modulate_by_attractor(Sk, traj)
        return _hdr(m, n, k) + struct.pack("<f", b) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
