...
class MultiStableGeneralizedMultistability:
    """Generalized N-stable"""

    name = "multistable_generalized_multistability"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        N_s = max(2, min(8, int(2 + 4 * _extract_chaos_param(tensor))))
        x = 0.0
        traj = np.zeros(256)
        for i in range(256):
            dx = -np.prod([(x - j) for j in range(-N_s // 2, N_s // 2 + 1)])
            x += 0.001 * dx
            x = np.clip(x, -N_s, N_s)
            traj[i] = x
        Smod = _modulate_by_attractor(Sk, traj, float(N_s))
        return _hdr(m, n, k) + struct.pack("<I", N_s) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
