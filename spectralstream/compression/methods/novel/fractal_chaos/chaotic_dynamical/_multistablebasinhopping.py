...
class MultiStableBasinHopping:
    """Basin hopping"""

    name = "multistable_basin_hopping"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        D_m = _extract_chaos_param(tensor) * 0.2
        x = 0.5
        traj = np.zeros(256)
        for i in range(256):
            dx = 4.0 * x * (1 - x) - 0.5 * x
            x += 0.01 * dx + math.sqrt(D_m) * np.random.randn()
            x = np.clip(x, 0, 1)
            traj[i] = x
        Smod = _modulate_by_attractor(Sk, traj, D_m)
        return _hdr(m, n, k) + struct.pack("<f", D_m) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
