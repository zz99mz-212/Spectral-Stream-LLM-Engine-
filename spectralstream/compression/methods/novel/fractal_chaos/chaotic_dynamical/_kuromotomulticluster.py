...
class KuromotoMultiCluster:
    """Multi-cluster"""

    name = "kuromoto_multi_cluster"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        K_mc = _extract_chaos_param(tensor) * 5.0 + 2.0
        traj = _kuromoto(64, K_mc, 0.3, 256)
        Smod = _modulate_by_attractor(Sk, traj)
        return _hdr(m, n, k) + struct.pack("<f", K_mc) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
