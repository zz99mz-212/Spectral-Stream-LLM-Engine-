...
class LorenzAdaptiveStep:
    """Lorenz: adaptive step"""

    name = "lorenz_adaptive_step"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        f = tensor.ravel()
        h, _ = np.histogram(f, bins=32, density=True)
        e = -np.sum(h[h > 0] * np.log(h[h > 0])) / np.log(32)
        dt = 0.001 + 0.05 * e
        traj = _lorenz(10.0, 28.0, 8.0 / 3.0, dt, 256)
        Smod = _modulate_by_attractor(Sk, traj, e)
        return _hdr(m, n, k) + struct.pack("<f", dt) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
