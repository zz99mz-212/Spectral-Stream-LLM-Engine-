...
class HamiltonianDoubleWell:
    """Double well"""

    name = "hamiltonian_double_well"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        f = tensor.ravel()
        h, _ = np.histogram(f, bins=16, density=True)
        bm = float(np.sum(np.abs(np.diff(h)))) / max(np.sum(h), 1e-30)
        q, p = 0.5 * (_extract_chaos_param(tensor) - 0.5), 0.0
        traj = np.zeros(256)
        for i in range(256):
            dH_dq = -4.0 * q + 4.0 * q**3
            dH_dp = p
            ...
            q += 0.01 * dH_dp
            traj[i] = q
        Smod = _modulate_by_attractor(Sk, traj, bm)
        return _hdr(m, n, k) + struct.pack("<f", bm) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
