...
class LagrangianFreeParticle:
    """Free particle"""

    name = "lagrangian_free_particle"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        mass = _extract_chaos_param(tensor) * 5.0
        q, v = 0.0, mass * 0.1
        traj = np.zeros(256)
        for i in range(256):
            q += 0.01 * v
            traj[i] = q
        Smod = _modulate_by_attractor(Sk, traj, mass)
        return _hdr(m, n, k) + struct.pack("<f", mass) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
