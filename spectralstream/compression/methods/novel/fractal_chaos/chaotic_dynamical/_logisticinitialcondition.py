...
class LogisticInitialCondition:
    """Initial x0"""

    name = "logistic_initial_condition"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        x0 = _extract_chaos_param(tensor)
        traj = _logistic(3.8, x0, 256)
        Smod = _modulate_by_attractor(Sk, traj)
        return _hdr(m, n, k) + struct.pack("<f", x0) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
