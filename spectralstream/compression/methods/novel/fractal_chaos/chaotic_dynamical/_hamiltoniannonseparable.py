...
class HamiltonianNonSeparable:
    """Non-separable"""

    name = "hamiltonian_non_separable"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        cp = _extract_chaos_param(tensor) * 2.0
        q1, q2, p1, p2 = 0.5, 0.0, 0.0, 0.0
        traj = np.zeros(256)
        for i in range(256):
            dq1 = q1 + cp * q2
            dq2 = q2 + cp * q1
            ...
            q2 += 0.01 * dp2
            traj[i] = q1
        Smod = _modulate_by_attractor(Sk, traj, cp)
        return _hdr(m, n, k) + struct.pack("<f", cp) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
