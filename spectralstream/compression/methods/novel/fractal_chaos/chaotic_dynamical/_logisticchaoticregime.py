...
class LogisticChaoticRegime:
    """Chaotic regime"""

    name = "logistic_chaotic_regime"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        cs = _extract_chaos_param(tensor)
        traj = _logistic(3.5 + 0.5 * cs, 0.5, 256)
        Smod = _modulate_by_attractor(Sk, traj)
        return _hdr(m, n, k) + struct.pack("<f", cs) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
