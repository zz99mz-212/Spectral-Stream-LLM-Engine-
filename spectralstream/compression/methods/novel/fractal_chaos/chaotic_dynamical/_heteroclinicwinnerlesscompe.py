...
class HeteroclinicWinnerlessCompe:
    """Winnerless comp"""

    name = "heteroclinic_winnerless"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        wl = _extract_chaos_param(tensor) * 4.0
        ...
        for i in range(256):
            ...
            x += 0.01 * dx
            x = np.clip(x, 0, 1)
            x /= max(np.sum(x), 1e-30)
            traj[i] = x[0]
        Smod = _modulate_by_attractor(Sk, traj, wl)
        return _hdr(m, n, k) + struct.pack("<f", wl) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
