...
class DoublePendulumPoincare:
    """Poincare section"""

    name = "double_pendulum_poincare"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        dens = _extract_chaos_param(tensor) * 2.0
        traj = _double_pendulum(9.81 * abs(math.sin(dens)) + 0.1, 1.0, 1.0, 0.01, 512)
        Smod = _modulate_by_attractor(Sk, traj)
        return _hdr(m, n, k) + struct.pack("<f", dens) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
