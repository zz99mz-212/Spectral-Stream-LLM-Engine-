...
class LagrangianMultipleParticles:
    """Multi-particle"""

    name = "lagrangian_multiple_particles"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        for t in range(256):
            for i_ in range(np_):
                a = -Sk[min(i_, len(Sk) - 1)] * qs[i_]
                vs[i_] += 0.01 * a
                qs[i_] += 0.01 * vs[i_]
            traj[t] = np.mean(qs)
        Smod = _modulate_by_attractor(Sk, traj, float(np_))
        return _hdr(m, n, k) + struct.pack("<I", np_) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
