...
class HeteroclinicSequentialSwitch:
    """Sequential switch"""

    name = "heteroclinic_sequential_switch"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        sw = _extract_chaos_param(tensor) * 5.0
        x1, x2 = 0.1, 0.1
        traj = np.zeros(256)
        for i in range(256):
            dx1 = x1 * (sw - x1 - x2) - 0.1 * x1
            dx2 = x2 * (sw - x1 - x2) - 0.1 * x2
            x1 += 0.01 * dx1
            x2 += 0.01 * dx2
            x1 = np.clip(x1, 0, 1)
            x2 = np.clip(x2, 0, 1)
            traj[i] = x1 - x2
        Smod = _modulate_by_attractor(Sk, traj, sw)
        return _hdr(m, n, k) + struct.pack("<f", sw) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
