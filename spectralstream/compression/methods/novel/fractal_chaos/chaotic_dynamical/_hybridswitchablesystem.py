...
class HybridSwitchableSystem:
    """Switchable"""

    name = "hybrid_switchable_system"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        sw_h = _extract_chaos_param(tensor)
        traj = (
            _lorenz(10.0, 28.0, 8.0 / 3.0, 0.01, 128)
            if sw_h > 0.5
            else _logistic(3.8, 0.5, 128)
        )
        Smod = _modulate_by_attractor(Sk, traj, sw_h)
        return _hdr(m, n, k) + struct.pack("<f", sw_h) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
