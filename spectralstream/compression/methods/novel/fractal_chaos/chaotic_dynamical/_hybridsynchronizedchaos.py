...
class HybridSynchronizedChaos:
    """Synchronized Lorenz"""

    name = "hybrid_synchronized_chaos"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        synch_h = _extract_chaos_param(tensor) * 5.0
        x1, y1, z1 = 1.0, 1.0, 1.0
        x2, y2, z2 = 2.0, 2.0, 2.0
        traj = np.zeros(256)
        for i in range(256):
            dx1 = 10 * (y1 - x1)
            dy1 = x1 * (28 - z1) - y1 + synch_h * (x2 - x1)
            dz1 = x1 * y1 - 8 / 3 * z1
            dx2 = 10 * (y2 - x2) + synch_h * (x1 - x2)
            dy2 = x2 * (28 - z2) - y2
            dz2 = x2 * y2 - 8 / 3 * z2
            x1 += 0.01 * dx1
            y1 += 0.01 * dy1
            ...
            z2 += 0.01 * dz2
            traj[i] = x1 - x2
        Smod = _modulate_by_attractor(Sk, traj, synch_h)
        return _hdr(m, n, k) + struct.pack("<f", synch_h) + _ser(Uk) + _ser(
            Smod
