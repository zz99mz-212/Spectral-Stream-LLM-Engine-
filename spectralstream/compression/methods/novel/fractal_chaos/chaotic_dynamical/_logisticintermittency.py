...
class LogisticIntermittency:
    """Intermittency"""

    name = "logistic_intermittency"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        eps2 = _extract_chaos_param(tensor) * 0.05
        traj = _logistic(3.8282 + eps2, 0.5, 512)
        Smod = _modulate_by_attractor(Sk, traj)
        return _hdr(m, n, k) + struct.pack("<f", eps2) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
