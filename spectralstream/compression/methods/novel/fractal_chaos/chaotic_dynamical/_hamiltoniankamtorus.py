...
class HamiltonianKAMTorus:
    """KAM torus"""

    name = "hamiltonian_kam_torus"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        ig = _extract_chaos_param(tensor) * 2.0
        ...
        for i in range(256):
            ...
            for _ in range(2):
                ...
                p2 -= 0.005 * dq2
            traj[i] = q1
        Smod = _modulate_by_attractor(Sk, traj, ig)
        return _hdr(m, n, k) + struct.pack("<f", ig) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
