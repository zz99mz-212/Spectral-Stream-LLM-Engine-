...
class LagrangianConstrained:
    """Constrained"""

    name = "lagrangian_constrained"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        q, v = 0.5, 0.0
        lam = 1.0 + 5.0 * _extract_chaos_param(tensor)
        traj = np.zeros(256)
        for i in range(256):
            F = -q - lam * q**3
            C_ = q
            dC = 1.0
            vf = v + 0.01 * F
            lv = -(vf) * dC / (dC * dC + 1e-30) if abs(C_) > 0.01 else 0.0
            v = vf + lv * dC
            q += 0.01 * v
            v *= 0.99
            traj[i] = q
        Smod = _modulate_by_attractor(Sk, traj, lam)
        return _hdr(m, n, k) + struct.pack("<f", lam) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
