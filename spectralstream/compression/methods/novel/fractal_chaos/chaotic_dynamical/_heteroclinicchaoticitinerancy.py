...
class HeteroclinicChaoticItinerancy:
    """Chaotic itinerancy"""

    name = "heteroclinic_chaotic_itinerancy"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        ci = _extract_chaos_param(tensor) * 5.0
        x1, x2, x3 = 0.5, 0.5, 0.5
        traj = np.zeros(256)
        for i in range(256):
            dx1 = ci * x1 * (1 - x1) - 0.5 * x1 * x2
            dx2 = ci * x2 * (1 - x2) - 0.5 * x2 * x3
            dx3 = ci * x3 * (1 - x3) - 0.5 * x3 * x1
            x1 += 0.01 * dx1
            x2 += 0.01 * dx2
            x3 += 0.01 * dx3
            x1 = np.clip(x1, 0, 1)
            ...
            x3 = np.clip(x3, 0, 1)
            traj[i] = x1
        Smod = _modulate_by_attractor(Sk, traj, ci)
        return _hdr(m, n, k) + struct.pack("<f", ci) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
