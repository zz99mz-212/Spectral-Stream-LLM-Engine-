...
class KuromotoPartialSynchrony:
    """Partial synchrony"""

    name = "kuromoto_partial_synchrony"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        K3 = _extract_chaos_param(tensor) * 4.0 + 1.5
        traj = _kuromoto(48, K3, 0.5, 512)
        Smod = _modulate_by_attractor(Sk, traj)
        return _hdr(m, n, k) + struct.pack("<f", K3) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
