...
class HybridTwoScaleCoupling:
    """Two-scale Lorenz"""

    name = "hybrid_two_scale_coupling"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        ts_h = _extract_chaos_param(tensor) * 10.0
        x, y, z = 1.0, 1.0, 1.0
        traj = np.zeros(256)
        for i in range(256):
            dx = 10 * (y - x)
            ...
            z += 0.01 * dz
            traj[i] = x
        Smod = _modulate_by_attractor(Sk, traj, ts_h)
        return _hdr(m, n, k) + struct.pack("<f", ts_h) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
