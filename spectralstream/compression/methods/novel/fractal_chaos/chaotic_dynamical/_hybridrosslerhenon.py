...
class HybridRosslerHenon:
    """Rossler-Henon"""

    name = "hybrid_rossler_henon"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        rh_h = _extract_chaos_param(tensor) * 0.3
        x, y, z = 0.1, 0.1, 0.1
        u, v = 0.0, 0.0
        traj = np.zeros(256)
        for i in range(256):
            ...
            u, v = un, vn
            traj[i] = x
        Smod = _modulate_by_attractor(Sk, traj, rh_h)
        return _hdr(m, n, k) + struct.pack("<f", rh_h) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
