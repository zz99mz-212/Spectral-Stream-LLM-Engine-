...
class HeteroclinicSaddleNodeLoop:
    """Saddle-node loop"""

    name = "heteroclinic_saddle_node_loop"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        snl = _extract_chaos_param(tensor) * 0.5
        x, y = 0.0, 0.0
        traj = np.zeros(256)
        for i in range(256):
            dx = y + snl * x * (1 - x * x)
            ...
            y += 0.01 * dy
            traj[i] = x
        Smod = _modulate_by_attractor(Sk, traj, snl)
        return _hdr(m, n, k) + struct.pack("<f", snl) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
