...
class DoublePendulumEnergyExchange:
    """Energy exchange"""

    name = "double_pendulum_energy_exchange"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        exc = _extract_chaos_param(tensor) * 3.0
        traj = _double_pendulum(9.81, 1.0, 1.0 + exc * 0.1, 0.01, 256)
        Smod = _modulate_by_attractor(Sk, traj)
        return _hdr(m, n, k) + struct.pack("<f", exc) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
