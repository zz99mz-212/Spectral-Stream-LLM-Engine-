...
class LogisticPeriodDoubling:
    """Period doubling"""

    name = "logistic_period_doubling"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        r2 = 2.8 + 1.2 * _extract_chaos_param(tensor)
        traj = _logistic(r2, 0.5, 512)
        Smod = _modulate_by_attractor(Sk, traj)
        return _hdr(m, n, k) + struct.pack("<f", r2) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
