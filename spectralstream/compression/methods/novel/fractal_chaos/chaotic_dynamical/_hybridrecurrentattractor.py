...
class HybridRecurrentAttractor:
    """Recurrent attractor"""

    name = "hybrid_recurrent_attractor"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        rc_h = _extract_chaos_param(tensor) * 3.0
        x = np.array([0.5] * 4)
        traj = np.zeros(256)
        W_rc = np.random.randn(4, 4) * rc_h
        for i in range(256):
            x = np.tanh(W_rc @ x)
            traj[i] = x[0]
        Smod = _modulate_by_attractor(Sk, traj, rc_h)
        return _hdr(m, n, k) + struct.pack("<f", rc_h) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
