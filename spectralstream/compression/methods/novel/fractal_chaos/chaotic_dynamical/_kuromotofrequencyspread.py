...
class KuromotoFrequencySpread:
    """Frequency spread"""

    name = "kuromoto_frequency_spread"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        omv = _extract_chaos_param(tensor) * 3.0 + 0.1
        traj = _kuromoto(32, 3.0, omv, 256)
        Smod = _modulate_by_attractor(Sk, traj)
        return _hdr(m, n, k) + struct.pack("<f", omv) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
