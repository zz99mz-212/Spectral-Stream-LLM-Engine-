...
class LorenzBetaEncoding:
    """Lorenz: beta kurtosis"""

    name = "lorenz_beta_encoding"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        b = (
            ...
            + 3.0
            * (
                float(np.mean((f - f.mean()) ** 4))
                / max(float(np.mean((f - f.mean()) ** 2)) ** 2 + 1e-30, 1e-30)
            )
            % 1.0
        )
        traj = _lorenz(10.0, 28.0, b, 0.01, 128)
        Smod = _modulate_by_attractor(Sk, traj, b)
