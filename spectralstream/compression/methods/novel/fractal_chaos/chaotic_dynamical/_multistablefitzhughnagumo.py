...
class MultiStableFitzHughNagumo:
    """FitzHugh-Nagumo"""

    name = "multistable_fitzhugh_nagumo"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        I_ext = _extract_chaos_param(tensor) * 2.0
        v, w_ = -0.3, 0.3
        traj = np.zeros(256)
        a_fh = 0.7
        b_fh = 0.8
        c_fh = 3.0
        for i in range(256):
            dv = v * (v - a_fh) * (1 - v) - w_ + I_ext
            dw_ = b_fh * (v - c_fh * w_)
            v += 0.01 * dv
            w_ += 0.01 * dw_
            traj[i] = v
        Smod = _modulate_by_attractor(Sk, traj, I_ext)
        return _hdr(m, n, k) + struct.pack("<f", I_ext) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
