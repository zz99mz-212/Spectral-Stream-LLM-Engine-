...
class LorenzSpectralWarming:
    """Lorenz: warming"""

    name = "lorenz_spectral_warming"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        f = tensor.ravel()
        sk = float(np.mean((f - f.mean()) ** 3)) / max(
            float(np.std(f)) ** 3 + 1e-30, 1e-30
        )
        r = 28.0 + 5.0 * sk
        traj = _lorenz(10.0, r, 8.0 / 3.0, 0.01, 256)
        Smod = _modulate_by_attractor(Sk, traj, sk)
