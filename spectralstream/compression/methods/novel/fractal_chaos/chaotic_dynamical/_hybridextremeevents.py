...
class HybridExtremeEvents:
    """Extreme events"""

    name = "hybrid_extreme_events"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        ee_h = _extract_chaos_param(tensor) * 5.0
        x, y = 0.5, 0.0
        traj = np.zeros(512)
        for i in range(512):
            dx = ee_h * x * (1 - x) - x * y
            ...
            y = np.clip(y, 0, 1)
            traj[i] = x
        Smod = _modulate_by_attractor(Sk, traj, ee_h)
        return _hdr(m, n, k) + struct.pack("<f", ee_h) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
