...
class KuromotoWeightedCoupling:
    """Weighted coupling"""

    name = "kuromoto_weighted_coupling"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        K_eff = _extract_chaos_param(tensor) * 10.0
        traj = _kuromoto(32, K_eff, 1.0, 128)
        Smod = _modulate_by_attractor(Sk, traj)
        return _hdr(m, n, k) + struct.pack("<f", K_eff) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
