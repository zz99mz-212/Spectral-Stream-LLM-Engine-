...
class HybridThreeWayCoupling:
    """Three-way coupling"""

    name = "hybrid_three_way_coupling"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        tw_h = _extract_chaos_param(tensor) * 2.0
        traj = _lorenz(10.0, 28.0, 8.0 / 3.0, 0.01, 256)
        Smod = _modulate_by_attractor(Sk, traj, tw_h)
        return _hdr(m, n, k) + struct.pack("<f", tw_h) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
