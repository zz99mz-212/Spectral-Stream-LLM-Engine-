...
class KuromotoMetabolic:
    """Metabolic"""

    name = "kuromoto_metabolic"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        metab = _extract_chaos_param(tensor) * 5.0
        traj = _kuromoto(24, metab, 1.0, 128)
        Smod = _modulate_by_attractor(Sk, traj, metab)
        return _hdr(m, n, k) + struct.pack("<f", metab) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
