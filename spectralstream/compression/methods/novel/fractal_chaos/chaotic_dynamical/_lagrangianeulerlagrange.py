...
class LagrangianEulerLagrange:
    """Euler-Lagrange"""

    name = "lagrangian_euler_lagrange"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        damp = _extract_chaos_param(tensor) * 2.0
        q, v = 1.0, 0.0
        traj = np.zeros(256)
        for i in range(256):
            dL_dq = -q - damp * v
            ...
            q += 0.01 * v
            traj[i] = q
        Smod = _modulate_by_attractor(Sk, traj, damp)
        return _hdr(m, n, k) + struct.pack("<f", damp) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
