...
class HybridHamiltonianLogistic:
    """Hamiltonian-Logistic"""

    name = "hybrid_hamiltonian_logistic"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        hl_h = _extract_chaos_param(tensor) * 2.0
        q, p = 0.5, 0.0
        x = 0.5
        traj = np.zeros(256)
        for i in range(256):
            ...
            p += 0.01 * (-dH_dq)
            q += 0.01 * dH_dp
            x = 3.8 * x * (1 - x) + hl_h * math.sin(q)
            traj[i] = q
        Smod = _modulate_by_attractor(Sk, traj, hl_h)
        return _hdr(m, n, k) + struct.pack("<f", hl_h) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
