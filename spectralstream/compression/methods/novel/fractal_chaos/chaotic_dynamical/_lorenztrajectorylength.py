...
class LorenzTrajectoryLength:
    """Lorenz: trajectory length"""

    name = "lorenz_trajectory_length"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        tl = max(64, min(1024, int(256 * _extract_chaos_param(tensor) + 64)))
        traj = _lorenz(10.0, 28.0, 8.0 / 3.0, 0.01, tl)
        Smod = _modulate_by_attractor(Sk, traj)
        return _hdr(m, n, k) + struct.pack("<I", tl) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
