...
class HenonAttractorDimension:
    """Henon: attractor dim"""

    name = "henon_attractor_dimension"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        dim = 1.2 + 0.6 * _extract_chaos_param(tensor)
        traj = _henon(1.4, 0.3, 512)
        Smod = _modulate_by_attractor(Sk, traj, dim)
        return _hdr(m, n, k) + struct.pack("<f", dim) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
