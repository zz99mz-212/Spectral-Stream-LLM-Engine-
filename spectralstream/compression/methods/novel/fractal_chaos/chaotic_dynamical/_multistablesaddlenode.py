...
class MultiStableSaddleNode:
    """Saddle-node"""

    name = "multistable_saddle_node"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        rsn = _extract_chaos_param(tensor) * 1.0 - 0.5
        x = 0.1
        traj = np.zeros(256)
        for i in range(256):
            dx = rsn + x**2
            ...
            x = np.clip(x, -5, 5)
            traj[i] = x
        Smod = _modulate_by_attractor(Sk, traj, rsn)
        return _hdr(m, n, k) + struct.pack("<f", rsn) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
