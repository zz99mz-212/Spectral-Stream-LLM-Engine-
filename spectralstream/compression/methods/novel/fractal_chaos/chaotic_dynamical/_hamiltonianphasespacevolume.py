...
class HamiltonianPhaseSpaceVolume:
    """Phase vol"""

    name = "hamiltonian_phase_space_volume"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        dv = float(np.prod(Sk[: min(4, len(Sk))]))
        q, p = 0.0, 0.0
        traj = np.zeros(256)
        for i in range(256):
            dH_dq = q * abs(dv) % 1.0
        ...
        return _hdr(m, n, k) + struct.pack("<f", dv) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
