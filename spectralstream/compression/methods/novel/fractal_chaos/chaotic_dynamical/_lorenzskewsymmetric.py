...
class LorenzSkewSymmetric:
    """Lorenz: skew"""

    name = "lorenz_skew_symmetric"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        sk = (
            float(np.linalg.norm(t2 - t2.T)) / max(float(np.linalg.norm(t2)), 1e-30)
            if min(t2.shape) > 1
            else 0.0
        )
        s = 10.0 * (1.0 + sk)
        r = 28.0 * (1.0 + 0.1 * sk)
        traj = _lorenz(s, r, 8.0 / 3.0, 0.01, 256)
        Smod = _modulate_by_attractor(Sk, traj)
        return _hdr(m, n, k) + struct.pack("<f", sk) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
