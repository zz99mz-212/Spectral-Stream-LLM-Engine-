...
class LorenzNoiseAmplitude:
    """Lorenz: noise amplitude"""

    name = "lorenz_noise_amplitude"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        n = float(np.min(np.abs(tensor.ravel()))) * 0.1
        s = 10.0 + n
        r = 28.0
        b = 8.0 / 3.0
        traj = _lorenz(s, r, b, 0.01, 128)
        Smod = _modulate_by_attractor(Sk, traj, n)
        return _hdr(m, n, k) + struct.pack("<f", n) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
