...
class MultiStableHysteresisLoop:
    """Hysteresis"""

    name = "multistable_hysteresis_loop"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        hyst = _extract_chaos_param(tensor) * 2.0
        x = 0.5
        traj = np.zeros(256)
        for i in range(256):
            drive = 0.5 * math.sin(2 * math.pi * i / 64)
            dx = -(x**3) + x + drive + hyst * np.tanh(10 * x)
            x += 0.01 * dx
            x = np.clip(x, -3, 3)
            traj[i] = x
        Smod = _modulate_by_attractor(Sk, traj, hyst)
        return _hdr(m, n, k) + struct.pack("<f", hyst) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
