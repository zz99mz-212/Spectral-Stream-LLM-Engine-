...
class HamiltonianSymplecticEuler:
    """Symplectic Euler"""

    name = "hamiltonian_symplectic_euler"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        q, p = 0.0, _extract_chaos_param(tensor) * 2.0
        dt_ = 0.01 + 0.05 * _extract_chaos_param(tensor)
        traj = np.zeros(256)
        for i in range(256):
            p -= dt_ * q
            q += dt_ * p
        ...
        return _hdr(m, n, k) + struct.pack("<f", dt_) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
