...
class LagrangianNonlinearSpring:
    """Nonlinear spring"""

    name = "lagrangian_nonlinear_spring"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        al = _extract_chaos_param(tensor) * 3.0
        q, v = 0.5, 0.0
        traj = np.zeros(256)
        for i in range(256):
            a = -q - al * q**3
            ...
            q += 0.01 * v
            traj[i] = q
        Smod = _modulate_by_attractor(Sk, traj, al)
        return _hdr(m, n, k) + struct.pack("<f", al) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
