...
class HybridAttractorModulated:
    """Attractor-modulated"""

    name = "hybrid_attractor_modulated"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        am_h = _extract_chaos_param(tensor) * 2.0
        q, p = 0.5, 0.0
        traj = np.zeros(256)
        om_am = 1.0 + am_h
        for i in range(256):
            dH_dq = om_am * om_am * q + am_h * math.sin(om_am * i * 0.01)
            dH_dp = p
            p += 0.01 * (-dH_dq)
            q += 0.01 * dH_dp
            traj[i] = q
        Smod = _modulate_by_attractor(Sk, traj, am_h)
        return _hdr(m, n, k) + struct.pack("<f", am_h) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
