...
class LorenzButterflyEffect:
    """Lorenz: butterfly"""

    name = "lorenz_butterfly_effect"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        p = _extract_chaos_param(tensor)
        eps = 1e-10 * (1.0 + 100.0 * p)
        t1 = _lorenz(10.0, 28.0, 8.0 / 3.0, 0.01, 128)
        t2 = _lorenz(10.0, 28.0 + eps, 8.0 / 3.0, 0.01, 128)
        diff = t1 - t2
        Smod = _modulate_by_attractor(Sk, diff)
        return _hdr(m, n, k) + struct.pack("<f", eps) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
