...
class LorenzTransientChaos:
    """Lorenz: transient"""

    name = "lorenz_transient_chaos"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        b = 7.0 + p * 5.0
        t2 = _lorenz(10.0, 28.0, b, 0.1, 64)
        d = float(np.mean(np.abs(np.diff(t2))))

        Smod = Sk * (1.0 + 0.1 * np.sin(np.arange(len(Sk)) * d * 0.1))
