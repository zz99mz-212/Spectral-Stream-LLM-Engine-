...
class RosslerFullABC:
    """Rossler: full ABC"""

    name = "rossler_full_abc"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        a, b, c = 0.1 + 0.4 * p[0], 0.1 + 0.4 * p[1], 4.0 + 10.0 * p[2]
        traj = _rossler(a, b, c, 0.01, 512)
        Smod = _modulate_by_attractor(Sk, traj)
        return _hdr(m, n, k) + struct.pack("<fff", a, b, c) + _ser(Uk) + _ser(
            Smod
