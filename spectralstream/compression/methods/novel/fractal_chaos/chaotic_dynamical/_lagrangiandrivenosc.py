...
class LagrangianDrivenOsc:
    """Driven osc"""

    name = "lagrangian_driven_osc"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        F0 = _extract_chaos_param(tensor) * 5.0
        om2 = 0.5 + _extract_chaos_param(tensor) * 2.0
        q, v = 0.0, 0.0
        traj = np.zeros(256)
        for i in range(256):
            a = -q + F0 * math.sin(om2 * i * 0.01)
            v += 0.01 * a
            q += 0.01 * v
            traj[i] = q
        Smod = _modulate_by_attractor(Sk, traj, F0)
        return _hdr(m, n, k) + struct.pack("<ff", F0, om2) + _ser(Uk) + _ser(
            Smod
