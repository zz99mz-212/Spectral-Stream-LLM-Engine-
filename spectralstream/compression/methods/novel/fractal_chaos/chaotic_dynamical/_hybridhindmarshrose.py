...
class HybridHindmarshRose:
    """Hindmarsh-Rose"""

    name = "hybrid_hindmarsh_rose"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        I_hr = _extract_chaos_param(tensor) * 5.0
        x_hr, y_hr, z_hr = -1.0, -10.0, 0.0
        traj = np.zeros(512)
        for i in range(512):
            dx_hr = y_hr + 3 * x_hr * x_hr - x_hr**3 - z_hr + I_hr
            dy_hr = 1 - 5 * x_hr * x_hr - y_hr
            dz_hr = 0.006 * (4 * (x_hr + 1.6) - z_hr)
            x_hr += 0.01 * dx_hr
            y_hr += 0.01 * dy_hr
            z_hr += 0.01 * dz_hr
            traj[i] = x_hr
        Smod = _modulate_by_attractor(Sk, traj, I_hr)
        return _hdr(m, n, k) + struct.pack("<f", I_hr) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
