...
class LorenzLyapunovEstimator:
    """Lorenz: Lyapunov"""

    name = "lorenz_lyapunov_estimator"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        ly = _extract_chaos_param(tensor) * 2.0 - 1.0
        s = 10.0 + 5.0 * max(ly, 0)
        r = 28.0 + 10.0 * ly
        traj = _lorenz(s, r, 8.0 / 3.0, 0.01, 256)
        Smod = _modulate_by_attractor(Sk, traj, ly)
        return _hdr(m, n, k) + struct.pack("<f", ly) + _ser(Uk) + _ser(Smod) + _ser(
            Vk
        ), {"shape": tensor.shape}
