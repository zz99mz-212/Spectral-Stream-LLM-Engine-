...
class JuliaSimple:
    """C1: JULIA-SIMPLE — Simple Julia set z→z²+c parameter encoding."""
    ...
    def compress(self, tensor: np.ndarray, **params) -> Tuple[bytes, dict]:
        ...
        n = len(t)
        c_real = params.get("c_real", float(t.mean()))
        c_imag = params.get("c_imag", 0.1)
        k = params.get("k", max(n // 8, 4))
        if k >= n:
            return _block_int8_fallback(tensor)
        kept, idx, _ = _sparsify_topk(t, k)
        return struct.pack("<IIff", n, k, c_real, c_imag) + _pack_f32(
            kept
        ) + _pack_int32(idx), {"shape": tensor.shape, "n": n, "k": k}

    def decompress(self, data: bytes, metadata: dict) -> np.ndarray:
        n, k = struct.unpack_from("<II", data, 0)
        kept = np.frombuffer(data[16 : 16 + k * 4], dtype=np.float32)
        idx = np.frombuffer(data[16 + k * 4 : 16 + k * 8], dtype=np.int32)
