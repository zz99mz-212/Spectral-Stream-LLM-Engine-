...
class MultiFractalLegendre:
    """I3: MULTIFRACTAL-LEGENDRE — Legendre transform multifractal spectrum encoding."""

    name = "multifractal_legendre"
    ...
    def compress(self, tensor: np.ndarray, **params) -> Tuple[bytes, dict]:
        ...
        tau = np.polyval([0.1, -0.3, 0.5], np.linspace(-1, 1, k))
        legendre = np.convolve(kept, tau[: min(len(tau), k)], mode="same")[:k].astype(
            np.float32
        )
        return struct.pack("<II", n, k) + _pack_f32(legendre) + _pack_f32(
            kept
