...
class InterferStandingWave:
    """E8: INTERFER-STANDING-WAVE — Standing wave interference encoding."""

    name = "interfer_standing_wave"
    ...
    def compress(self, tensor: np.ndarray, **params) -> Tuple[bytes, dict]:
        ...
        wave = kept * np.sin(np.linspace(0, 2 * np.pi, k))
        return struct.pack("<II", n, k) + _pack_f32(wave) + _pack_f32(
            kept
