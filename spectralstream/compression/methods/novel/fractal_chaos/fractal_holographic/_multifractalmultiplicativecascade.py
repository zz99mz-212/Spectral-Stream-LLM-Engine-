...
class MultiFractalMultiplicativeCascade:
    """I5: MULTIFRACTAL-MULTIPLICATIVE-CASCADE — Multiplicative cascade multifractal encoding."""

    name = "multifractal_multiplicative_cascade"
    ...
    def compress(self, tensor: np.ndarray, **params) -> Tuple[bytes, dict]:
        ...
        cascade = kept * (
            1
            + 0.1
            * np.cumprod(1 + 0.01 * np.random.RandomState(42).randn(k)).astype(
                np.float32
            )
        )
        return struct.pack("<II", n, k) + _pack_f32(cascade) + _pack_f32(
            kept
