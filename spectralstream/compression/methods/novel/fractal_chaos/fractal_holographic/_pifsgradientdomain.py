...
class PIFSGradientDomain:
    """G8: PIFS-GRADIENT-DOMAIN — Gradient-domain PIFS encoding."""

    name = "pifs_gradient_domain"
    ...
    def compress(self, tensor: np.ndarray, **params) -> Tuple[bytes, dict]:
        ...
        grad = np.gradient(kept).astype(np.float32)[:k]
        return struct.pack("<II", n, k) + _pack_f32(grad) + _pack_f32(
            kept
