...
class MultiFractalRandomWalk:
    """I7: MULTIFRACTAL-RANDOM-WALK — Random walk-based multifractal encoding."""

    name = "multifractal_random_walk"
    ...
    def compress(self, tensor: np.ndarray, **params) -> Tuple[bytes, dict]:
        ...
        rw = np.cumsum(kept * 0.01).astype(np.float32)[:k]
        return struct.pack("<II", n, k) + _pack_f32(rw) + _pack_f32(kept) + _pack_int32(
            idx
