...
class InterferSpeckle:
    """E18: INTERFER-SPECKLE — Speckle pattern interference encoding."""

    name = "interfer_speckle"
    ...
    def compress(self, tensor: np.ndarray, **params) -> Tuple[bytes, dict]:
        ...
        speckle = kept * (0.5 + 0.5 * rng.rand(k).astype(np.float32))
        return struct.pack("<II", n, k) + _pack_f32(speckle) + _pack_f32(
            kept
