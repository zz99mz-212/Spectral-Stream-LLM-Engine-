...
class MultiFractalMultiAffine:
    """I10: MULTIFRACTAL-MULTI-AFFINE — Multi-affine multifractal encoding."""

    name = "multifractal_multi_affine"
    ...
    def compress(self, tensor: np.ndarray, **params) -> Tuple[bytes, dict]:
        ...
        affine = kept + 0.3 * np.roll(kept, -1) + 0.1 * np.roll(kept, 1)
        return struct.pack("<II", n, k) + _pack_f32(affine) + _pack_f32(
            kept
