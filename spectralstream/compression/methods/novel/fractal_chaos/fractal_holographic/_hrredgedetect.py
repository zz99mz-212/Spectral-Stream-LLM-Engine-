...
class HRREdgeDetect:
    """D23: HRR-EDGE-DETECT — Edge-detected holographic encoding."""

    name = "hrr_edge_detect"
    ...
    def compress(self, tensor: np.ndarray, **params) -> Tuple[bytes, dict]:
        ...
        edges = np.abs(np.diff(kept, prepend=kept[0]))[:k]
        return struct.pack("<II", n, k) + _pack_f32(edges) + _pack_f32(
            kept
