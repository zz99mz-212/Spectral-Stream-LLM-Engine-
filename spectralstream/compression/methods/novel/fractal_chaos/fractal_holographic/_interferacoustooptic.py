...
class InterferAcoustoOptic:
    """E15: INTERFER-ACOUSTO-OPTIC — Acousto-optic interference encoding."""

    name = "interfer_acousto_optic"
    ...
    def compress(self, tensor: np.ndarray, **params) -> Tuple[bytes, dict]:
        ...
        ao = (
            kept
            * np.sin(np.linspace(0, 2 * np.pi, k))
            * np.cos(np.linspace(0, 2 * math.sqrt(2) * np.pi, k))
        )
        return struct.pack("<II", n, k) + _pack_f32(ao) + _pack_f32(kept) + _pack_int32(
            idx
        ), {"shape": tensor.shape, "n": n, "k": k}
