...
class IFSContractive:
    """A24: IFS-CONTRACTIVE — Contractivity factor encoding with convergence guarantee."""

    name = "ifs_contractive"
    ...
    def compress(self, tensor: np.ndarray, **params) -> Tuple[bytes, dict]:
        ...
        contract = np.clip(kept / (np.abs(kept).max() + 1e-10), -0.99, 0.99)
        return struct.pack("<II", n, k) + _pack_f32(contract) + _pack_f32(
            kept
