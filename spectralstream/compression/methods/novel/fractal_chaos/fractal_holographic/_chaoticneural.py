...
class ChaoticNeural:
    """J6: CHAOTIC-NEURAL — Neural activation-modulated chaotic IFS encoding."""
    ...
    def compress(self, tensor: np.ndarray, **params) -> Tuple[bytes, dict]:
        ...
        neural = kept * (1 + 0.1 * np.sin(np.linspace(0, 4 * np.pi, k)))
        return struct.pack("<II", n, k) + _pack_f32(neural) + _pack_f32(
            kept
