...
class PhaseConjPhaseLocking:
    """F15: PHASE-CONJ-PHASE-LOCKING — Phase locking via conjugate feedback encoding."""

    name = "phase_conj_phase_locking"
    ...
    def compress(self, tensor: np.ndarray, **params) -> Tuple[bytes, dict]:
        ...
        lock = kept * np.cos(np.mean(kept) * np.linspace(0, 2 * np.pi, k))
        return struct.pack("<II", n, k) + _pack_f32(lock) + _pack_f32(
            kept
