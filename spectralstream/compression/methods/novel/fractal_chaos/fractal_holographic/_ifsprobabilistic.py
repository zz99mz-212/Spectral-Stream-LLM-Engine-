...
class IFSProbabilistic:
    """A2: IFS-PROBABILISTIC — Probabilistic IFS with weighted transform selection."""

    name = "ifs_probabilistic"
    category = "fractal_holographic"

    def compress(self, tensor: np.ndarray, **params) -> Tuple[bytes, dict]:
        t = tensor.ravel().astype(np.float32)
        n = len(t)
        n_tf = params.get("n_transforms", 6)
        k = n_tf * 3
        if k >= n:
            return _block_int8_fallback(tensor)
        kept, idx, _ = _sparsify_topk(t, k)
        return struct.pack("<II", n, n_tf) + _pack_f32(kept) + _pack_int32(idx), {
            "shape": tensor.shape,
            "n": n,
            "k": k,
        }
    ...
    def decompress(self, data: bytes, metadata: dict) -> np.ndarray:
        n, n_tf = struct.unpack_from("<II", data, 0)
        k = n_tf * 3
