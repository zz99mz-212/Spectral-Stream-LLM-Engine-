...
class InterferBeatPattern:
    """E9: INTERFER-BEAT-PATTERN — Beat frequency interference pattern encoding."""

    name = "interfer_beat_pattern"
    ...
    def compress(self, tensor: np.ndarray, **params) -> Tuple[bytes, dict]:
        ...
        beat = kept * np.cos(np.linspace(0, 4 * np.pi, k))
        return struct.pack("<II", n, k) + _pack_f32(beat) + _pack_f32(
            kept
