...
class InterferFrequencyMod:
    """E13: INTERFER-FREQUENCY-MOD — Frequency modulated interference encoding."""

    name = "interfer_frequency_mod"
    ...
    def compress(self, tensor: np.ndarray, **params) -> Tuple[bytes, dict]:
        ...
        fm = kept * np.cos(
            2 * np.pi * np.linspace(0, 1, k)
            + 0.5 * np.sin(2 * np.pi * np.linspace(0, 8, k))
        )
        return struct.pack("<II", n, k) + _pack_f32(fm) + _pack_f32(kept) + _pack_int32(
            idx
