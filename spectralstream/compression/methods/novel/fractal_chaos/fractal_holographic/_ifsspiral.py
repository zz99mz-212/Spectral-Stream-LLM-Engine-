...
class IFSSpiral:
    """A10: IFS-SPIRAL — Spiral-based IFS encoding with rotation transforms."""
    ...
    def compress(self, tensor: np.ndarray, **params) -> Tuple[bytes, dict]:
        ...
        spiral = kept * np.sin(np.linspace(0, 4 * math.pi, k))
        return struct.pack("<II", n, k) + _pack_f32(spiral) + _pack_f32(
            kept
        ) + _pack_int32(idx), {"shape": tensor.shape, "n": n, "k": k}

    def decompress(self, data: bytes, metadata: dict) -> np.ndarray:
        n, k = struct.unpack_from("<II", data, 0)
        spiral = np.frombuffer(data[8 : 8 + k * 4], dtype=np.float32)
        kept = np.frombuffer(data[8 + k * 4 : 8 + k * 8], dtype=np.float32)
        idx = np.frombuffer(data[8 + k * 8 : 8 + k * 12], dtype=np.int32)
        corr = kept / (np.sin(np.linspace(0, 4 * math.pi, len(spiral))) + 1e-10)
        return _reconstruct_from_topk(corr, idx, n).reshape(metadata["shape"])
