...
class ChaoticPhase:
    """J9: CHAOTIC-PHASE — Phase chaos modulated IFS encoding."""
    ...
    def compress(self, tensor: np.ndarray, **params) -> Tuple[bytes, dict]:
        ...
        phase_mod = kept * np.cos(np.linspace(0, 8 * np.pi, k) + kept[:k])
        return struct.pack("<II", n, k) + _pack_f32(phase_mod) + _pack_f32(
            kept
