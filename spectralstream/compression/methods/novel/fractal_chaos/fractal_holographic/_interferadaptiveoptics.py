...
class InterferAdaptiveOptics:
    """E17: INTERFER-ADAPTIVE-OPTICS — Adaptive optics interference encoding."""

    name = "interfer_adaptive_optics"
    ...
    def compress(self, tensor: np.ndarray, **params) -> Tuple[bytes, dict]:
        ...
        ao_mod = kept + 0.1 * np.random.RandomState(42).randn(k).astype(np.float32)
        return struct.pack("<II", n, k) + _pack_f32(ao_mod) + _pack_f32(
            kept
