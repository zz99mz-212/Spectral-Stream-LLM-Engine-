...
class HRRFourierBind:
    """D6: HRR-FOURIER-BIND — Fourier-domain binding via element-wise multiplication."""

    name = "hrr_fourier_bind"
    ...
    def compress(self, tensor: np.ndarray, **params) -> Tuple[bytes, dict]:
        ...
        spec = np.abs(np.fft.rfft(kept)).astype(np.float32)
        return struct.pack("<II", n, k) + spec.tobytes() + _pack_f32(
            kept
    ...
    def decompress(self, data: bytes, metadata: dict) -> np.ndarray:
        ...
        spec_len = k // 2 + 1
        kept = np.frombuffer(
            data[8 + spec_len * 4 : 8 + spec_len * 4 + k * 4], dtype=np.float32
        )
        idx = np.frombuffer(
            data[8 + spec_len * 4 + k * 4 : 8 + spec_len * 4 + k * 8], dtype=np.int32
        )
