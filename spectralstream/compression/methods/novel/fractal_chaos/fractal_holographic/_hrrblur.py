...
class HRRBlur:
    """D21: HRR-BLUR — Blurred holographic representation for noise-robust encoding."""
    ...
    def compress(self, tensor: np.ndarray, **params) -> Tuple[bytes, dict]:
        ...
        blurred = np.convolve(kept, np.ones(3) / 3, mode="same")[:k]
        return struct.pack("<II", n, k) + _pack_f32(blurred) + _pack_f32(
            kept
