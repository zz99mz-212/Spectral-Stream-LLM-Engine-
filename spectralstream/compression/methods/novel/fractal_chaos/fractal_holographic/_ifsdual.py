...
class IFSDual:
    """A4: IFS-DUAL — Dual IFS encoding with forward/backward transforms."""
    ...
    def compress(self, tensor: np.ndarray, **params) -> Tuple[bytes, dict]:
        ...
        n = len(t)
        k = params.get("k", max(n // 6, 4))
        if k >= n:
            return _block_int8_fallback(tensor)
        kept, idx, _ = _sparsify_topk(t, k)
        dual = (
            np.fft.rfft(t)[: k // 2].astype(np.float32)
            if k > 2
            else np.array([0.0], dtype=np.float32)
        )
        return struct.pack("<II", n, k) + dual.tobytes() + _pack_f32(
            kept
    ...
    def decompress(self, data: bytes, metadata: dict) -> np.ndarray:
        ...
        n_spec = max(k // 2, 1)
        off = 8 + n_spec * 4
        kept = np.frombuffer(data[off : off + k * 4], dtype=np.float32)
        idx = np.frombuffer(data[off + k * 4 : off + k * 8], dtype=np.int32)
