...
class HRRBinarySpatter:
    """D14: HRR-BINARY-SPATTER — Binary spatter code HRR encoding."""

    name = "hrr_binary_spatter"
    ...
    def compress(self, tensor: np.ndarray, **params) -> Tuple[bytes, dict]:
        ...
        bits = (kept > 0).astype(np.uint8)
        return struct.pack("<II", n, k) + _pack_uint8(bits) + _pack_f32(
            kept
        ) + _pack_int32(idx), {"shape": tensor.shape, "n": n, "k": k}

    def decompress(self, data: bytes, metadata: dict) -> np.ndarray:
        n, k = struct.unpack_from("<II", data, 0)
        bits = np.frombuffer(data[8 : 8 + k], dtype=np.uint8).astype(np.float32) * 2 - 1
        kept = np.frombuffer(data[8 + k : 8 + k + k * 4], dtype=np.float32)
        idx = np.frombuffer(data[8 + k + k * 4 : 8 + k + k * 8], dtype=np.int32)
        return _reconstruct_from_topk(kept * bits, idx, n).reshape(metadata["shape"])
