...
class InterferAmplitudeMod:
    """E12: INTERFER-AMPLITUDE-MOD — Amplitude modulated interference encoding."""

    name = "interfer_amplitude_mod"
    ...
    def compress(self, tensor: np.ndarray, **params) -> Tuple[bytes, dict]:
        ...
        am = kept * (1 + 0.5 * np.sin(np.linspace(0, 4 * np.pi, k)))
        return struct.pack("<II", n, k) + _pack_f32(am) + _pack_f32(kept) + _pack_int32(
            idx
