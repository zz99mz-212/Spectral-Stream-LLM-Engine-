...
class IFSAffine:
    """A1: IFS-AFFINE — Store affine transform coefficients for block self-similarity."""
    ...
    def compress(self, tensor: np.ndarray, **params) -> Tuple[bytes, dict]:
        ...
        n = len(t)
        n_ifs = params.get("n_ifs", min(8, n // 16))
        k = n_ifs * 2
        if k >= n:
            return _block_int8_fallback(tensor)
        kept, idx, _ = _sparsify_topk(t, k)
        return struct.pack("<II", n, n_ifs) + _pack_f32(kept) + _pack_int32(idx), {
            "shape": tensor.shape,
            "n": n,
            "k": k,
        }
    ...
    def decompress(self, data: bytes, metadata: dict) -> np.ndarray:
        n, n_ifs = struct.unpack_from("<II", data, 0)
        k = n_ifs * 2
