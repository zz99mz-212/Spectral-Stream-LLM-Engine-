...
class ChaoticTemporal:
    """J7: CHAOTIC-TEMPORAL — Temporal chaos-driven IFS encoding."""

    name = "chaotic_temporal"
    ...
    def compress(self, tensor: np.ndarray, **params) -> Tuple[bytes, dict]:
        ...
        logistic = 4 * np.linspace(0.1, 0.9, k) * (1 - np.linspace(0.1, 0.9, k))
        temporal = kept * logistic.astype(np.float32)
        return struct.pack("<II", n, k) + _pack_f32(temporal) + _pack_f32(
            kept
