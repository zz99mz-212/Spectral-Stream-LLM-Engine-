...
class MultiFractalSingularity:
    """I1: MULTIFRACTAL-SINGULARITY — Singularity spectrum f(α) encoding."""

    name = "multifractal_singularity"
    ...
    def compress(self, tensor: np.ndarray, **params) -> Tuple[bytes, dict]:
        ...
        alpha = np.gradient(np.log(np.abs(kept) + 1e-10))
        return struct.pack("<II", n, k) + _pack_f32(alpha) + _pack_f32(
            kept
