...
class ChaoticNoiseIFS:
    """J2: CHAOTIC-NOISE-IFS — Noise-modulated IFS encoding."""

    name = "chaotic_noise_ifs"
    ...
    def compress(self, tensor: np.ndarray, **params) -> Tuple[bytes, dict]:
        ...
        noisy = kept + 0.05 * rng.randn(k).astype(np.float32)
        return struct.pack("<II", n, k) + _pack_f32(noisy) + _pack_f32(
            kept
