...
class InterferSelfInterfer:
    """E19: INTERFER-SELF-INTERFER — Self-interference encoding."""

    name = "interfer_self_interfer"
    ...
    def compress(self, tensor: np.ndarray, **params) -> Tuple[bytes, dict]:
        ...
        si = kept + 0.3 * np.roll(kept, 1) + 0.1 * np.roll(kept, 2)
        return struct.pack("<II", n, k) + _pack_f32(si) + _pack_f32(kept) + _pack_int32(
            idx
