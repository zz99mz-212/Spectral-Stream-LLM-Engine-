...
class PIFSWeightedMatching:
    """G13: PIFS-WEIGHTED-MATCHING — Weighted domain matching PIFS encoding."""

    name = "pifs_weighted_matching"
    ...
    def compress(self, tensor: np.ndarray, **params) -> Tuple[bytes, dict]:
        ...
        w = np.abs(kept) / (np.abs(kept).sum() + 1e-10)
        return struct.pack("<II", n, k) + _pack_f32(w) + _pack_f32(kept) + _pack_int32(
            idx
