...
class InterferQuantumInterfer:
    """E20: INTERFER-QUANTUM-INTERFER — Quantum interference pattern encoding."""

    name = "interfer_quantum_interfer"
    ...
    def compress(self, tensor: np.ndarray, **params) -> Tuple[bytes, dict]:
        ...
        qi = np.abs(
            np.fft.ifft(np.fft.fft(kept) * np.conj(np.fft.fft(np.roll(kept, 1))))
        )[:k]
        return struct.pack("<II", n, k) + _pack_f32(qi) + _pack_f32(kept) + _pack_int32(
            idx
