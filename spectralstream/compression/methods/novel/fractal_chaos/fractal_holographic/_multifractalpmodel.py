...
class MultiFractalPModel:
    """I6: MULTIFRACTAL-P-MODEL — p-model cascade multifractal encoding."""

    name = "multifractal_p_model"
    ...
    def compress(self, tensor: np.ndarray, **params) -> Tuple[bytes, dict]:
        ...
        p = 0.5 + 0.1 * np.sin(np.linspace(0, 2 * np.pi, k))
        pmodel = kept * p.astype(np.float32)
        return struct.pack("<II", n, k) + _pack_f32(pmodel) + _pack_f32(
            kept
