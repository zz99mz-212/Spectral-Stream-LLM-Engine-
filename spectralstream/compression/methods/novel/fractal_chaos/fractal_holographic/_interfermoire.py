...
class InterferMoire:
    """E10: INTERFER-MOIRE — Moiré pattern interference encoding."""
    ...
    def compress(self, tensor: np.ndarray, **params) -> Tuple[bytes, dict]:
        ...
        moire = (
            kept
            * np.sin(np.linspace(0, 8 * np.pi, k))
            * np.cos(np.linspace(0, 6 * np.pi, k))
        )
        return struct.pack("<II", n, k) + _pack_f32(moire) + _pack_f32(
            kept
