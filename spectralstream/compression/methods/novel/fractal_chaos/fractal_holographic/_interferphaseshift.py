...
class InterferPhaseShift:
    """E11: INTERFER-PHASE-SHIFT — Phase-shifted interference encoding."""

    name = "interfer_phase_shift"
    ...
    def compress(self, tensor: np.ndarray, **params) -> Tuple[bytes, dict]:
        ...
        shift = np.fft.ifft(
            np.fft.fft(kept) * np.exp(1j * np.linspace(0, np.pi, k))
        ).real.astype(np.float32)[:k]
        return struct.pack("<II", n, k) + _pack_f32(shift) + _pack_f32(
            kept
