...
class InterferRadioInterfer:
    """E16: INTERFER-RADIO-INTERFER — Radio interferometry fringe encoding."""

    name = "interfer_radio_interfer"
    ...
    def compress(self, tensor: np.ndarray, **params) -> Tuple[bytes, dict]:
        ...
        fringe = kept * np.cos(2 * np.pi * np.linspace(0, 1, k) * kept.mean())
        return struct.pack("<II", n, k) + _pack_f32(fringe) + _pack_f32(
            kept
