...
class HRRPhaseEncoding:
    """D17: HRR-PHASE-ENCODING — Phase-based HRR encoding."""

    name = "hrr_phase_encoding"
    ...
    def compress(self, tensor: np.ndarray, **params) -> Tuple[bytes, dict]:
        ...
        phases = np.arccos(np.clip(kept / (np.abs(kept).max() + 1e-10), -1, 1))
        return struct.pack("<II", n, k) + _pack_f32(phases) + _pack_f32(
            kept
