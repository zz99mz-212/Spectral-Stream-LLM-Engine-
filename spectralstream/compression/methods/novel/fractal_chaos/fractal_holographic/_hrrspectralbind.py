...
class HRRSpectralBind:
    """D12: HRR-SPECTRAL-BIND — Spectral domain HRR binding."""

    name = "hrr_spectral_bind"
    ...
    def compress(self, tensor: np.ndarray, **params) -> Tuple[bytes, dict]:
        ...
        spectrum = np.abs(np.fft.rfft(kept)).astype(np.float32)
        return struct.pack("<II", n, k) + spectrum.tobytes() + _pack_f32(
            kept
    ...
    def decompress(self, data: bytes, metadata: dict) -> np.ndarray:
        ...
        n_spec = k // 2 + 1
        kept = np.frombuffer(
            data[8 + n_spec * 4 : 8 + n_spec * 4 + k * 4], dtype=np.float32
        )
        idx = np.frombuffer(
            data[8 + n_spec * 4 + k * 4 : 8 + n_spec * 4 + k * 8], dtype=np.int32
        )
