...
class ChaoticParameterModulated:
    """J10: CHAOTIC-PARAMETER-MODULATED — Parameter-modulated chaotic IFS encoding."""

    name = "chaotic_parameter_modulated"
    ...
    def compress(self, tensor: np.ndarray, **params) -> Tuple[bytes, dict]:
        ...
        pm = kept * (0.5 + 0.5 * np.sin(np.linspace(0, 2 * np.pi * 3.7, k)))
        return struct.pack("<II", n, k) + _pack_f32(pm) + _pack_f32(kept) + _pack_int32(
            idx
