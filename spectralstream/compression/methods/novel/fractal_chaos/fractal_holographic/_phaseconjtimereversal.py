...
class PhaseConjTimeReversal:
    """F10: PHASE-CONJ-TIME-REVERSAL — Time reversal via phase conjugation."""

    name = "phase_conj_time_reversal"
    ...
    def compress(self, tensor: np.ndarray, **params) -> Tuple[bytes, dict]:
        ...
        tr = np.conj(np.fft.ifft(np.fft.fft(kept))).real.astype(np.float32)[:k]
        return struct.pack("<II", n, k) + _pack_f32(tr) + _pack_f32(kept) + _pack_int32(
            idx
