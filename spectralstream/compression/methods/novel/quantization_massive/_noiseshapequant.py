...
class NoiseShapeQuant:
    name = "noise_shape_quant"
    ...
    def compress(self, tensor, **params):
        ...
        q0 = np.round(flat * scale)
        err0 = flat - q0 / scale
        noise = np.zeros_like(err0)
        noise[1:] = err0[:-1] * 0.5
        q = np.clip(q0 + np.round(noise * scale), -half, half - 1).astype(np.int8)
        return struct.pack("<If", n, scale) + q.tobytes(), {"n": n}

    def decompress(self, data, metadata):
        n, scale = struct.unpack_from("<If", data, 0)
        q = np.frombuffer(data[8:], dtype=np.int8).astype(np.float32)
        return q / scale
