...
class Bit8_DCTDomain:
    name = "8bit_dct_domain"
    category = "transform_quant"

    def compress(self, tensor, **params):
        flat = tensor.ravel().astype(np.float32)
        n = len(flat)
        d = _dct(np.pad(flat, (0, _next_pow2(n) - n)))
        q, scale, lo, _ = _uniform_quantize(d[:n], 8, True)
        return struct.pack("<IIf", n, _next_pow2(n), scale) + q.astype(
            np.uint8
        ).tobytes(), {"n": n}

    def decompress(self, data, metadata):
        n, padded, scale = struct.unpack_from("<IIf", data, 0)
        q = np.frombuffer(data[12:], dtype=np.uint8).astype(np.float32)
        d = (q - 128) / scale
        return _idct(np.pad(d, (0, padded - n)))[:n]
