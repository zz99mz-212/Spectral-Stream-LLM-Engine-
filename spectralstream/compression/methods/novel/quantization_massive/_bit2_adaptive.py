...
class Bit2_Adaptive:
    ...
    def compress(self, tensor, **params):
        ...
        thresh = 2.0 * std if std > 0 else 1.0
        mask = np.abs(flat) > thresh
        n_high = int(mask.sum())
        high = flat[mask]
        low = flat[~mask]
        high_amax = float(np.max(np.abs(high))) if n_high > 0 else 1.0
        low_amax = float(np.max(np.abs(low))) if n - n_high > 0 else 1.0
        high_q = np.clip(np.round(high * 2.0 / (high_amax + 1e-10)), -2, 1).astype(
            np.int8
        )
        low_q = np.clip(np.round(low * 1.0 / (low_amax + 1e-10)), -1, 0).astype(np.int8)
        mask_bytes = np.packbits(mask.astype(np.uint8)).tobytes()
        data = (
            struct.pack("<IIff", n, n_high, high_amax, low_amax)
            + mask_bytes
            + high_q.tobytes()
            + low_q.tobytes()
        )
    ...
    def decompress(self, data, metadata):
        n, n_high, high_amax, low_amax = struct.unpack_from("<IIff", data, 0)
        pos = 16
        mb_len = (n + 7) // 8
        ...
        pos += mb_len
        high = np.frombuffer(data[pos : pos + n_high], dtype=np.int8).astype(
            np.float32
        ) * (high_amax / 2.0)
        pos += n_high
        low = np.frombuffer(data[pos:], dtype=np.int8).astype(np.float32) * low_amax
        result = np.zeros(n, dtype=np.float32)
        result[mask] = high
        result[~mask] = low[: n - n_high]
        return result
