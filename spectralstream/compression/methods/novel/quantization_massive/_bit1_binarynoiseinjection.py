...
class Bit1_BinaryNoiseInjection:
    name = "1bit_binary_noise"
    ...
    def compress(self, tensor, **params):
        ...
        for i in range(0, n, 8):
            ...
            for j in range(8):
                if i + j < n:
                    byte |= (1 if noisy[i + j] >= 0 else 0) << j
            packed.append(byte)
        return struct.pack("<If", n, amax) + bytes(packed), {"n": n}

    def decompress(self, data, metadata):
        return Bit1_BinarySign().decompress(data, metadata)
