...
class Bit7_NearLossless:
    name = "7bit_near_lossless"
    ...
    def compress(self, tensor, **params):
        ...
        levels = (1 << 7) - 1
        scale = levels / (hi - lo) if hi > lo else 1.0
        q = np.clip(np.round((flat - lo) * scale), 0, levels).astype(np.uint8)
        recon = q.astype(np.float32) / scale + lo
        residual = flat - recon
        r_scale = 127.0 / (float(np.max(np.abs(residual))) + 1e-30)
        r_q = np.clip(np.round(residual * r_scale), -128, 127).astype(np.int8)
        data = struct.pack("<Iff", n, lo, scale) + q.tobytes() + r_q.tobytes()
        return data, {"n": n}

    def decompress(self, data, metadata):
        n, lo, scale = struct.unpack_from("<Iff", data, 0)
        pos = 12
        q = np.frombuffer(data[pos : pos + n], dtype=np.uint8).astype(np.float32)
        pos += n
        r = np.frombuffer(data[pos : pos + n], dtype=np.int8).astype(np.float32)
        return q / scale + lo + r / 127.0
