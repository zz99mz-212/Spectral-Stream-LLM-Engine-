...
class StochasticRoundQuant:
    name = "stochastic_round_quant"
    category = "quantization"

    def compress(self, tensor, bits=8, **params):
        ...
        rounded = np.where(rng.random(n) < frac, floor + 1, floor)
        q = np.clip(rounded, -half, half - 1).astype(np.int16)
        return struct.pack("<If", n, scale) + q.tobytes(), {"n": n}

    def decompress(self, data, metadata):
        n, scale = struct.unpack_from("<If", data, 0)
        q = np.frombuffer(data[8:], dtype=np.int16).astype(np.float32)
        return q / scale
