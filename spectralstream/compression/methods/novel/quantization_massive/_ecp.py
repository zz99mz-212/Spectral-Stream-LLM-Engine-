...
class ECP:
    ...
    def compress(self, tensor, **params):
        ...
        nb = blocks.shape[0]
        result = bytearray()
        err_prev = np.zeros(bs)
        for i in range(nb):
            b = blocks[i] + err_prev
            amax = float(np.max(np.abs(b)))
            scale = 127.0 / amax if amax > 1e-10 else 1.0
            q = np.clip(np.round(b * scale), -128, 127).astype(np.int8)
            recon = q.astype(np.float32) / scale
            err_prev = blocks[i] - recon
            result += struct.pack("<f", 1.0 / scale) + q.tobytes()
    ...
    def decompress(self, data, metadata):
        ...
        idx = 0
        for _ in range(nb):
            inv_s = struct.unpack_from("<f", data, pos)[0]
            pos += 4
            q = np.frombuffer(data[pos : pos + bs], dtype=np.int8).astype(np.float32)
            pos += bs
