...
class Bit6_PerGroup:
    ...
    def compress(self, tensor, group_size=32, **params):
        ...
        q = np.zeros((ng, group_size), dtype=np.uint16)
        for i in range(ng):
            g = groups[i]
            amax = float(np.max(np.abs(g)))
            scales[i] = half / amax if amax > 1e-30 else 1.0
            q[i] = np.clip(np.round(g * scales[i]), -half, half - 1).astype(np.uint16)
        return struct.pack(
            "<III", n, group_size, ng
    ...
    def decompress(self, data, metadata):
        ...
        pos += ng * 4
        q = (
            np.frombuffer(data[pos:], dtype=np.uint16)
            .reshape(ng, gs)
            .astype(np.float32)
        )
        return ((q - 32) / scales[:, None]).ravel()[:n]
