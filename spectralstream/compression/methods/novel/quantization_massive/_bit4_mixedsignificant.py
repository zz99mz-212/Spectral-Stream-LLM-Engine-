...
class Bit4_MixedSignificant:
    name = "4bit_mixed_significant"
    ...
    def compress(self, tensor, **params):
        ...
        n = len(flat)
        amax = float(np.max(np.abs(flat)))
        sig_mask = np.abs(flat) > 0.3 * amax
        sig_vals = flat[sig_mask]
        n_sig = len(sig_vals)
        sig_q = np.clip(np.round(sig_vals / (amax / 7 + 1e-10)), -8, 7).astype(np.int8)
        nonsig_vals = flat[~sig_mask]
        nonsig_q = (
            np.clip(np.round(nonsig_vals / (amax / 3 + 1e-10)), -4, 3).astype(np.int8)
            if len(nonsig_vals) > 0
            else np.array([], dtype=np.int8)
        )
        mask_bytes = np.packbits(sig_mask.astype(np.uint8)).tobytes()
        data = (
            struct.pack("<IIf", n, n_sig, amax)
            + mask_bytes
            + sig_q.tobytes()
            + nonsig_q.tobytes()
        )
    ...
    def decompress(self, data, metadata):
        n, n_sig, amax = struct.unpack_from("<IIf", data, 0)
        pos = 12
        mb_len = (n + 7) // 8
        ...
        pos += mb_len
        sig = np.frombuffer(data[pos : pos + n_sig], dtype=np.int8).astype(
            np.float32
        ) * (amax / 7)
        pos += n_sig
        nonsig = np.frombuffer(data[pos:], dtype=np.int8).astype(np.float32) * (
            amax / 3
        )
        result = np.zeros(n, dtype=np.float32)
        result[mask] = sig
        result[~mask] = nonsig[: n - n_sig]
        return result
