...
class GradientDescentQuant:
    name = "gradient_descent_quant"
    ...
    def compress(self, tensor, bits=8, **params):
        ...
        levels = np.linspace(lo, hi, nl).astype(np.float32)
        lr = 0.01
        for _ in range(10):
            idx = np.argmin(np.abs(flat[:, None] - levels[None, :]), axis=1)
            for j in range(nl):
                m = idx == j
                if m.any():
                    levels[j] += lr * float(np.mean(flat[m] - levels[j]))
        idx = np.argmin(np.abs(flat[:, None] - levels[None, :]), axis=1).astype(
            np.uint16
        )
        return struct.pack("<II", n, nl) + levels.tobytes() + idx.tobytes(), {"n": n}
