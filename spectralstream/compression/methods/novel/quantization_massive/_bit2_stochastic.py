...
class Bit2_Stochastic:
    name = "2bit_stochastic"
    ...
    def compress(self, tensor, **params):
        ...
        for i in range(n):
            j = np.argmin(np.abs(flat[i] - levels))
            if j < 3:
                d = abs(flat[i] - levels[j])
                d2 = abs(flat[i] - levels[j + 1])
                if rng.random() < d2 / max(d + d2, 1e-30):
                    j += 1
            q[i] = j
        data = (
            struct.pack("<II", n, 4) + levels.astype(np.float32).tobytes() + q.tobytes()
        )
        return data, {"n": n}
    ...
    def decompress(self, data, metadata):
        ...
        pos += nl * 4
        q = np.frombuffer(data[pos:], dtype=np.uint8)
        return lv[q][:n]
