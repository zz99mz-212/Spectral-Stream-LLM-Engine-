...
class Bit1_BinaryAdaptive:
    name = "1bit_binary_adaptive"
    ...
    def compress(self, tensor, **params):
        ...
        amax = float(np.max(np.abs(flat)))
        if amax < 1e-10:
            return struct.pack("<If", n, 0.0) + b"\x00" * ((n + 7) // 8), {"n": n}
        ...
        for i in range(0, n, 8):
            ...
            for j in range(8):
                if i + j < n:
                    byte |= (1 if flat[i + j] >= bias else 0) << j
            packed.append(byte)
        return struct.pack("<Iff", n, amax, bias) + bytes(packed), {"n": n}

    def decompress(self, data, metadata):
        n, amax, bias = struct.unpack_from("<Iff", data, 0)
        out = np.zeros(n, dtype=np.float32)
        for i in range(n):
            byte = data[12 + i // 8]
            out[i] = amax + bias if (byte >> (i % 8)) & 1 else -amax + bias
        return out
