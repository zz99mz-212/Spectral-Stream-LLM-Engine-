...
class Bit5_AdaptiveBlock:
    name = "5bit_adaptive_block"
    category = "quantization"

    def compress(self, tensor, block_size=64, **params):
        ...
        for i in range(nb):
            ...
            amax = float(np.max(np.abs(b)))
            if amax < 1e-30:
                result += struct.pack("<Bf", 1, 1.0)
                continue
            bbits = max(2, min(8, int(np.ceil(np.log2(amax * 4 + 1)))))
            half = 1 << (bbits - 1)
            scale = half / amax
            q = np.clip(np.round(b * scale), -half, half - 1).astype(np.int16)
            result += struct.pack("<Bf", bbits, 1.0 / scale) + q.tobytes()
        return struct.pack("<II", n, nb) + bytes(result), {
            "n": n,
    ...
    def decompress(self, data, metadata):
        ...
        pos = 8
        bs = metadata.get("block_size", 64)
        out = np.zeros(n, dtype=np.float32)
        idx = 0
        for _ in range(nb):
            bbits = struct.unpack_from("<B", data, pos)[0]
            pos += 1
            inv_scale = struct.unpack_from("<f", data, pos)[0]
            pos += 4
            q = np.frombuffer(data[pos : pos + bs * 2], dtype=np.int16).astype(
                np.float32
            )
            pos += bs * 2
            vals = q * inv_scale
            for j in range(min(bs, n - idx)):
                out[idx] = vals[j]
                idx += 1
        return out
