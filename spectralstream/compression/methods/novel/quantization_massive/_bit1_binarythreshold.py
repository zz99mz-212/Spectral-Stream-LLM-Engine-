...
class Bit1_BinaryThreshold:
    name = "1bit_binary_threshold"
    ...
    def compress(self, tensor, **params):
        ...
        for i in range(0, n, 8):
            ...
            for j in range(8):
                if i + j < n:
                    byte |= (1 if flat[i + j] >= median else 0) << j
            packed.append(byte)
        amax = float(np.max(np.abs(flat)))
        return struct.pack("<Iff", n, median, amax) + bytes(packed), {"n": n}

    def decompress(self, data, metadata):
        n, median, amax = struct.unpack_from("<Iff", data, 0)
        out = np.zeros(n, dtype=np.float32)
        for i in range(n):
            byte = data[12 + i // 8]
            out[i] = amax if (byte >> (i % 8)) & 1 else median
        return out
