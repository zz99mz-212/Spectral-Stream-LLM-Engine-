...
class Bit2_OutlierAware:
    name = "2bit_outlier_aware"
    ...
    def compress(self, tensor, **params):
        ...
        n = len(flat)
        amax = float(np.max(np.abs(flat)))
        clip_thresh = 0.5 * amax
        clipped = np.clip(flat, -clip_thresh, clip_thresh)
        q, scale, lo, _ = _uniform_quantize(clipped, 2, True)
        mask = (np.abs(flat) > clip_thresh).astype(np.uint8)
        mask_bytes = np.packbits(mask).tobytes()
        data = (
            struct.pack("<Iff", n, clip_thresh, scale)
            + mask_bytes
    ...
    def decompress(self, data, metadata):
        n, clip_thresh, scale = struct.unpack_from("<Iff", data, 0)
        pos = 12
        mb_len = (n + 7) // 8
        ...
        result = (q - 2) / scale
        result[mask] = clip_thresh * np.sign(result[mask]) if result[mask].any() else 0
        return result[:n]
