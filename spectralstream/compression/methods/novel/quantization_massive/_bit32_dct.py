...
class Bit32_DCT:
    ...
    def compress(self, tensor, threshold=0.01, **params):
        ...
        n_keep = int(mask.sum())
        data = (
            struct.pack("<II", n, n_keep)
            + np.packbits(mask.astype(np.uint8)).tobytes()
            + keep.astype(np.complex64).tobytes()
        )
        return data, {"n": n}
    ...
    def decompress(self, data, metadata):
        ...
        d_full = np.zeros(n, dtype=np.complex64)
        d_full[mask] = keep
        return _idct(np.pad(d_full.real[:n], (0, _next_pow2(n) - n)))[:n]
