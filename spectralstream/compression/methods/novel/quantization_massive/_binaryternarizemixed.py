...
class BinaryTernarizeMixed:
    name = "binary_ternary_mixed"
    ...
    def compress(self, tensor, **params):
        ...
        n = len(flat)
        amax = float(np.max(np.abs(flat)))
        bin_mask = np.abs(flat) >= 0.3 * amax
        ter_mask = (~bin_mask) & (np.abs(flat) >= 0.1 * amax)
        n_bin = int(bin_mask.sum())
        n_ter = int(ter_mask.sum())
        bin_bits = np.packbits(bin_mask.astype(np.uint8)).tobytes()
        ter_bits = np.packbits(ter_mask.astype(np.uint8)).tobytes()
        sig = np.sign(flat[bin_mask]).astype(np.int8)
        ter_vals = np.clip(np.round(flat[ter_mask] / (amax / 3 + 1e-10)), -2, 2).astype(
            np.int8
        )
        data = (
            struct.pack("<IIf", n, n_bin, amax)
            + bin_bits
            + ter_bits
            + sig.tobytes()
            + ter_vals.tobytes()
    ...
    def decompress(self, data, metadata):
        n, n_bin, amax = struct.unpack_from("<IIf", data, 0)
        pos = 12
        mb = (n + 7) // 8
        bm = np.unpackbits(np.frombuffer(data[pos : pos + mb], dtype=np.uint8))[
            :n
        ].astype(bool)
        pos += mb
        tm = np.unpackbits(np.frombuffer(data[pos : pos + mb], dtype=np.uint8))[
            :n
        ].astype(bool)
        pos += mb
        sig = np.frombuffer(data[pos : pos + n_bin], dtype=np.int8).astype(np.float32)
        pos += n_bin
        tv = np.frombuffer(data[pos:], dtype=np.int8).astype(np.float32) * (amax / 3)
        result = np.zeros(n, dtype=np.float32)
        result[bm] = sig * amax
        result[tm] = tv[: int(tm.sum())]
        return result
