...
class GPTQLike:
    ...
    def compress(self, tensor, **params):
        ...
        err = np.zeros(n)
        for i in range(n):
            q[i] = np.clip(np.round(flat[i] / (amax / 127.0 + 1e-10)), -128, 127)
            err[i] = flat[i] - q[i].astype(np.float32) * (amax / 127.0)
            if i + 1 < n and abs(flat[i]) > 1e-10:
                correction = (flat[i + 1] / (flat[i] + 1e-10)) * err[i] * 0.5
                flat[i + 1] -= correction
        return struct.pack("<If", n, amax) + q.tobytes(), {"n": n}

    def decompress(self, data, metadata):
        n, amax = struct.unpack_from("<If", data, 0)
