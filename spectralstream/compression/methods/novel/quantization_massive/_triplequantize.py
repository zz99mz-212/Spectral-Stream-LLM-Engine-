...
class TripleQuantize:
    name = "triple_quantize"
    ...
    def compress(self, tensor, **params):
        ...
        s1 = np.where(amax > 1e-8, amax / 127.0, 1.0)
        q1 = np.clip(np.round(blocks / s1[:, None]), -128, 127).astype(np.int8)
        s1_amax = float(np.max(s1))
        s2 = np.where(s1 > 1e-8, s1 / (s1_amax / 127.0), 1.0)
        q2 = np.clip(np.round(s1 / s2), -128, 127).astype(np.int8)
        q3 = np.clip(np.round(s2), 0, 255).astype(np.uint8)
        data = (
            struct.pack("<IIf", n, nb, s1_amax)
            + q3.tobytes()
            + q2.tobytes()
            + q1.tobytes()
        )
    ...
    def decompress(self, data, metadata):
        n, nb, s1_amax = struct.unpack_from("<IIf", data, 0)
        pos = 12
        bs = 128
        q3 = np.frombuffer(data[pos : pos + nb], dtype=np.uint8).astype(np.float32)
        pos += nb
        q2 = np.frombuffer(data[pos : pos + nb], dtype=np.int8).astype(np.float32)
        pos += nb
        s1 = q2 * (s1_amax / 127.0) + q3
        q1 = np.frombuffer(data[pos:], dtype=np.int8).reshape(nb, bs).astype(np.float32)
        return (q1 * s1[:, None]).ravel()[:n]
