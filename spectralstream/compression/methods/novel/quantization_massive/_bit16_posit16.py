...
class Bit16_Posit16:
    ...
    def compress(self, tensor, **params):
        ...
        n = len(flat)
        sign = (flat.view(np.uint32) >> 31).astype(np.uint16)
        abs_v = np.abs(flat)
        log2 = np.floor(np.log2(abs_v + 1e-30)).astype(np.int32)
        regime = np.where(log2 >= 0, log2 + 1, -log2).astype(np.uint16)
        mantissa = ((abs_v / (2.0**log2) - 1.0) * (1 << 14)).astype(np.uint16)
        posit = (sign << 15) | (regime << 10) | mantissa
        return struct.pack("<I", n) + posit.astype(np.uint16).tobytes(), {"n": n}

    def decompress(self, data, metadata):
        n = struct.unpack_from("<I", data, 0)[0]
        p = np.frombuffer(data[4 : 4 + n * 2], dtype=np.uint16)
        sign = (p >> 15).astype(np.float32) * -2 + 1
        regime = (p >> 10) & 0x1F
        mantissa = p & 0x3FF
        return (
            sign
            * (2.0 ** (regime.astype(np.float32) - 1))
            * (1.0 + mantissa.astype(np.float32) / (1 << 10))
        )
