...
class Bit1_BinarySign:
    name = "1bit_binary_sign"
    ...
    def compress(self, tensor, **params):
        ...
        for i in range(0, n, 8):
            ...
            for j in range(8):
                if i + j < n:
                    byte |= (1 if flat[i + j] >= 0 else 0) << j
            packed.append(byte)
        amax = float(np.max(np.abs(flat)))
        return struct.pack("<If", n, amax) + bytes(packed), {"n": n, "amax": amax}
    ...
    def decompress(self, data, metadata):
        ...
        for i in range(n):
            byte = data[8 + i // 8]
            out[i] = amax if (byte >> (i % 8)) & 1 else -amax
        return out
