...
class Bit1_BinaryStochastic:
    name = "1bit_binary_stochastic"
    category = "quantization"

    def compress(self, tensor, **params):
        flat = tensor.ravel().astype(np.float32)
        n = len(flat)
        norm = float(np.max(np.abs(flat)))
        prob = (
            np.clip((flat / norm + 1) / 2, 0, 1)
            if norm > 1e-10
            else np.ones_like(flat) * 0.5
        )
        rng = np.random.default_rng(42)
        bits = (rng.random(n) < prob).astype(np.uint8)
        packed = bytearray()
        for i in range(0, n, 8):
            ...
            for j in range(8):
                if i + j < n:
                    byte |= int(bits[i + j]) << j
            packed.append(byte)
        return struct.pack("<If", n, norm) + bytes(packed), {"n": n}
    ...
    def decompress(self, data, metadata):
        ...
        for i in range(n):
            byte = data[8 + i // 8]
            out[i] = norm if (byte >> (i % 8)) & 1 else -norm
        return out
