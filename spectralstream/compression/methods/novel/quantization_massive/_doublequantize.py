...
class DoubleQuantize:
    ...
    def compress(self, tensor, **params):
        ...
        quantized = np.clip(np.round(blocks / scales[:, None]), -128, 127).astype(
            np.int8
        )
        scale_amax = float(np.max(scales))
        scale_scales = scale_amax / 127.0 if scale_amax > 1e-8 else 1.0
        q_scales = np.clip(np.round(scales / scale_scales), -128, 127).astype(np.int8)
        data = (
            struct.pack("<IIf", n, bs, scale_scales)
            + q_scales.tobytes()
            + quantized.tobytes()
        )
        return data, {"n": n}
    ...
    def decompress(self, data, metadata):
        n, bs, ss = struct.unpack_from("<IIf", data, 0)
        pos = 12
        nb = (n + bs - 1) // bs
        qs = np.frombuffer(data[pos : pos + nb], dtype=np.int8).astype(np.float32)
        pos += nb
        scales = qs * ss
        q = np.frombuffer(data[pos:], dtype=np.int8).reshape(nb, bs).astype(np.float32)
        return (q * scales[:, None]).ravel()[:n]
