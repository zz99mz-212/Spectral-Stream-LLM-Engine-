...
class SparseQuant:
    ...
    def compress(self, tensor, sparsity=0.5, **params):
        flat = tensor.ravel().astype(np.float32)
        n = len(flat)
        mag = np.abs(flat)
        thresh = float(np.percentile(mag, sparsity * 100))
        mask = mag >= thresh
        nz = int(mask.sum())
        flat_masked = flat[mask]
        amax = float(np.max(np.abs(flat_masked))) if nz > 0 else 1.0
        q = np.clip(np.round(flat_masked / (amax / 127.0 + 1e-10)), -128, 127).astype(
            np.int8
        )
        mb = np.packbits(mask.astype(np.uint8)).tobytes()
        return struct.pack("<IIf", n, nz, amax) + mb + q.tobytes(), {"n": n}

    def decompress(self, data, metadata):
        n, nz, amax = struct.unpack_from("<IIf", data, 0)
        pos = 12
        mb_len = (n + 7) // 8
        ...
        pos += mb_len
        q = np.frombuffer(data[pos:], dtype=np.int8).astype(np.float32) * (amax / 127.0)
