...
class Bit16_BFloat16:
    ...
    def compress(self, tensor, **params):
        ...
        n = len(flat)
        high = (flat.view(np.uint32) >> 16).astype(np.uint16)
        return struct.pack("<I", n) + high.tobytes(), {"n": n}

    def decompress(self, data, metadata):
        n = struct.unpack_from("<I", data, 0)[0]
        high = np.frombuffer(data[4:], dtype=np.uint16).astype(np.uint32)
        return (high << 16).view(np.float32).astype(np.float32)[:n]
