...
class MixedPrecision3:
    name = "mixed_precision_3"
    ...
    def compress(self, tensor, **params):
        ...
        for i in range(nb):
            ...
            amax = float(np.max(np.abs(b)))
            bbits = (
                8 if amax > 0.8 else (4 if amax > 0.3 else (3 if amax > 0.05 else 2))
            )
            half = 1 << (bbits - 1)
            scale = half / amax if amax > 1e-10 else 1.0
            q = np.clip(np.round(b * scale), -half, half - 1).astype(np.int16)
            result += struct.pack("<Bf", bbits, scale) + q.tobytes()
        return struct.pack("<II", n, nb) + bytes(result), {"n": n}
