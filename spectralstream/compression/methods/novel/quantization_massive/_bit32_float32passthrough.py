...
class Bit32_Float32Passthrough:
    name = "32bit_float32_passthrough"
    category = "quantization"

    def compress(self, tensor, **params):
        data = tensor.ravel().astype(np.float32).tobytes()
        return struct.pack("<I", tensor.size) + data, {"n": tensor.size}

    def decompress(self, data, metadata):
        n = struct.unpack_from("<I", data, 0)[0]
        return np.frombuffer(data[4:], dtype=np.float32)[:n]
