...
class SmoothQuant:
    ...
    def compress(self, tensor, alpha=0.5, **params):
        flat = tensor.ravel().astype(np.float32)
        n = len(flat)
        if tensor.ndim >= 2:
            amax_per_ch = np.max(
                np.abs(tensor.astype(np.float32).reshape(tensor.shape[0], -1)), axis=1
            )
        else:
            amax_per_ch = np.array([np.max(np.abs(flat))])
        s = amax_per_ch**alpha if np.any(amax_per_ch > 0) else np.ones_like(amax_per_ch)
        s_reshape = s.reshape(-1, *([1] * (tensor.ndim - 1))) if tensor.ndim >= 2 else s
        t_smooth = tensor.astype(np.float32) / (s_reshape + 1e-10)
        q, scale, lo, _ = _uniform_quantize(t_smooth, 8, True)
        return struct.pack("<II", n, len(s)) + s.astype(
            np.float32
        ).tobytes() + q.astype(np.uint8).tobytes(), {"n": n, "scale": scale}

    def decompress(self, data, metadata):
        n, ns = struct.unpack_from("<II", data, 0)
        pos = 8
        s = np.frombuffer(data[pos : pos + ns * 4], dtype=np.float32)
        pos += ns * 4
        q = np.frombuffer(data[pos:], dtype=np.uint8).astype(np.float32)
        return ((q - 128) / metadata["scale"] * s)[:n]
