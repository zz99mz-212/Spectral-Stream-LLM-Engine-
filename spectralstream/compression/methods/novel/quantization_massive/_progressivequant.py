...
class ProgressiveQuant:
    name = "progressive_quant"
    ...
    def compress(self, tensor, **params):
        ...
        data = bytearray()
        residual = flat.copy()
        for bits in levels_list:
            half = 1 << (bits - 1)
            amax = float(np.max(np.abs(residual)))
            if amax < 1e-10:
                continue
            scale = half / amax
            q = np.clip(np.round(residual * scale), -half, half - 1).astype(np.int8)
            recon = q.astype(np.float32) / scale
            residual -= recon
            data += struct.pack("<Bf", bits, scale) + q.tobytes()
    ...
    def decompress(self, data, metadata):
        ...
        pos = 0
        while pos < len(data):
            bits = struct.unpack_from("<B", data, pos)[0]
            pos += 1
            scale = struct.unpack_from("<f", data, pos)[0]
            pos += 4
            q = np.frombuffer(data[pos : pos + n], dtype=np.int8).astype(np.float32)
            pos += n
            result += q / scale
        return result
