...
class Bit4_PerGroup:
    ...
    def compress(self, tensor, group_size=32, **params):
        flat = tensor.ravel().astype(np.float32)
        n = len(flat)
        half = 8
        padded_n = int(math.ceil(n / group_size) * group_size)
        padded = np.zeros(padded_n, dtype=np.float32)
        padded[:n] = flat
        groups = padded.reshape(-1, group_size)
        ng = groups.shape[0]
        scales = np.zeros(ng, dtype=np.float32)
        q = np.zeros((ng, group_size), dtype=np.uint8)
        for i in range(ng):
            amax = float(np.max(np.abs(groups[i])))
            scales[i] = half / amax if amax > 1e-30 else 1.0
            q[i] = np.clip(np.round(groups[i] * scales[i]), -half, half - 1).astype(
                np.uint8
            )
        return struct.pack(
            "<III", n, group_size, ng
        ) + scales.tobytes() + q.tobytes(), {"n": n}

    def decompress(self, data, metadata):
        n, gs, ng = struct.unpack_from("<III", data, 0)
        pos = 12
        scales = np.frombuffer(data[pos : pos + ng * 4], dtype=np.float32)
        pos += ng * 4
        q = np.frombuffer(data[pos:], dtype=np.uint8).reshape(ng, gs).astype(np.float32)
        return ((q - 8) / scales[:, None]).ravel()[:n]
