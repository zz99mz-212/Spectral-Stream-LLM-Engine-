...
class Bit3_SparseTernary:
    name = "3bit_sparse_ternary"
    category = "quantization"

    def compress(self, tensor, **params):
        flat = tensor.ravel().astype(np.float32)
        n = len(flat)
        thresh = float(np.percentile(np.abs(flat), 50))
        q = np.zeros(n, dtype=np.uint8)
        q[flat > thresh] = 1
        q[flat < -thresh] = 2
        amax = float(np.max(np.abs(flat)))
        return struct.pack("<Iff", n, thresh, amax) + q.tobytes(), {"n": n}

    def decompress(self, data, metadata):
        n, thresh, amax = struct.unpack_from("<Iff", data, 0)
        q = np.frombuffer(data[12:], dtype=np.uint8)
