...
class QATSimulated:
    ...
    def compress(self, tensor, **params):
        ...
        noise = rng.normal(0, 0.01 * float(np.std(flat)), n).astype(np.float32)
        noisy = flat + noise
        amax = float(np.max(np.abs(noisy)))
        q = np.clip(np.round(noisy / (amax / 127.0 + 1e-10)), -128, 127).astype(np.int8)
