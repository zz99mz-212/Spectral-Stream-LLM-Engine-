...
class AdaRound:
    ...
    def compress(self, tensor, **params):
        ...
        scaled = (flat - lo) * scale
        floor_v = np.floor(scaled)
        frac = scaled - floor_v
        alpha = np.clip(np.abs(flat) / (np.max(np.abs(flat)) + 1e-10), 0, 1)
        rounded = np.where(frac > alpha, floor_v + 1, floor_v)
        q = np.clip(rounded, 0, 255).astype(np.uint8)
        return struct.pack("<Iff", n, lo, scale) + q.tobytes(), {"n": n}

    def decompress(self, data, metadata):
        n, lo, scale = struct.unpack_from("<Iff", data, 0)
        q = np.frombuffer(data[12:], dtype=np.uint8).astype(np.float32)
        return q / scale + lo
