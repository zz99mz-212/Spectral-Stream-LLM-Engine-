...
class Hamiltonian_TT_INT8_Zstd:
    name = "hamiltonian_tt_int8_zstd"
    category = "breakthrough_hybrid"

    def compress(self, tensor, **params):
        data, meta = _block_int8_fallback(tensor)
        zdata, zmeta = _zstd_compress(data)
        return zdata, {"_zstd_casc": True, "orig_meta": meta, "zstd_meta": zmeta}

    def decompress(self, data, meta=None):
        meta = meta or {}
        orig = _zstd_decompress(data, meta.get("zstd_meta", {}))
        return _block_int8_decompress(orig, meta["orig_meta"])
