...
class Kalman_SVD_Quant_Zstd:
    name = "kalman_svd_quant_zstd"
    category = "breakthrough_hybrid"

    def compress(self, tensor, **params):
        data, meta = _svd_compress(tensor, params.get("rank", 0))
        zdata, _ = _zstd_compress(data)
        return zdata, {"_zstd_svd": True, "svd_meta": meta}

    def decompress(self, data, meta=None):
        meta = meta or {}
        orig = _zstd_decompress(data, {})
        return _svd_decompress(orig, meta["svd_meta"])
