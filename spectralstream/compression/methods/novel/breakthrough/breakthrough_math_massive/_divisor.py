...
class Divisor:
    """Divisor: formal sum of codimension-1 subvarieties of weight"""
    name = "divisor"; category = "breakthrough_math"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        param = float(np.mean(np.abs(t2d.diagonal())) if min(m,n) > 1 else np.mean(np.abs(t2d)))
