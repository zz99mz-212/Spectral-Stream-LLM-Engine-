...
class ExactSequenceX:
    """Exact sequence: kernel = image — precise weight factorization"""
    name = "exact_sequence_x"; category = "breakthrough_math"

    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        m, n, k, Uk, Sk, Vk = _svd_core(tensor, rank)
        param = float(np.sum(Sk[:k//2]) / max(np.sum(Sk), 1e-30))
