...
class CohomologyX:
    """Cohomology: closed modulo exact forms in weight space"""
    name = "cohomology_x"; category = "breakthrough_math"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        param = float(np.mean(np.abs(np.diff(t2d, axis=0)))) / max(np.mean(np.abs(t2d)), 1e-30)
