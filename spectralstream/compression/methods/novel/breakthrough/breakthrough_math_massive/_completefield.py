...
class CompleteField:
    """Complete field: Cauchy sequences converge — spectral completion"""
    name = "complete_field"; category = "breakthrough_math"

    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        m, n, k, Uk, Sk, Vk = _svd_core(tensor, rank)
        param = float(np.sum(Sk[k//2:] if k > 1 else Sk) / max(np.sum(Sk), 1e-30))
