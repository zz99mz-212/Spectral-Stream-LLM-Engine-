...
class TorGroup:
    """Tor group: torsion product — higher derived functor of tensor"""
    name = "tor_group"; category = "breakthrough_math"

    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        m, n, k, Uk, Sk, Vk = _svd_core(tensor, rank)
        param = float(np.sum(Sk[Sk < np.mean(Sk) * 0.1]) / max(np.sum(Sk), 1e-30))
