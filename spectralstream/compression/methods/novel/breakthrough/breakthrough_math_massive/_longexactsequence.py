...
class LongExactSequence:
    """Long exact sequence: connecting homomorphisms in homology"""
    name = "long_exact_sequence"; category = "breakthrough_math"

    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        m, n, k, Uk, Sk, Vk = _svd_core(tensor, rank)
        param = float(np.sum(np.abs(Sk[:-1] - Sk[1:])) / max(np.sum(Sk), 1e-30))
