...
class ZigZagLemma:
    """Zig-zag lemma: long exact sequence from short exact of complexes"""
    name = "zigzag_lemma"; category = "breakthrough_math"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        param = float(np.mean(np.abs(np.diff(t2d[:,:min(n,4)].ravel()))))
