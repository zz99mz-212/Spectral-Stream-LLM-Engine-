...
class LocallySymmetricSpace:
    """Locally symmetric space: curvature parallel transported"""
    name = "locally_symmetric_space"; category = "breakthrough_math"

    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        m, n, k, Uk, Sk, Vk = _svd_core(tensor, rank)
        param = float(np.linalg.norm(Vk @ Vk.T - np.eye(k)))
