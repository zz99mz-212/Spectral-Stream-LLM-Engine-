...
class DivisibleModule:
    """Divisible module: injective Z-module — every element divisible"""
    name = "divisible_module"; category = "breakthrough_math"

    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        m, n, k, Uk, Sk, Vk = _svd_core(tensor, rank)
        param = float(np.mean(Sk) / max(np.min(Sk[Sk > 1e-10]), 1e-30))
