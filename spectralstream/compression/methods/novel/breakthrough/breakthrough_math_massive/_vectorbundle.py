...
class VectorBundle:
    """Vector bundle: family of vector spaces parametrized by weight base"""
    name = "vector_bundle"; category = "breakthrough_math"

    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        m, n, k, Uk, Sk, Vk = _svd_core(tensor, rank)
        param = float(np.mean(np.abs(Uk[:min(m,8),:])) * np.mean(np.abs(Vk[:,:min(n,8)])))
