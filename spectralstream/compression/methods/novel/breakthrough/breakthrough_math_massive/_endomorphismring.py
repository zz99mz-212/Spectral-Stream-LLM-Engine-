...
class EndomorphismRing:
    """Endomorphism ring: linear maps from weight space to itself"""
    name = "endomorphism_ring"; category = "breakthrough_math"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        param = float(np.linalg.norm(t2d[:min(m,n),:min(m,n)] - np.eye(min(m,n))))
