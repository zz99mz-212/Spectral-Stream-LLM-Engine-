...
class SemisimpleRing:
    """Semisimple ring: completely decomposable into simple components"""
    name = "semisimple_ring"; category = "breakthrough_math"

    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        m, n, k, Uk, Sk, Vk = _svd_core(tensor, rank)
        param = float(np.sum(Sk > np.mean(Sk) * 0.1))
