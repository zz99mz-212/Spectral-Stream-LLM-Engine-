...
class MonoidalCategory:
    """Monoidal category: tensor product structure between weight objects"""
    name = "monoidal_category"; category = "breakthrough_math"

    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        m, n, k, Uk, Sk, Vk = _svd_core(tensor, rank)
        param = float(np.mean(np.abs(Uk[:min(m,16)])) * np.mean(np.abs(Vk[:,:min(n,16)])))
