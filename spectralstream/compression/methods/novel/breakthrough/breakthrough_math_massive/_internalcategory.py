...
class InternalCategory:
    """Internal category: category object inside another category"""
    name = "internal_category"; category = "breakthrough_math"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        param = float(np.mean(np.abs(t2d[:min(m,8),:min(n,8)])) * np.mean(Sk))
