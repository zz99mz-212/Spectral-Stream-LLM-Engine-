...
class SingularHomology:
    """Singular homology: continuous simplices in weight space"""
    name = "singular_homology"; category = "breakthrough_math"

    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        m, n, k, Uk, Sk, Vk = _svd_core(tensor, rank)
        param = float(np.mean(np.abs(np.diff(Sk[:max(1,k//2)])))) if k > 1 else 0.0
