...
class QuaternionKahlerManifold:
    """Quaternion-Kähler manifold: holonomy Sp(n)·Sp(1)"""
    name = "quaternion_kahler_manifold"; category = "breakthrough_math"

    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        m, n, k, Uk, Sk, Vk = _svd_core(tensor, rank)
        param = float(np.mean(Sk ** 4) / max(np.mean(Sk) ** 4, 1e-30))
