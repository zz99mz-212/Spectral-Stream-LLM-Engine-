...
class SheafCohomology:
    """Sheaf cohomology: derived functor of global sections"""
    name = "sheaf_cohomology"; category = "breakthrough_math"

    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        m, n, k, Uk, Sk, Vk = _svd_core(tensor, rank)
        param = float(np.sum(Sk[:k//3] if k > 2 else Sk) / max(np.sum(Sk), 1e-30))
