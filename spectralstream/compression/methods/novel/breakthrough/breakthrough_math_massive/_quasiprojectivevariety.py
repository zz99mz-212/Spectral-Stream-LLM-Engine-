...
class QuasiProjectiveVariety:
    """Quasi-projective variety: open subset of projective variety"""
    name = "quasi_projective_variety"; category = "breakthrough_math"

    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        m, n, k, Uk, Sk, Vk = _svd_core(tensor, rank)
        param = float(np.var(Sk) / max(np.mean(Sk) ** 2, 1e-30))
