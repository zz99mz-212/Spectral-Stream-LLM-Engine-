...
class VermaModule:
    """Verma module: universal highest weight module in weight decomposition"""
    name = "verma_module"; category = "breakthrough_math"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        param = float(np.mean(np.abs(np.diff(np.sort(t2d.ravel())))) / max(np.std(t2d), 1e-30))
