...
class ArtinianRing:
    """Artinian ring: descending chain condition on weight ideals"""
    name = "artinian_ring"; category = "breakthrough_math"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        param = float(np.mean(np.abs(t2d[::max(1,m//8),:max(1,n//8)])))
