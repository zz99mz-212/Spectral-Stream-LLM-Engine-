...
class CyclicGroup:
    """Cyclic group C_n: weight has n-fold rotational symmetry in spectral domain"""
    name = "cyclic_group"; category = "breakthrough_math"

    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        m, n, k, Uk, Sk, Vk = _svd_core(tensor, rank)
        param = max(2, int(np.argmax(np.abs(np.fft.rfft(Sk[:min(16, len(Sk))]))) + 1))
