...
class CyclotomicField:
    """Cyclotomic field Q(ζ_n): roots of unity symmetry in weight spectrum"""
    name = "cyclotomic_field"; category = "breakthrough_math"

    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        m, n, k, Uk, Sk, Vk = _svd_core(tensor, rank)
        param = max(2, int(np.angle(np.fft.fft(Sk[:min(16, len(Sk))])[1]) * 16 / math.pi) % 16) if len(Sk) > 1 else 2
