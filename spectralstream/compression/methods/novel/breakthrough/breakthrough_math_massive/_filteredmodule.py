...
class FilteredModule:
    """Filtered module: increasing filtration by singular value threshold"""
    name = "filtered_module"; category = "breakthrough_math"

    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        m, n, k, Uk, Sk, Vk = _svd_core(tensor, rank)
        param = float(np.sum(Sk ** 3) / max(np.sum(Sk) ** 3, 1e-30))
