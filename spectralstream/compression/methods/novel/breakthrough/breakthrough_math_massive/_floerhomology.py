...
class FloerHomology:
    """Floer homology: infinite-dimensional Morse theory on loop space"""
    name = "floer_homology"; category = "breakthrough_math"

    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        m, n, k, Uk, Sk, Vk = _svd_core(tensor, rank)
        param = float(np.sum(np.exp(-np.abs(Sk))) / max(k, 1))
