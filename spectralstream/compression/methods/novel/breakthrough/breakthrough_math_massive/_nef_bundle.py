...
class NEF_Bundle:
    """NEF bundle: numerically effective — non-negative on curves"""
    name = "nef_bundle"; category = "breakthrough_math"

    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        m, n, k, Uk, Sk, Vk = _svd_core(tensor, rank)
        param = float(np.sum(Sk[Sk > 0]) / max(np.sum(np.abs(Sk)), 1e-30))
