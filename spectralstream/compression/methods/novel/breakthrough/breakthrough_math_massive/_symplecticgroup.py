...
class SymplecticGroup:
    """Sp(2n): symplectic structure preserving area in phase space"""
    name = "symplectic_group"; category = "breakthrough_math"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        h = min(m, n//2)
        param = float(np.mean((t2d[:h*2:2,:h]*t2d[:h*2,1:h+1][:,:h])))
