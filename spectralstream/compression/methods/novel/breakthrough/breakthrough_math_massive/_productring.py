...
class ProductRing:
    """Product ring: direct product decomposition of weight structure"""
    name = "product_ring"; category = "breakthrough_math"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        param = float(np.linalg.norm(t2d[:min(m,4)] @ t2d[:,:min(n,4)].T))
