...
class SemisimpleModule:
    """Semisimple module: direct sum of simple modules — full diagonalization"""
    name = "semisimple_module"; category = "breakthrough_math"

    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        m, n, k, Uk, Sk, Vk = _svd_core(tensor, rank)
        param = float(np.sum(Sk > np.mean(Sk) * 0.05) / max(k, 1))
