...
class CompactGroup:
    """Compact group: weight spectrum concentrated in bounded region"""
    name = "compact_group"; category = "breakthrough_math"

    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        m, n, k, Uk, Sk, Vk = _svd_core(tensor, rank)
        param = float(np.max(Sk) / max(np.min(Sk[Sk > 1e-10]), 1e-30))
