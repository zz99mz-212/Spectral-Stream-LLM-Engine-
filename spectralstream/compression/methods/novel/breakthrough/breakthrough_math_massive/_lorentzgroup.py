...
class LorentzGroup:
    """O(1,3): Lorentz symmetry of special relativity in spectral spacetime"""
    name = "lorentz_group"; category = "breakthrough_math"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        param = float(t2d[0,0]**2 - np.sum(t2d[0,1:]**2)) if n > 1 else float(t2d[0,0])
