...
class SpectralSeqHom:
    """Spectral sequence: page-by-page approximation of weight homology"""
    name = "spectral_seq_hom"; category = "breakthrough_math"

    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        m, n, k, Uk, Sk, Vk = _svd_core(tensor, rank)
        param = float(np.sum(np.abs(np.diff(Sk))) / max(np.sum(Sk), 1e-30))
