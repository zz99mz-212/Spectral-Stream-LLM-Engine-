...
class LocalField:
    """Local field: complete valued field — p-adic structure in weight"""
    name = "local_field"; category = "breakthrough_math"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        param = float(np.mean(np.abs(np.diff(t2d[:,:min(n,16)].ravel())))) if n > 1 else 0.0
