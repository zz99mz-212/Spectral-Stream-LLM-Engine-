...
class GeneralLinearGroup:
    """GL(n): full linear group acting on weight rows"""
    name = "general_linear_group"; category = "breakthrough_math"

    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        m, n, k, Uk, Sk, Vk = _svd_core(tensor, rank)
        param = float(np.linalg.matrix_rank(tensor.reshape(tensor.shape[0], -1)))
