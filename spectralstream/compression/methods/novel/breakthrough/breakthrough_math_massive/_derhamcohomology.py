...
class deRhamCohomology:
    """de Rham cohomology: differential forms modulo exact forms"""
    name = "de_rham_cohomology"; category = "breakthrough_math"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        param = float(np.linalg.norm(t2d - t2d.T)) / max(np.linalg.norm(t2d), 1e-30)
