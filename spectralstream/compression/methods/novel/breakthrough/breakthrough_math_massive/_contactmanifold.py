...
class ContactManifold:
    """Contact manifold: maximally non-integrable hyperplane distribution"""
    name = "contact_manifold"; category = "breakthrough_math"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        param = float(np.linalg.det(t2d_sq)) if min(m,n) > 1 else float(t2d[0,0])
