...
class SimpleGroup:
    """Simple group: no non-trivial normal subgroups — irreducible weight structure"""
    name = "simple_group"; category = "breakthrough_math"

    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        m, n, k, Uk, Sk, Vk = _svd_core(tensor, rank)
        param = float(np.std(Sk) / max(np.mean(Sk), 1e-30))
