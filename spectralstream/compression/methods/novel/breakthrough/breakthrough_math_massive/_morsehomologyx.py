...
class MorseHomologyX:
    """Morse homology: gradient flow of Morse function on weight"""
    name = "morse_homology_x"; category = "breakthrough_math"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        param = float(np.sum(np.abs(np.diff(t2d.ravel()))) * np.sqrt(t2d.size))
