...
class TensorProductRing:
    """Tensor product ring: weight as tensor product of smaller rings"""
    name = "tensor_product_ring"; category = "breakthrough_math"

    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        m, n, k, Uk, Sk, Vk = _svd_core(tensor, rank)
        param = float(np.mean(np.abs(np.kron(Uk[:min(m,8),:min(k,4)], Vk[:min(k,4),:min(n,8)]))))
