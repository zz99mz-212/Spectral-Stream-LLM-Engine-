...
class TorsionFreeModule:
    """Torsion-free module: no nonzero torsion — injective weight structure"""
    name = "torsion_free_module"; category = "breakthrough_math"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        param = float(np.sum(np.abs(t2d) > 1e-6) / max(t2d.size, 1))
