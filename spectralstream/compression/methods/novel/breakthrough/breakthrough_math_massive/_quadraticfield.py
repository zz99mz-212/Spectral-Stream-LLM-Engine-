...
class QuadraticField:
    """Quadratic field Q(√d): quadratic irrational structure in weights"""
    name = "quadratic_field"; category = "breakthrough_math"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        param = float(np.mean(np.abs(t2d[:min(m,8),:min(n,8)]) ** 2))
