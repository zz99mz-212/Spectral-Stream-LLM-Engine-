...
class AlternatingGroup:
    """Alternating group A_n: even permutation structure in spectral flow"""
    name = "alternating_group"; category = "breakthrough_math"

    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        m, n, k, Uk, Sk, Vk = _svd_core(tensor, rank)
        param = int(np.sum(np.abs(np.diff(Sk))) % 2)
