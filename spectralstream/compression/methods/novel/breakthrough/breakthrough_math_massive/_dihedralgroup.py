...
class DihedralGroup:
    """Dihedral group D_n: dihedral symmetry from reflection + rotation"""
    name = "dihedral_group"; category = "breakthrough_math"

    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        m, n, k, Uk, Sk, Vk = _svd_core(tensor, rank)
        t2d = tensor.astype(np.float64).reshape(tensor.shape[0], -1)
        param = int(np.mean(np.abs(t2d - t2d[::-1])) > np.std(t2d) * 0.5)
        buf = _build_header(m, n, k) + struct.pack("<B", param) + _serialize(Uk) + _serialize(Sk) + _serialize(Vk)
