...
class ChainComplex:
    """Chain complex: sequence of weight spaces with boundary operators"""
    name = "chain_complex"; category = "breakthrough_math"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        param = float(np.linalg.norm(t2d @ t2d)) / max(np.linalg.norm(t2d), 1e-30)
