...
class FiniteField:
    """Finite field F_q: discrete weight structure with prime-power order"""
    name = "finite_field"; category = "breakthrough_math"

    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        m, n, k, Uk, Sk, Vk = _svd_core(tensor, rank)
        param = max(2, int(np.exp(np.mean(np.log(np.abs(Sk[Sk > 1e-10]) + 1e-30)))))
