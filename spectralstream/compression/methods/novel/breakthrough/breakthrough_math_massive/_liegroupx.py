...
class LieGroupX:
    """Lie group: continuous symmetry group of the weight manifold"""
    name = "lie_group_x"; category = "breakthrough_math"

    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        m, n, k, Uk, Sk, Vk = _svd_core(tensor, rank)
        param = float(np.mean(np.log(np.abs(Sk[:max(1,k//2)]) + 1e-30)))
