...
class Profunctor:
    """Profunctor: bimodule between categories — weight as relation"""
    name = "profunctor"; category = "breakthrough_math"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        param = float(np.mean(np.abs(t2d)) / max(np.std(t2d), 1e-30))
