...
class VeryAmpleBundle:
    """Very ample bundle: embedding into projective space"""
    name = "very_ample_bundle"; category = "breakthrough_math"

    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        m, n, k, Uk, Sk, Vk = _svd_core(tensor, rank)
        param = float(np.sum(Sk ** 2) / max(np.sum(Sk), 1e-30))
