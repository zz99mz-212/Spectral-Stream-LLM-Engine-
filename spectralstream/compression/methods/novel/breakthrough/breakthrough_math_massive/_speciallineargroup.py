...
class SpecialLinearGroup:
    """SL(n): volume-preserving linear transformations in weight geometry"""
    name = "special_linear_group"; category = "breakthrough_math"

    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        m, n, k, Uk, Sk, Vk = _svd_core(tensor, rank)
        param = float(np.prod(Sk[:min(k, len(Sk))] + 1e-30))
