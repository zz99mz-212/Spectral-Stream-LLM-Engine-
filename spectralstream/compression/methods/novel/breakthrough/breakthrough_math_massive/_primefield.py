...
class PrimeField:
    """Prime field F_p: minimal subfield — prime characteristic of weights"""
    name = "prime_field"; category = "breakthrough_math"

    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        m, n, k, Uk, Sk, Vk = _svd_core(tensor, rank)
        param = max(2, int(np.median(Sk[:min(8,k)]) * 10) % 97 + 2)
