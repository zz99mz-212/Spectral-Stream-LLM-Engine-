...
class EquivariantCohomology:
    """Equivariant cohomology: group-equivariant differential forms"""
    name = "equivariant_cohomology"; category = "breakthrough_math"

    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        m, n, k, Uk, Sk, Vk = _svd_core(tensor, rank)
        param = float(np.mean(np.abs(Sk)) * np.std(Sk) / max(np.mean(Sk), 1e-30))
