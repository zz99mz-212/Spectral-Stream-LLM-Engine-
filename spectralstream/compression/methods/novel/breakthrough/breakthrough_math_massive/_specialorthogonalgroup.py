...
class SpecialOrthogonalGroup:
    """SO(n): rotation-only subgroup preserving orientation"""
    name = "special_orthogonal_group"; category = "breakthrough_math"

    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        m, n, k, Uk, Sk, Vk = _svd_core(tensor, rank)
        sq = min(k, m, n)
        param = float(np.linalg.det(Uk[:,:sq].T @ Uk[:,:sq])) if sq > 0 else 1.0
