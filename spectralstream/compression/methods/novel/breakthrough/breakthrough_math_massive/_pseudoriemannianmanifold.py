...
class PseudoRiemannianManifold:
    """Pseudo-Riemannian: indefinite metric — time-like and space-like directions"""
    name = "pseudo_riemannian_manifold"; category = "breakthrough_math"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        param = float(np.sum(np.linalg.eigvalsh(t2d.T @ t2d) > 0))
