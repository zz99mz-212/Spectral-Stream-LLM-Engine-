...
class NumberField:
    """Number field: algebraic extension over rationals in weight"""
    name = "number_field"; category = "breakthrough_math"

    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        m, n, k, Uk, Sk, Vk = _svd_core(tensor, rank)
        param = float(np.mean(np.abs(Sk)) / max(np.std(Sk), 1e-30))
