...
class ConstantCurvatureManifold:
    """Constant curvature: sphere, hyperbolic, or Euclidean geometry"""
    name = "constant_curvature_manifold"; category = "breakthrough_math"

    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        m, n, k, Uk, Sk, Vk = _svd_core(tensor, rank)
        param = float(np.var(Sk) / max(np.mean(Sk), 1e-30))
