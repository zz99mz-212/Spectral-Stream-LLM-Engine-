...
class EinsteinManifold:
    """Einstein manifold: Ricci tensor proportional to metric"""
    name = "einstein_manifold"; category = "breakthrough_math"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        param = float(np.linalg.norm((t2d @ t2d.T) - np.trace(t2d @ t2d.T)/max(m,1)*np.eye(m)))
