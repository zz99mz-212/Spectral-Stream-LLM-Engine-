...
class ProjectiveModule:
    """Projective module: direct summand of free module — projection onto subspace"""
    name = "projective_module"; category = "breakthrough_math"

    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        m, n, k, Uk, Sk, Vk = _svd_core(tensor, rank)
        param = float(np.trace(Uk.T @ Uk) / max(k, 1))
