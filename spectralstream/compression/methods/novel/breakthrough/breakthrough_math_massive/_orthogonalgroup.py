...
class OrthogonalGroup:
    """Orthogonal group O(n): preserves inner product structure in weight space"""
    name = "orthogonal_group"; category = "breakthrough_math"

    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        m, n, k, Uk, Sk, Vk = _svd_core(tensor, rank)
        param = float(np.linalg.norm(Uk.T @ Uk - np.eye(k)))
