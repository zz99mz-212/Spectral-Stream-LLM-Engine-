...
class HarishChandraModule:
    """Harish-Chandra module: (g,K)-module with infinitesimal character"""
    name = "harish_chandra_module"; category = "breakthrough_math"

    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        m, n, k, Uk, Sk, Vk = _svd_core(tensor, rank)
        param = float(np.sum(np.log(Sk[:max(1,k//2)]+1e-30))/max(np.sum(np.log(Sk+1e-30)),1e-30))
