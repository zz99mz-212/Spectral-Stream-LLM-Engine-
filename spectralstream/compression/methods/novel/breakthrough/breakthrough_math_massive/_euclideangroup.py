...
class EuclideanGroup:
    """E(n): rigid motions — rotations + translations preserving distances"""
    name = "euclidean_group"; category = "breakthrough_math"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        param = float(np.linalg.norm(np.diff(t2d, axis=0))) / max(m, 1)
