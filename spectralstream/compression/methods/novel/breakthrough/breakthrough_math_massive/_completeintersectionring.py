...
class CompleteIntersectionRing:
    """Complete intersection ring: weight defined by minimal relations"""
    name = "complete_intersection_ring"; category = "breakthrough_math"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        param = float(np.mean(np.abs(t2d)) * np.sqrt(m*n) / max(np.std(t2d), 1e-30))
