...
class ShortExactSequence:
    """Short exact sequence: 0→A→B→C→0 — minimal exactness pattern"""
    name = "short_exact_sequence"; category = "breakthrough_math"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        param = float(np.linalg.norm(t2d @ t2d.T @ t2d - t2d @ t2d.T @ t2d)) / max(np.linalg.norm(t2d), 1e-30)
