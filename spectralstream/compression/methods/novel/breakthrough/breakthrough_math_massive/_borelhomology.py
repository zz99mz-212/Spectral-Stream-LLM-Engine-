...
class BorelHomology:
    """Borel homology: equivariant homology with group action"""
    name = "borel_homology"; category = "breakthrough_math"

    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        m, n, k, Uk, Sk, Vk = _svd_core(tensor, rank)
        param = float(np.linalg.norm(Uk @ Vk[:,:min(n,k)] - np.eye(min(m,k,n)), 'fro'))
