...
class RationalField:
    """Rational field Q: weight entries as ratios of integers"""
    name = "rational_field"; category = "breakthrough_math"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        param = float(np.std(t2d)) / max(np.mean(np.abs(t2d)), 1e-30)
