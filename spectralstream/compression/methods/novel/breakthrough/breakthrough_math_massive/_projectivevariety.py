...
class ProjectiveVariety:
    """Projective variety: homogeneous polynomial equations in P^n"""
    name = "projective_variety"; category = "breakthrough_math"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        param = float(np.mean(np.abs(t2d[:,:min(n,8)])) / max(np.mean(np.abs(t2d[:,-min(n,8):])), 1e-30))
