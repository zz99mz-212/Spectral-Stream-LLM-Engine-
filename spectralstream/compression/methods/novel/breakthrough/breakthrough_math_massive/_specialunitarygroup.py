...
class SpecialUnitaryGroup:
    """SU(n): special unitary transformations in complex weight space"""
    name = "special_unitary_group"; category = "breakthrough_math"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        param = float(np.mean(np.abs(np.fft.fft(t2d[:min(m,64)], axis=1)[:,:min(n,32)])))
