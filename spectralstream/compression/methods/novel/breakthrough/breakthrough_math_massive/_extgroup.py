...
class ExtGroup:
    """Ext group: extension classes between weight modules"""
    name = "ext_group"; category = "breakthrough_math"

    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        m, n, k, Uk, Sk, Vk = _svd_core(tensor, rank)
        param = float(np.sum(((Uk @ Vk[:,:min(n,k)])[:min(m,k),:min(k,n)] - np.eye(min(k,m,n))) ** 2))
