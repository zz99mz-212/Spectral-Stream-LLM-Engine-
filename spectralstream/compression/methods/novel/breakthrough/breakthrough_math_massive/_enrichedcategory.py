...
class EnrichedCategory:
    """Enriched category: hom-sets in another category — weight-valued morphisms"""
    name = "enriched_category"; category = "breakthrough_math"

    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        m, n, k, Uk, Sk, Vk = _svd_core(tensor, rank)
        param = float(np.sum(Sk) / max(np.sqrt(m * n), 1))
