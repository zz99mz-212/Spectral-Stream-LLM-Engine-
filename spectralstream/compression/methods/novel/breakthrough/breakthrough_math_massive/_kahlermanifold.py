...
class KahlerManifold:
    """Kähler manifold: Riemannian + symplectic + complex compatible"""
    name = "kahler_manifold"; category = "breakthrough_math"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        t2d_sq = t2d[:min(m,n),:min(m,n)]
        param = float(np.mean(np.abs(t2d_sq - t2d_sq.T)) + np.linalg.norm(t2d_sq - t2d_sq.T, 'fro'))
