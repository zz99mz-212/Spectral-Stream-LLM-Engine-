...
class FormalPowerSeries:
    """Formal power series: weight as infinite series truncated to finite rank"""
    name = "formal_power_series"; category = "breakthrough_math"

    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        m, n, k, Uk, Sk, Vk = _svd_core(tensor, rank)
        param = float(np.mean(np.abs(Sk[:-1] - Sk[1:]))) if k > 1 else 0.0
