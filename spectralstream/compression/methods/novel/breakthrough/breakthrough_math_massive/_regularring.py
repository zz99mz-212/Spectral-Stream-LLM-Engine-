...
class RegularRing:
    """Regular ring: finite global dimension — smooth weight manifold"""
    name = "regular_ring"; category = "breakthrough_math"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        param = float(np.linalg.cond(t2d[:min(m,n),:min(m,n)])) if min(m,n) > 1 else 1.0
