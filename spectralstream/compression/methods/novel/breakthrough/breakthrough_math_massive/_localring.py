...
class LocalRing:
    """Local ring: unique maximal ideal — dominant singular component"""
    name = "local_ring"; category = "breakthrough_math"

    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        m, n, k, Uk, Sk, Vk = _svd_core(tensor, rank)
        param = float(Sk[0] / max(np.sum(Sk[1:]), 1e-30)) if k > 1 else 1.0
