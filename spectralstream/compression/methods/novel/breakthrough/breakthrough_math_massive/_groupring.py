...
class GroupRing:
    """Group ring: convolution algebra on the group of permutations"""
    name = "group_ring"; category = "breakthrough_math"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        param = float(np.mean(np.abs(np.correlate(t2d[0,:min(n,64)], t2d[0,:min(n,64)], mode='valid'))))
