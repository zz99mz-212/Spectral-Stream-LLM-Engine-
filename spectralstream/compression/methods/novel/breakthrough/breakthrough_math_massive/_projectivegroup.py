...
class ProjectiveGroup:
    """PGL(n): projective linear group acting on homogeneous weight coordinates"""
    name = "projective_group"; category = "breakthrough_math"
    ...
    def compress(self, tensor: np.ndarray, rank: int = 8) -> Tuple[bytes, dict]:
        ...
        param = float(np.mean(t2d[:,:min(n,3)] @ np.array([1,-1,1]))) if n >= 3 else 0.0
