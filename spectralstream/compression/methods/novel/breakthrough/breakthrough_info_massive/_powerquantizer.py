...
class PowerQuantizer:
    ...
    def compress(self, tensor, **params):
        ...
        if amax < 1e-10:
            return _block_int8_fallback(tensor)
        normed = np.abs(flat) / amax
        transformed = normed**power
        q = np.clip(np.round(transformed * 127), 0, 127).astype(np.uint8)
        header = struct.pack("<Iff", n, amax, power)
        return header + q.tobytes() + signs.astype(np.int8).tobytes(), {
            "n": n,
            "power_quant": True,
        }

    def decompress(self, data, metadata):
        n, amax, power = struct.unpack_from("<Iff", data, 0)
        q = np.frombuffer(data[12 : 12 + n], dtype=np.uint8).astype(np.float32) / 127.0
        signs = np.frombuffer(data[12 + n : 12 + n * 2], dtype=np.int8).astype(
            np.float32
        )
        return signs * amax * (q ** (1.0 / power))
