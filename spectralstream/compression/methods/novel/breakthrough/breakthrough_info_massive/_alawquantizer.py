...
class ALawQuantizer:
    ...
    def compress(self, tensor, **params):
        ...
        n = len(flat)
        A = 87.6
        abs_norm = np.abs(flat) / (float(np.max(np.abs(flat))) + 1e-10)
        signs = np.sign(flat)
        mask = abs_norm < 1.0 / A
        companded = np.where(
            mask, A * abs_norm / (1.0 + np.log(A)), 1.0 + np.log(A * abs_norm)
        ) / (1.0 + np.log(A))
        companded = signs * companded
        q = np.clip(np.round(companded * 127), -128, 127).astype(np.int8)
        header = struct.pack("<If", n, A)
        return header + q.tobytes(), {"n": n, "alaw": True}

    def decompress(self, data, metadata):
        n, A = struct.unpack_from("<If", data, 0)
        q = np.frombuffer(data[8:], dtype=np.int8).astype(np.float32) / 127.0
        signs = np.sign(q)
        abs_q = np.abs(q)
        mask = abs_q < 1.0 / (1.0 + np.log(A))
        expanded = np.where(
            mask,
            abs_q * (1.0 + np.log(A)) / A,
            np.exp(abs_q * (1.0 + np.log(A)) - 1.0) / A,
        )
        return signs * expanded[:n]
