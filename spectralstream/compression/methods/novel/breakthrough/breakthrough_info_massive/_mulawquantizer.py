...
class MuLawQuantizer:
    ...
    def compress(self, tensor, **params):
        ...
        companded = signs * np.log1p(mu * abs_norm) / np.log1p(mu)
        q = np.clip(np.round(companded * 127), -128, 127).astype(np.int8)
        header = struct.pack("<If", n, mu)
        return header + q.tobytes(), {"n": n, "mu": mu, "mulaw": True}

    def decompress(self, data, metadata):
        n, mu = struct.unpack_from("<If", data, 0)
        q = np.frombuffer(data[8:], dtype=np.int8).astype(np.float32) / 127.0
        signs = np.sign(q)
        abs_q = np.abs(q)
        expanded = signs * (1.0 / mu) * ((1.0 + mu) ** abs_q - 1.0)
        return expanded[:n]
