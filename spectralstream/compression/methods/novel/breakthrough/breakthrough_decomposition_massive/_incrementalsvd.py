...
class IncrementalSVD:
    """IncrementalSVD:         k = params.get('rank', max(1, min(m, n) // 8))
        # Simulate incremental: process in blocks
        block m = min(m, 256)
        U, S, Vt = np.linalg.svd(mat[:block m], full matrices=False)."""
    name = "incremental_svd"
    ...
    def compress(self, tensor: np.ndarray, **params) -> Tuple[bytes, dict]:
        ...
        m, n = mat.shape
        k = params.get('rank', max(1, min(m, n) // 8))
        # Simulate incremental: process in blocks
        block_m = min(m, 256)
        U, S, Vt = np.linalg.svd(mat[:block_m], full_matrices=False)
        k = min(k, m, n)
