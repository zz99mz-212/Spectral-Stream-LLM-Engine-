...
class CrossValidationSVD:
    """CrossValidationSVD:         k = params.get('rank', max(1, min(m, n) // 6))
        if hasattr(params, 'cv fold'):
            k = max(1, min(m, n) // 10)."""
    name = "cross_validation_svd"
