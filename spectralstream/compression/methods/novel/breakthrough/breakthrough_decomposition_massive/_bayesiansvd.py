...
class BayesianSVD:
    """BayesianSVD:         k = params.get('rank', max(1, min(m, n) // 6))
        # Bayesian-inspired: choose rank that explains >90% variance
        s full = np.linalg.svd(mat, full matrices=False)[1]
        total = np.sum(s full) + 1e-30
        cum = np.cumsum(s full) / total
        k = max(1, int(np.searchsorted(cum, 0.90) + 1))
        k = min(k, max(1, min(m, n) // 3))."""
    name = "bayesian_svd"
    ...
    def compress(self, tensor: np.ndarray, **params) -> Tuple[bytes, dict]:
        ...
        m, n = mat.shape
        k = params.get('rank', max(1, min(m, n) // 6))
        # Bayesian-inspired: choose rank that explains >90% variance
        s_full = np.linalg.svd(mat, full_matrices=False)[1]
        total = np.sum(s_full) + 1e-30
        cum = np.cumsum(s_full) / total
        k = max(1, int(np.searchsorted(cum, 0.90) + 1))
        k = min(k, max(1, min(m, n) // 3))
        k = min(k, m, n)
