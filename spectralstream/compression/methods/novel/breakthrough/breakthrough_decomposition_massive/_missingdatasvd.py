...
class MissingDataSVD:
    """MissingDataSVD:         k = params.get('rank', max(1, min(m, n) // 6))
        # Simple mean imputation for missing data
        col means = np.nanmean(mat, axis=0)
        mat = np.where(np.isnan(mat), col means, mat)."""
    name = "missing_data_svd"
    ...
    def compress(self, tensor: np.ndarray, **params) -> Tuple[bytes, dict]:
        ...
        mat = np.where(np.isnan(mat), col_means, mat)
        k = min(k, m, n)
