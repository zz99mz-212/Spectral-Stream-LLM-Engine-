...
class ComplexSVD:
    """ComplexSVD:         k = params.get('rank', max(1, min(m, n) // 5))
        # Complex embedding: interleave real/imag as complex
        mat c = mat[:, :n//2*2].view(np.complex128).reshape(m, -1)."""
    name = "complex_svd"
    ...
    def compress(self, tensor: np.ndarray, **params) -> Tuple[bytes, dict]:
        ...
        # Complex embedding: interleave real/imag as complex
        mat_c = mat[:, :n//2*2].view(np.complex128).reshape(m, -1)
        k = min(k, m, n)
