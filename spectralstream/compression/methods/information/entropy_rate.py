...
class EntropyRateCoding:
    name = "entropy_rate_coding"
    ...
    def _estimate_entropy_rate(self, data: np.ndarray) -> float:
    ...
    def compress(
        ...
        entropy = self._estimate_entropy_rate(quantized.ravel())
        ...
        meta = {"orig_shape": orig_shape, "method": self.name, "entropy_rate": entropy}
        return data, meta
    ...
    def estimate_ratio(self, tensor: np.ndarray, **kwargs) -> float:
        orig = tensor.nbytes
        comp = tensor.size * self.n_bits / 8
