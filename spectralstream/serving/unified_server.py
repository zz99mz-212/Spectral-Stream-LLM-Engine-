...
from spectralstream.serving.api._chatmessage import ChatMessage
from spectralstream.serving.api._chatcompletionrequest import ChatCompletionRequest
from spectralstream.serving.api._completionrequest import CompletionRequest
from spectralstream.serving.api._modelinfo import ModelInfo
from spectralstream.serving.api._modellistresponse import ModelListResponse
from spectralstream.serving.api._compressionrequest import CompressionRequest
from spectralstream.serving.api._compressionstatus import CompressionStatus
from spectralstream.serving.api._tokenizer import Tokenizer
from spectralstream.serving.api._sessionstate import SessionState
from spectralstream.serving.api._continuousbatcher import ContinuousBatcher
from spectralstream.serving.streaming import TokenStreamer, format_sse, StreamChunk
from spectralstream.serving.streaming_handler import (
    StreamingHandler,
    parse_tool_call,
    build_tool_call_chunk,
)
from spectralstream.serving.model_manager import ModelManager, ModelEntry
from spectralstream.serving.request_queue import RequestQueue, Priority as QueuePriority
...
def _detokenize_fallback(token_ids: list[int]) -> str:
...
class UnifiedSpectralServer:
    def __init__(self, config: Optional[ServerConfig] = None):
        ...
        self.app = FastAPI(
            ...
            description=self.config.metadata.get(
                "description",
                "Unified API server with OpenAI + Anthropic compatibility",
            ),
            docs_url="/docs",
            redoc_url="/redoc",
        )
        ...
        self.request_queue = RequestQueue(
            max_concurrent=self.config.max_concurrent_sessions, max_queued=256
        )
    ...
    def _track_error(self, error_type: str):
        ...
        self._metrics["error_by_type"][error_type] = (
            self._metrics["error_by_type"].get(error_type, 0) + 1
        )
    ...
    def _setup_routes(self):
        ...
        def _check_rate_limit(key: str, cost: float = 1.0) -> bool:
            return server.rate_limiter.check(key, cost)
        ...
        async def health():
            ...
            queue_stats = server.request_queue.stats()
            return {
                "status": "healthy",
                "version": SERVER_VERSION,
                "models_loaded": len(server.loaded_models),
                "api_keys_configured": len(server.api_key_store),
                "device": "cpu",
        ...
        async def list_models():
            ...
            for name, info in server.loaded_models.items():
                models.append(ModelInfo(id=name, created=int(info.get("loaded_at", 0))))
            if not models and server.config.model_path:
                models.append(
                    ModelInfo(
                        id=Path(server.config.model_path).stem, created=int(time.time())
                    )
                )
            models.append(ModelInfo(id="spectralstream", created=int(time.time())))
            return ModelListResponse(data=models)

        @app.get("/v1/models/{model_id}")
        async def get_model(model_id: str):
            entry = (
                server.model_manager.get(model_id)
                if hasattr(server.model_manager, "get")
                else None
            )
            if entry:
                return {
            ...
            if model_id in server.loaded_models:
                return {
                    "id": model_id,
                    "object": "model",
                    "created": int(server.loaded_models[model_id].get("loaded_at", 0)),
                    "owned_by": "spectralstream",
                }
            return {
                "id": model_id,
        ...
        async def chat_completions(
            ...
            server._track_request(
                "chat",
                len(prompt_tokens),
                len(result.get("tokens", [])) if result else 0,
            )
            ...
            return {
                ...
                "usage": {
                    ...
                    "completion_tokens": len(result.get("tokens", [])) if result else 0,
                    "total_tokens": len(prompt_tokens) + len(result.get("tokens", []))
                    if result
                    else 0,
                },
            }
        ...
        async def embeddings(
            ...
            model = body.get("model", "spectralstream")
            ...
            for i, text in enumerate(inputs):
                ...
                if encoding_format == "base64":
                    ...
                    embedding_val = vec
                data.append(
                    {"object": "embedding", "index": i, "embedding": embedding_val}
                )

            server._track_request("embedding", total_tokens, 0)
            return {
                "object": "list",
                "data": data,
                "model": model,
        ...
        async def anthropic_messages(
            ...
            prompt_tokens = tokenizer.encode(prompt)
            session_id = server.batcher.add_session(
                prompt_tokens=prompt_tokens, max_tokens=max_tokens, temperature=0.7
            )
            ...
            server._track_request("anthropic", len(prompt_tokens), completion_tokens)
        ...
        async def admin_compress(
            ...
            return {
                ...
                "model_path": body.get("model_path", ""),
                "target_ratio": body.get("target_ratio", 5000.0),
            }

        @app.get("/metrics")
        async def metrics():
            lines = []
            for key, val in server._metrics.items():
                if isinstance(val, (int, float)):
                    lines.append(f"# HELP spectralstream_{key} {key}")
                    lines.append(f"# TYPE spectralstream_{key} counter")
                    lines.append(f"spectralstream_{key} {val}")
            lines.append(f"# HELP spectralstream_uptime_seconds Server uptime")
            lines.append(f"# TYPE spectralstream_uptime_seconds gauge")
            lines.append(
                f"spectralstream_uptime_seconds {round(time.time() - server._start_time, 2)}"
            )
            return StreamingResponse(
                iter(["\n".join(lines) + "\n"]),
        ...
        async def certificate_page(request: Request):
            ...
            return server.templates.TemplateResponse(
                ...
                {"request": request, "version": SERVER_VERSION},
            )
        ...
        async def compress_page(request: Request):
            ...
            return server.templates.TemplateResponse(
                "compress.html", {"request": request, "version": SERVER_VERSION}
            )
                    ...
                    models.append(
                        {
                            ...
                            "name": str(rel),
                            "size_gb": round(safetensors_file.stat().st_size / 1e9, 2),
        ...
        async def list_compression_methods(
            category: Optional[str] = None, tier: Optional[int] = None
        ):
            ...
            patterns = [
                {
                    ...
                    "description": "Multi-stage aggressive quantization cascade",
                },
                {
                    ...
                    "description": "Spectral decomposition cascade",
                },
                {
                    ...
                    "description": "Balanced hybrid cascade",
                },
                {
                    ...
                    "description": "Progressive quantization refinement",
                },
                {
                    "name": "entropy_first",
                    "stages": [
                    ...
                    "description": "Entropy-aware cascade",
                },
            ]
                ...
                for name, tensor in tensors.items():
                    ...
                    for method_info in all_methods[:10]:
                        ...
                        try:
                            ...
                            ct = engine.compress_tensor_with_validation(
                                tensor, profile, [method_name], req.max_error
                            )
                            ...
                            results.append(
                                {"tensor": name, "method": method_name, "error": str(e)}
                            )
    ...
    async def _stream_chat(
        ...
        for i in range(max_gen):
            ...
            new_text = tokenizer.decode(tokens[-1:]) if tokens else ""
            chunk = {
                "id": f"chatcmpl-{session_id[:8]}",
    ...
    def _start_lan_discovery(self):
        try:
            ...
            service_info = ServiceInfo(
                ...
                properties={"version": SERVER_VERSION, "path": "/v1/health"},
            )
            self._zeroconf = Zeroconf()
            self._zeroconf.register_service(service_info)
            logger.info(f"LAN discovery active on port {self.config.port}")
        except ImportError:
    ...
    def _run_compression_job(
        ...
        try:
            ...
            config = CompressionConfig(
                target_ratio=target_ratio, max_error=max_error, streaming=streaming
            )
            engine = CompressionIntelligenceEngine(config)
            io = _SafetensorsIO()
            ...
            for i, (name, info) in enumerate(tensor_info.items()):
                try:
                    ...
                    profiles[name] = TensorProfile(name=name)
                    logger.warning("Profile failed for %s", name)
            ...
            if compressed:
                ...
                job["elapsed"] = time.perf_counter() - job["started_at"]
                cert = CertificateBuilder.from_compression_report(
                    report, model_name=Path(model_path).stem, model_architecture="auto"
                )
                job["result"] = {
                    "report": report.to_dict(),
                    "certificate_json": cert.to_dict(),
            ...
            self.compression_jobs[job_id]["status"] = "failed"
            self.compression_jobs[job_id]["error"] = str(e)
            self._compression_telemetry["active_jobs"].pop(job_id, None)
    ...
    def _get_telemetry(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_ratio": round(total_ratio / max(completed, 1), 2),
            "avg_error": round(total_error / max(completed, 1), 6),
            "timestamp": datetime.utcnow().isoformat(),
        }
    ...
    def run(self):
        ...
        logger.info(
            f"Starting SpectralStream Unified API on {self.config.host}:{self.config.port}"
        )
        ...
        logger.info(
            f"  Anthropic API: http://{self.config.host}:{self.config.port}/v1/messages"
        )
        logger.info(
